...
# - fix recall@5 by setting both voting_k and top_k to 5
#FIXED_RECALL_K=10
...
# Decode Task ID (NUM_CHUNKS * NUM_GLOSSARIES tasks)
TASK_ID=${SLURM_ARRAY_TASK_ID:-0}
CHUNK_IDX=$((TASK_ID % NUM_CHUNKS))
G_IDX=$((TASK_ID / NUM_CHUNKS))

TOTAL_TASKS=$((NUM_CHUNKS * NUM_GLOSSARIES))
...
RAG_MODEL_PATH="${RAG_MODEL_PATHS[$CHUNK_IDX]}"
...
# 输出目录 (包含超参信息)
OUTPUT_BASE="/mnt/gemini/data2/jiaxuanluo/infinisst_omni_vllm_rag_sweep_v4_chunk_size"
...
OUTPUT_PATH="${OUTPUT_BASE}/${INDEX_NAME}_m${M_TAG}_cs${CUR_CHUNK}_hs${CUR_HOP}_rk${FIXED_RECALL_K}_vk${FIXED_VOTING_K}"
