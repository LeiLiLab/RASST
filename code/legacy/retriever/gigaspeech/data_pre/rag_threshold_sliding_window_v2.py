...
CONFIG = {
    ...
    "grid_overlap_min": 1.5,      # 最小 overlap（秒）
    "grid_overlap_max": 1.5,      # 最大 overlap（秒）
    "grid_overlap_step": 0.1,     # overlap 步长（秒）
    "grid_threshold_values": [0.75,0.80,0.85],  # threshold 取值
    ...
    "rag_batch_size": 64,  # 批量推理的 batch size
}
...
def load_ground_truth(txt_path, glossary_terms):
    ...
    ⚠️ 重要：只保留在 glossary_terms 中的术语，过滤掉不在 glossary 中的术语。
    """
    ground_truths = []
    total_tagged_terms = 0
    filtered_out_terms = 0
    filtered_out_examples = set()  # 记录被过滤掉的术语示例
    ...
    for line in lines:
        ...
        for term in matches:
            ...
            total_tagged_terms += 1
            
            # 关键过滤：只保留在 glossary 中的术语
            if term_lower in glossary_terms:
                gt_in_sent.add(term_lower)
            else:
                filtered_out_terms += 1
                if len(filtered_out_examples) < 10:  # 只记录前10个示例
                    filtered_out_examples.add(term_lower)
        
        ground_truths.append(gt_in_sent)
    
    # 打印过滤统计
    print(f"\n=== Ground Truth 加载统计 ===")
    print(f"总标注术语数: {total_tagged_terms}")
    print(f"在 glossary 中的术语: {total_tagged_terms - filtered_out_terms}")
    print(f"不在 glossary 中（已过滤）: {filtered_out_terms}")
    if filtered_out_terms > 0:
        filter_rate = filtered_out_terms / total_tagged_terms * 100
        print(f"过滤率: {filter_rate:.2f}%")
        if filtered_out_examples:
            print(f"被过滤术语示例: {', '.join(list(filtered_out_examples)[:10])}")
    print("="*80)
...
def print_sample_analysis(predictions, ground_truths, txt_path, threshold=0.80, num_samples=10):
    """
    打印前 N 个句子的详细分析
    ...
    """
    print("\n" + "="*100)
    print(f"前 {num_samples} 个句子的详细分析 (Threshold={threshold:.2f})")
    print("="*100)
    ...
    for i in range(min(num_samples, len(predictions), len(ground_truths), len(lines))):
        ...
        print(f"句子 #{i+1}")
        print(f"{'─'*100}")
        print(f"原文: {clean_text[:150]}{'...' if len(clean_text) > 150 else ''}")
        
        # Ground Truth
        print(f"\n📌 Ground Truth ({len(gt_set)} terms):")
        if gt_set:
            for term in sorted(gt_set):
                print(f"   ✓ {term}")
        else:
            print(f"   (无标注术语)")
        
        # 所有召回的术语（未过滤）
        print(f"\n🔍 RAG 召回结果 (Top-{len(pred_dict)}, 未过滤):")
        if pred_dict:
            # 按分数降序排序
            sorted_preds = sorted(pred_dict.items(), key=lambda x: x[1], reverse=True)
            for term, score in sorted_preds:
                # 判断是否在 GT 中
                in_gt = "✅ TP" if term in gt_set else "❌ FP"
                # 判断是否通过阈值
                pass_threshold = "✓" if score >= threshold else "✗"
                print(f"   {pass_threshold} [{score:.4f}] {term:30s} {in_gt}")
        else:
            print(f"   (无召回)")
        
        # 过滤后的结果
        filtered_preds = {t: s for t, s in pred_dict.items() if s >= threshold}
        print(f"\n⚡ 过滤后结果 (Threshold ≥ {threshold:.2f}):")
        if filtered_preds:
            tp = sum(1 for t in filtered_preds if t in gt_set)
            fp = len(filtered_preds) - tp
            print(f"   召回 {len(filtered_preds)} 个术语 (TP={tp}, FP={fp})")
            for term, score in sorted(filtered_preds.items(), key=lambda x: x[1], reverse=True):
                in_gt = "✅" if term in gt_set else "❌"
                print(f"   {in_gt} [{score:.4f}] {term}")
        else:
            print(f"   (全部被过滤)")
        
        # 漏检的术语
        fn_terms = gt_set - set(pred_dict.keys())
        if fn_terms:
            print(f"\n❗ 漏检术语 (FN={len(fn_terms)}):")
            for term in sorted(fn_terms):
                print(f"   ✗ {term} (未召回)")
        
        # 统计
        tp = len(set(filtered_preds.keys()) & gt_set)
        fp = len(filtered_preds) - tp
        fn = len(gt_set) - tp
        ...
        print(f"\n📊 本句统计: TP={tp}, FP={fp}, FN={fn} | P={precision:.3f}, R={recall:.3f}, F1={f1:.3f}")
    
    print("\n" + "="*100 + "\n")

def diagnose_errors(predictions, ground_truths, threshold=0.80, output_dir=None, timestamp=None):
    """
    诊断错误：分析哪些词容易误报/漏检
    ...
    print(f"\n" + "="*80)
    print(f"错误诊断 (Threshold={threshold:.2f})")
    print("="*80)
    
    fp_counter = Counter()  # 记录哪些词经常被误报
    fn_counter = Counter()  # 记录哪些词经常被漏检
    tp_counter = Counter()  # 记录哪些词检测得最准
    ...
    for pred_dict, gt_set in zip(predictions, ground_truths):
        # 1. 应用阈值过滤
        filtered_preds = {t for t, s in pred_dict.items() if s >= threshold}
        
        # 2. 统计
        # TP & FP
        for term in filtered_preds:
    ...
    print(f"\n总体统计:")
    print(f"  总预测数: {total_preds}, 总GT数: {total_gt}")
    ...
    print("\n" + "-"*80)
    print("Top 10 '幻觉'词 (False Positives)")
    print("这些词模型即使没听到也喜欢瞎报，建议检查是否过于通用：")
    print("-"*80)
    for i, (term, count) in enumerate(fp_counter.most_common(10), 1):
        percentage = count / total_fp * 100 if total_fp > 0 else 0
        print(f"  {i:2d}. {term:30s} : {count:3d} 次 ({percentage:.1f}%)")
    
    print("\n" + "-"*80)
    print("Top 10 '耳背'词 (False Negatives)")
    print("这些词模型听到了但分低，可能是生僻词或发音不清：")
    print("-"*80)
    for i, (term, count) in enumerate(fn_counter.most_common(10), 1):
        percentage = count / total_fn * 100 if total_fn > 0 else 0
        print(f"  {i:2d}. {term:30s} : {count:3d} 次 ({percentage:.1f}%)")
    
    print("\n" + "-"*80)
    print("Top 10 检测最准的词 (True Positives)")
    print("-"*80)
    for i, (term, count) in enumerate(tp_counter.most_common(10), 1):
        percentage = count / total_tp * 100 if total_tp > 0 else 0
        print(f"  {i:2d}. {term:30s} : {count:3d} 次 ({percentage:.1f}%)")
    ...
    if output_dir and timestamp:
        ...
        # FP 统计
        fp_csv = os.path.join(output_dir, f'error_diagnosis_fp_{timestamp}.csv')
        with open(fp_csv, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['rank', 'term', 'count', 'percentage'])
            for i, (term, count) in enumerate(fp_counter.most_common(), 1):
                percentage = count / total_fp * 100 if total_fp > 0 else 0
                writer.writerow([i, term, count, f'{percentage:.2f}'])
        
        # FN 统计
        fn_csv = os.path.join(output_dir, f'error_diagnosis_fn_{timestamp}.csv')
        with open(fn_csv, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['rank', 'term', 'count', 'percentage'])
            for i, (term, count) in enumerate(fn_counter.most_common(), 1):
                percentage = count / total_fn * 100 if total_fn > 0 else 0
                writer.writerow([i, term, count, f'{percentage:.2f}'])
        
        # TP 统计
        tp_csv = os.path.join(output_dir, f'error_diagnosis_tp_{timestamp}.csv')
        with open(tp_csv, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['rank', 'term', 'count', 'percentage'])
            for i, (term, count) in enumerate(tp_counter.most_common(), 1):
                percentage = count / total_tp * 100 if total_tp > 0 else 0
                writer.writerow([i, term, count, f'{percentage:.2f}'])
...
def validate_distance_metric(rag_retriever, num_samples=100):
    """
    验证 FAISS 距离度量和转换公式的正确性
    
    检查：
    1. FAISS index 类型
    2. 距离值范围（应该在 [0, 4] 之间，因为是 squared L2）
    3. Confidence 范围（应该在 [0, 1] 之间）
    """
    if not rag_retriever or not rag_retriever.enabled:
        print("⚠️  RAG retriever 未启用，跳过距离度量验证")
        return
    
    print("\n" + "="*80)
    print("验证 FAISS 距离度量和转换公式")
    print("="*80)
    
    # 检查 FAISS index 类型
    index_type = type(rag_retriever.index).__name__
    print(f"FAISS Index 类型: {index_type}")
    
    # 生成随机样本进行测试
    sample_len = int(2.0 * 16000)  # 2秒音频
    test_samples = [
        np.random.uniform(-0.5, 0.5, sample_len).astype(np.float32) 
        for _ in range(min(num_samples, 10))
    ]
    ...
    try:
        with torch.no_grad():
            embeddings = rag_retriever.model.encode_audio(test_samples)
        
        if isinstance(embeddings, torch.Tensor):
            embeddings = embeddings.detach().cpu().float().numpy()
        
        # 搜索
        D, I = rag_retriever.index.search(embeddings, 5)
        
        for d_row in D:
            for d in d_row:
                if d >= 0:  # 有效的距离
                    distances.append(float(d))
                    conf = rag_retriever._distance_to_confidence(float(d))
                    confidences.append(conf)
        
        if distances:
            print(f"\n距离统计 (n={len(distances)}):")
            ...
            print(f"  Std: {np.std(confidences):.6f}")
            
            # 验证范围
            if 'L2' in index_type or 'Flat' in index_type:
                # 对于归一化向量，squared L2 距离应该在 [0, 4] 之间
                # 但实际通常在 [0, 2] 之间（相似的向量）
                if np.max(distances) > 4.1:
                    print(f"⚠️  警告：距离最大值 {np.max(distances):.4f} > 4，可能未归一化或索引类型不匹配！")
                elif np.max(distances) > 2.5:
                    print(f"ℹ️  注意：距离最大值 {np.max(distances):.4f} > 2.5，检索到的术语相似度较低")
                else:
                    print(f"✅ 距离范围正常（0-2之间）")
            
            # 转换公式验证
            print(f"\n转换公式验证:")
            print(f"  当前公式: cosine = 1.0 - 0.5 × distance")
            print(f"            confidence = (cosine + 1.0) / 2.0")
            ...
            for d in test_distances:
                cosine = 1.0 - 0.5 * d
                conf = (cosine + 1.0) / 2.0
                print(f"    distance={d:.2f} → cosine={cosine:.3f} → confidence={conf:.3f}")
            
            print(f"\n  期望行为:")
            print(f"    - distance=0.0 (完全相同) → confidence=1.0")
            print(f"    - distance=2.0 (正交) → confidence=0.5")
            print(f"    - distance=4.0 (相反) → confidence=0.0")
            
            if 'L2' in index_type:
                print(f"\n✅ 索引类型为 L2，转换公式适用于归一化向量的 squared L2 距离")
            elif 'IP' in index_type:
                print(f"\n⚠️  警告：索引类型为 Inner Product (IP)，当前转换公式可能不适用！")
                print(f"    IP 返回的是 <a,b>，范围通常是 [-1, 1] 或 [0, 1]（归一化后）")
...
def plot_score_distribution(predictions, ground_truths, output_dir, filename_prefix="score_distribution", overlap=None):
    """
    绘制分数分布图
    ...
    title = 'Score Distribution: Positive vs Negative Samples'
    if overlap is not None:
        stride = CONFIG["chunk_duration"] - overlap
        title += f'\nOverlap={overlap:.1f}s, Stride={stride:.1f}s'
    ...
    dist_path = os.path.join(output_dir, f'{filename_prefix}.png')
    ...
    return {
        ...
        'positive_mean': np.mean(positive_scores) if positive_scores else 0,
        'positive_std': np.std(positive_scores) if positive_scores else 0,
        'negative_mean': np.mean(negative_scores) if negative_scores else 0,
        'negative_std': np.std(negative_scores) if negative_scores else 0,
    }
...
def grid_search_2d_overlap_threshold(
    ...
    for overlap in tqdm(overlap_values, desc="Overlap 分析"):
        ...
        # C. 绘制该 overlap 的分数分布图
        if output_dir:
            filename = f"score_distribution_overlap_{overlap:.1f}"
            score_stats = plot_score_distribution(
                predictions, 
                ground_truths, 
                output_dir, 
            ...
            overlap_results[overlap]["score_stats"] = score_stats
    ...
    # 第三步：错误诊断（使用最佳配置）
    print("\n第 3 步：错误诊断...")
    best_predictions = overlap_results[best_overlap]["predictions"]
    error_diagnosis = diagnose_errors(
        best_predictions,
        ground_truths,
    ...
    if output_dir:
        ...
        # 保存分数统计信息
        score_stats_csv_path = os.path.join(output_dir, f'grid_search_2d_score_stats_{timestamp}.csv')
        with open(score_stats_csv_path, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow(['overlap', 'stride', 'positive_mean', 'positive_std', 'negative_mean', 'negative_std', 'separation'])
            for overlap in overlap_values:
                stride = CONFIG["chunk_duration"] - overlap
                if overlap in overlap_results and 'score_stats' in overlap_results[overlap]:
                    stats = overlap_results[overlap]['score_stats']
                    pos_mean = stats['positive_mean']
                    pos_std = stats['positive_std']
                    neg_mean = stats['negative_mean']
                    neg_std = stats['negative_std']
                    # 计算分离度（两个分布的均值差）
                    separation = pos_mean - neg_mean
                    writer.writerow([
                        f'{overlap:.1f}', f'{stride:.1f}', 
                        f'{pos_mean:.4f}', f'{pos_std:.4f}', 
                        f'{neg_mean:.4f}', f'{neg_std:.4f}',
                        f'{separation:.4f}'
                    ])
        print(f"分数统计已保存至: {score_stats_csv_path}")
...
if __name__ == "__main__":
    ...
    # 2.5 验证 FAISS 距离度量和转换公式
    validate_distance_metric(rag_retriever, num_samples=100)
