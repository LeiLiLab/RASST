#!/bin/bash
#SBATCH --job-name=v4_taurus_pipeline
#SBATCH --nodes=1
...
#SBATCH --output=/mnt/taurus/home/jiaxuanluo/InfiniSST/retriever/gigaspeech/modal/logs/%j_v4_taurus_for_ja.out
#SBATCH --error=/mnt/taurus/home/jiaxuanluo/InfiniSST/retriever/gigaspeech/modal/logs/%j_v4_taurus_for_ja.err
...
INPUT_GT_STAGE1="/mnt/gemini/data/jiaxuanluo/manifests_rag/train_s_ja_baseline.jsonl"
INPUT_TSV="/mnt/gemini/data1/jiaxuanluo/train_xl_case_robust_asr-filtered_ja_metricx-qe3.0_align.tsv"
NER_CANDIDATES_OUTPUT="/mnt/gemini/data1/jiaxuanluo/train_s_ja_ner_candidates_${SPACY_MODEL}.jsonl"
STAGE1_OUTPUT="/mnt/gemini/data1/jiaxuanluo/train_s_ja_v4_ner_baseline_aligned_${SAMPLING_STR}.jsonl"
STAGE2_OUTPUT_BASE="/mnt/gemini/data1/jiaxuanluo/train_s_ja_v4_ner_baseline_aligned_${SAMPLING_STR}_final"
TARGET_LANG_CODE="ja"
...
if [[ "$MODE" == "all" || "$MODE" == "stage0" ]]; then
    ...
    # CuPy wheels often rely on these being in LD_LIBRARY_PATH.
...
if [[ "$MODE" == "all" || "$MODE" == "stage1" ]]; then
    conda activate "${VLLM_ENV_PATH}"
    # Ensure pip-installed NVIDIA CUDA runtime libraries are discoverable for torch/vLLM as well.
    for d in "$CONDA_PREFIX"/lib/python*/site-packages/nvidia/*/lib; do
        if [ -d "$d" ]; then
            export LD_LIBRARY_PATH="$d:${LD_LIBRARY_PATH:-}"
        fi
...
if [[ "$MODE" == "all" || "$MODE" == "stage2" ]]; then
    ...
    preflight_vllm_env
    # LLM-driven negatives (match run_pipeline_v4_full_no_group_v3.sh)
    for i in "${!LOGICAL_GPUS[@]}"; do
        CUR_GPU="${LOGICAL_GPUS[$i]}"
