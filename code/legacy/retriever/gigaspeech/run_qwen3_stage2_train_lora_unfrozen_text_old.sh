...
#SBATCH --mem=256G
#SBATCH --gres=gpu:8
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_qwen3_unfrozen_text_old.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_qwen3_unfrozen_text_old.err
...
SCRIPT_PATH="/mnt/taurus/home/jiaxuanluo/InfiniSST/retriever/gigaspeech/qwen3_AuT_BGE_M3_train_lora_unfrozen_text_old.py"
RESUME_PATH="/mnt/gemini/data2/jiaxuanluo/q3rag_unfrozen_lora-r32-tr16_bs4k_w1.0-0.0_sampled_best_snapshot_v2.pt"
RESUME_PATH=""
# --- LoRA 消融实验配置区 ---
USE_LORA="true"
LORA_RANK=32
...
SAVE_NAME="old_q3rag_${MODE_NAME}_bs${BS_ABBR}_w${TERM_WEIGHT}-${TRANS_WEIGHT}-ttm=${TEXT_TARGET_MODULES}-temperature=${TEMPERATURE}"
...
if [ -n "${RESUME_PATH:-}" ]; then
    if [ -f "${RESUME_PATH}" ]; then
        OPTS="${OPTS} --resume ${RESUME_PATH}"
    else
        echo "[WRN] RESUME_PATH is set but file not found: ${RESUME_PATH}"
    fi
fi
