...
# Sweep configs (model x glossary x top_k). We intentionally remove threshold filtering
# by fixing score_threshold=0.0 for an easy-to-interpret upper bound diagnostic.
MODELS=(
  # m1-m4 retrievers aligned with chunk size multipliers (m = chunk_size / 0.96).
  "/mnt/gemini/data2/jiaxuanluo/q3rag_final_m1_best.pt"
  "/mnt/gemini/data2/jiaxuanluo/final_main_result_model_v1.pt"   # m2
  "/mnt/gemini/data2/jiaxuanluo/q3rag_final_m3_best.pt"
  "/mnt/gemini/data2/jiaxuanluo/q3rag_final_m4_best.pt"
)
GLOSSARIES=(
...
NUM_TOPKS=${#TOP_KS[@]}
TOTAL_TASKS=$((NUM_MODELS * NUM_GLOSSARIES * NUM_TOPKS))
...
echo "[INFO] Task mapping: task_id=${TASK_ID} model_idx=${M_IDX}/${NUM_MODELS} glossary_idx=${G_IDX}/${NUM_GLOSSARIES} topk_idx=${K_IDX}/${NUM_TOPKS}"
