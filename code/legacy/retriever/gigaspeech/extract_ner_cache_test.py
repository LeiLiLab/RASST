...
def extract_named_entities_test(tsv_path):
    """为测试数据集提取命名实体"""
    ...
    # 从本地TSV文件读取数据
    print(f"[INFO] 从 {tsv_path} 读取测试数据...")
    ...
    print(f"[INFO] 总共读取 {total_size} 个测试样本")
    ...
    print("[INFO] 开始提取命名实体...")
    for doc in tqdm(nlp.pipe(texts, batch_size=32), total=len(texts), ncols=100):
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="从测试TSV文件提取命名实体")
    parser.add_argument("--tsv_path", type=str, required=True, help="TSV file path containing test samples")
    args = parser.parse_args()
    extract_named_entities_test(tsv_path=args.tsv_path) 