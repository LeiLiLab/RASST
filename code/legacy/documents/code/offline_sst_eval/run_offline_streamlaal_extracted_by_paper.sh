...
# Compute StreamLAAL + BLEU + TERM metrics for offline instances.log using extracted per-paper glossary.
#
# - BLEU/StreamLAAL are computed once on the full instances.log (glossary-independent).
# - TERM metrics are computed by splitting instances per paper_id and using per-paper glossary
#   derived from extracted_glossary_with_translations.json (source_paper field).
#
...
if [[ ! -f "${EXTRACTED_GLOSSARY_MANIFEST}" ]]; then
  echo "[ERROR] EXTRACTED_GLOSSARY_MANIFEST not found: ${EXTRACTED_GLOSSARY_MANIFEST}" >&2
  exit "${EXIT_DATA_ERROR}"
fi
...
run_one() {
  ...
  local out_tsv="${offline_dir}/streamlaal_extracted_by_paper.tsv"
  local out_log="${offline_dir}/streamlaal_extracted_by_paper.json"
  local work_dir="${offline_dir}/__paper_eval_tmp__"

  echo "[INFO] Running extracted_by_paper eval: pair=${pair} lang=${lang_code}"
