...
GT_TERM_KEY = "term"
GT_VALUE_KEY = "zh"
# Hard cap for term_map pairs (GT terms are always preserved; negative terms are truncated first).
TERM_MAP_MAX_PAIRS = 30
# ======Configuration=====
        ...
        if budget <= 0:
            ...
            if budget < 0:
                print(
                    ...
                    "Keeping all GT terms and truncating all non-GT terms."
                )
            return out
        if len(sampled_non_gt) > budget:
            # Truncate negative terms deterministically for reproducibility.
            # (Sorting avoids dependence on dict insertion order across sources.)
            keys = sorted(sampled_non_gt.keys())
            keys = keys[:budget]
...
def main() -> None:
    ...
    ap.add_argument("--max-entries", type=int, default=TERM_MAP_MAX_PAIRS, help="Within kept chunks, cap term_map size (hard-capped by TERM_MAP_MAX_PAIRS)")
    ...
    # Enforce hard cap to avoid magic-number drift across callers.
    if args.max_entries is not None and args.max_entries > TERM_MAP_MAX_PAIRS:
        args.max_entries = int(TERM_MAP_MAX_PAIRS)
