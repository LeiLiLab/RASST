#!/bin/bash
#SBATCH --job-name=posneg_zv28ve3q_sec
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_posneg_zv28ve3q_sec.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_posneg_zv28ve3q_sec.err

# Pos/Neg sim-distribution diagnostic for the _best_acl6060_gs10000.pt
# (best_secondary/step=1240) snapshot of WandB run zv28ve3q.
#
# Unlike the threshold-sweep, this only produces the distribution:
#   - unbiased S_pos (every (has_term chunk, GT term) pair, not only top-10)
#   - S_neg_top{1,5,10,50} from a gs=10000 bank
#   - per-domain bucketing (gs_pod/gs_you/gs_aud/wiki_synth/acl6060)
#   - percentile TSV + alpha/beta suggestions for TCM tuning
#
# zv28ve3q = variantE smallest+dense MFA + pool HN k=64 + TCM(1.0, β=0.85, α=0.25).
...
MODEL_PATH="/mnt/gemini/home/jiaxuanluo/train_outputs/q3rag_scale_lora-r128-tr128_bs12k_t=0.07_3var_clean_gc_wr1000k_m0.0_maxsim_mfa_variantE_hardneg_k64_tcm_ep3_cold_smallest_dense_normAGGR_best_acl6060_gs10000.pt"
...
OUTPUT_DIR="/mnt/gemini/data2/jiaxuanluo/sim_dist_snapshot/zv28ve3q_secondary"
GS_SIZE=10000
TOPK_NEG=50

# zv28ve3q trained with alpha=0.25, beta=0.85 (so the plot vertical lines show where those sit).
CURRENT_ALPHA=0.25
CURRENT_BETA=0.85

# Model recipe (must match training of zv28ve3q).
LORA_RANK=128
LORA_ALPHA=256
TEXT_LORA_RANK=128
TEXT_LORA_ALPHA=256
...
echo "[POSNEG] zv28ve3q best_secondary pos/neg distribution"
