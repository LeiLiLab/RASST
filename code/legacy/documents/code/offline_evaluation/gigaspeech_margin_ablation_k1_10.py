...
"""
Offline evaluation: margin sweep for F1/F2/F3 on GigaSpeech dev term dataset.
...
Protocol:
  ...
  - Encode each audio chunk with the V4 audio retriever, retrieve Top-K1 candidates,
    and compute micro-averaged precision/recall and F1/F2/F3 for a margin sweep.

Margin definition (relative to top-10 score):
  - For each chunk, let s10 be the top-10 cosine similarity (lowest score among Top-10).
  - Keep a candidate with score s if (s - s10) >= D.
  - Sweep D and compute F-scores.
...
# Fixed Top-K1 for margin ablation.
K1 = 10

# Margin sweep settings (D = candidate_score - top10_score).
MARGIN_MIN = 0.0
MARGIN_MAX = 1.0
MARGIN_STEPS = 1001
MARGIN_USE_DATA_RANGE = True

# Evaluation limits (0 means no limit).
MAX_CHUNKS = 0

# Output paths
OUTPUT_DIR = "/mnt/gemini/data2/jiaxuanluo/offline_eval_margin_ablation_k1_10_gigaspeech_dev"
GLOSSARY_JSON_NAME = "gigaspeech_dev_terms_glossary.json"
INDEX_PKL_NAME = f"gigaspeech_dev_terms_index_v4_tr{TEXT_LORA_R}.pkl"
RESULT_TSV_NAME = "margin_f1_f2_f3_k1_10.tsv"
PLOT_PNG_NAME = "margin_f1_f2_f3_k1_10.png"
...
class ChunkPrediction:
    ...
    pos_indices: Set[int]
    top10_score: float
    candidates: List[Tuple[int, float]]  # (term_index, cosine_similarity)
...
def _build_margins(all_margins: List[float]) -> List[float]:
    min_margin = MARGIN_MIN
    max_margin = MARGIN_MAX
    if MARGIN_USE_DATA_RANGE and all_margins:
        min_margin = float(min(all_margins))
        max_margin = float(max(all_margins))
    if max_margin < min_margin:
        min_margin, max_margin = max_margin, min_margin
    if MARGIN_STEPS <= 1:
        return [min_margin]
    margins = np.linspace(min_margin, max_margin, MARGIN_STEPS, dtype=np.float64).tolist()
...
def main() -> int:
    ...
    global DEVICE, OUTPUT_DIR, MODEL_PATH, MARGIN_MIN, MARGIN_MAX, MARGIN_STEPS, MARGIN_USE_DATA_RANGE
    ...
    if env_margin_min:
        MARGIN_MIN = float(env_margin_min)
    env_margin_max = os.environ.get("OFFLINE_EVAL_MARGIN_MAX", "").strip()
    if env_margin_max:
        MARGIN_MAX = float(env_margin_max)
    env_margin_steps = os.environ.get("OFFLINE_EVAL_MARGIN_STEPS", "").strip()
    if env_margin_steps:
        MARGIN_STEPS = int(env_margin_steps)
    env_margin_data = os.environ.get("OFFLINE_EVAL_MARGIN_USE_DATA_RANGE", "").strip().lower()
    if env_margin_data in {"0", "false", "no"}:
        MARGIN_USE_DATA_RANGE = False
    elif env_margin_data in {"1", "true", "yes"}:
        MARGIN_USE_DATA_RANGE = True
    ...
    for start in range(0, len(chunk_pos_indices), EVAL_BATCH_SIZE):
        ...
        for i in range(B):
            ...
            if D.shape[1] > 0:
                top10_score = float(D[i][min(K1 - 1, D.shape[1] - 1)])
            else:
                top10_score = 0.0
            for dist, idx in zip(D[i], I[i]):
                ...
                all_margins.append(score - top10_score)
            predictions.append(ChunkPrediction(batch[i][0], batch[i][1], pos, top10_score, cand))

    margins = _build_margins(all_margins)
    if not margins:
        _err("No margins generated. Check margin settings and score collection.")
    ...
    for margin in margins:
        ...
        for pred in predictions:
            pred_set = {idx for idx, score in pred.candidates if (score - pred.top10_score) >= margin}
    ...
    if best_f1:
        _log(f"Best F1: margin={best_f1['margin']} f1={best_f1['f1']}")
    if best_f2:
        _log(f"Best F2: margin={best_f2['margin']} f2={best_f2['f2']}")
    if best_f3:
        _log(f"Best F3: margin={best_f3['margin']} f3={best_f3['f3']}")
    ...
    try:
        ...
        xs = [float(r["margin"]) for r in rows]
        ...
        plt.xlabel("Margin D (score - top10)")
        plt.ylabel("F-score")
        plt.title("GigaSpeech dev: Margin sweep (K1=10)")
