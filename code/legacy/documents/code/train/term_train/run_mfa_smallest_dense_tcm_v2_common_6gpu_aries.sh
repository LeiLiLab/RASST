...
# Common launcher body for TCM-v2 6-GPU Aries runs.
# Wrapper scripts are responsible for the SBATCH headers plus the variant-
# specific env vars (scope / thresholds / notes / naming).
...
TCM_POS_LOSS_WEIGHT="${TCM_POS_LOSS_WEIGHT:-0.25}"
TCM_NEG_LOSS_WEIGHT="${TCM_NEG_LOSS_WEIGHT:-1.0}"
TCM_POS_THRESHOLD="${TCM_POS_THRESHOLD:-0.74}"
: "${TCM_NEG_THRESHOLD:?TCM_NEG_THRESHOLD env var is required}"
...
TCM_WARMUP_STEPS="${TCM_WARMUP_STEPS:-100}"

HARD_NEG_K="${HARD_NEG_K:-0}"
HARD_NEG_K_PER_SAMPLE="${HARD_NEG_K_PER_SAMPLE:-512}"
...
EXTRA_WANDB_TAGS="${EXTRA_WANDB_TAGS:-compute:aries-6gpu}"
BASELINE_RUN_IDS="${BASELINE_RUN_IDS:-tys70s0y}"
# ======Configuration=====
...
echo "[TRAIN] TCM: base=${TCM_LOSS_WEIGHT} pos_w=${TCM_POS_LOSS_WEIGHT} neg_w=${TCM_NEG_LOSS_WEIGHT} T_beta=${TCM_POS_THRESHOLD} T_alpha=${TCM_NEG_THRESHOLD} scope=${TCM_NEG_SCOPE} topk=${TCM_NEG_TOPK} warmup=${TCM_WARMUP_STEPS}"
