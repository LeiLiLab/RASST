...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_temp_sweep.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_temp_sweep.err

# Temperature + margin sweep for threshold-based precision/F2 exploration.
# Runs 4 configs in parallel pairs (2 at a time), each using 4 GPUs.
#
# Configs:
#   A: T=0.03, margin=0.1  (current baseline -- for reference with new F2 metrics)
#   B: T=0.07, margin=0.0
#   C: T=0.07, margin=0.1
#   D: T=0.10, margin=0.0
...
GPUS_PER_RUN=4
PER_GPU_BATCH=1536
BATCH_SIZE=$((GPUS_PER_RUN * PER_GPU_BATCH))
...
run_config() {
    ...
    local VERSION="3var_clean_gc_wr$((WIKI_RANK / 1000))k_m${MARGIN}_maxsim_mfa_tsweep"
    local SAVE_NAME="q3rag_${MODE_NAME}_bs${BS_ABBR}_t=${TEMPERATURE}_${VERSION}"
    local SAVE_PATH="${SAVE_DIR}/${SAVE_NAME}.pt"
    local WANDB_EXP_NAME="tsweep_t${TEMPERATURE}_m${MARGIN}"

    echo "======================================================================"
    echo "[SWEEP] T=${TEMPERATURE}, margin=${MARGIN}, GPUs=${GPU_LIST}"
    echo "[SWEEP] Save: ${SAVE_PATH}"
    echo "[SWEEP] wandb: ${WANDB_EXP_NAME}"
    echo "======================================================================"
    ...
    echo "[SWEEP] T=${TEMPERATURE}, margin=${MARGIN} completed at $(date)"
}

# Run configs in parallel pairs (2 configs at a time, each on 4 GPUs).
for (( i=0; i<${#CONFIGS[@]}; i+=2 )); do
    ...
    for j in 0 1; do
        ...
        read -r T_VAL M_VAL <<< "${CONFIGS[$idx]}"
        GPU_LIST="${GPU_SETS[$j]}"
        MASTER_PORT=$((29960 + idx))

        run_config "${T_VAL}" "${M_VAL}" "${GPU_LIST}" "${MASTER_PORT}" &
        pids+=($!)
    done
    ...
    echo "[SWEEP] Pair $((i/2 + 1)) completed at $(date)"
done

echo "[SWEEP] All configs completed at $(date)"
