#!/bin/bash
#SBATCH --job-name=q3_mfa_hnps_k512_lambda5_6gpu
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_mfa_hnps_k512_lambda5_6gpu_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_mfa_hnps_k512_lambda5_6gpu_%x.err

# C1 — lambda-only ablation on top of tys70s0y (A1).
# Diffs vs run_mfa_smallest_dense_hnps_k512_6gpu_aries.sh (tys70s0y):
#   * TCM_LOSS_WEIGHT: 1.0 -> 5.0  (the only variable under test)
#   * TCM_WARMUP_STEPS: 0 -> 100   (safety ramp, NOT under ablation;
#     at step 0 L_pos+L_neg~=0.18, lambda*0.18=0.9 is ~10% of InfoNCE~9.2,
#     100-step linear ramp prevents early-step gradient spikes).
#   * naming: hnps_k512 -> hnps_k512_lambda5 (job, VERSION, WANDB_EXP_NAME).
#   * NOTES_FILE -> notes_lambda5_only.md
#   * BASELINE_RUN_IDS -> tys70s0y only (zv28ve3q uses pool HN, not comparable).
#   * EXTRA_WANDB_TAGS -> variant:hnps_k512_lambda5 compute:aries-6gpu
#   * MASTER_PORT 29964 -> 29965 (avoid collision if tys70s0y-style run is revived).
#
# Post-mortem context (A2 / we6ssrn7 retired): grad_cache_chunk_size and
# num_gpus are all-gather'd into a single global batch before InfoNCE/TCM
# (qwen3_glossary_neg_train.py:2124), so neither affects the loss. A2 also
# changed alpha/beta simultaneously; that stacked with lambda=5 and made the
# failure unattributable. C1 isolates lambda.
#
# Submit:
#   sbatch run_mfa_smallest_dense_lambda5_6gpu_aries.sh
#   MAX_STEPS=50 sbatch run_mfa_smallest_dense_lambda5_6gpu_aries.sh  (smoke)

set -euo pipefail

# ======Configuration=====
# --- Loss (A1 baseline kept verbatim; only lambda changed) ---
MARGIN="0.0"
HCL_BETA="0.0"
TCM_LOSS_WEIGHT="5.0"
...
# --- Data-side hard-neg (A1 baseline kept verbatim) ---
HARD_NEG_K=0
HARD_NEG_K_PER_SAMPLE=512
NEG_BANK_SIZE=0
NEG_BANK_REFRESH_STEPS=50

# --- term_id normalization (bug fix, same as A1) ---
TERM_ID_NORMALIZE="aggressive"

# --- Same smallest+dense MFA recipe as A1 ---
...
NOTES_FILE="/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/notes_lambda5_only.md"
...
# (qwen3_glossary_neg_train.py:3641). We keep 0 to match A1 (tys70s0y) behavior
...
TASK_TAG="train"
EXTRA_WANDB_TAGS="variant:hnps_k512_lambda5_smallest_dense_normAGGR_6gpu compute:aries-6gpu"
...
VERSION="3var_clean_gc_wr$((WIKI_RANK / 1000))k_m${MARGIN}_maxsim_mfa_variantE_hardneg_ps_k${HARD_NEG_K_PER_SAMPLE}_tcm_lambda5_wu${TCM_WARMUP_STEPS}_ep${EPOCHS}_cold_smallest_dense_normAGGR_6gpu${SMOKE_TAG}"
SAVE_NAME="q3rag_${MODE_NAME}_bs${BS_ABBR}_t=${TEMPERATURE}_${VERSION}"
SAVE_PATH="${SAVE_DIR}/${SAVE_NAME}.pt"
WANDB_EXP_NAME="variantE_hnps_k${HARD_NEG_K_PER_SAMPLE}_lambda5_smallest_dense_normAGGR_6gpu_${SAVE_NAME}"

echo "[TRAIN] VARIANT=E_hnps_k${HARD_NEG_K_PER_SAMPLE}_lambda5_smallest_dense_normAGGR_6gpu"
...
echo "[TRAIN] TCM: lambda=${TCM_LOSS_WEIGHT} T_beta=${TCM_POS_THRESHOLD} T_alpha=${TCM_NEG_THRESHOLD} warmup_steps=${TCM_WARMUP_STEPS}"
...
echo "[TRAIN] Variant E HN per-sample k=${HARD_NEG_K_PER_SAMPLE} lambda=5 smallest+dense 6GPU completed at $(date)"
