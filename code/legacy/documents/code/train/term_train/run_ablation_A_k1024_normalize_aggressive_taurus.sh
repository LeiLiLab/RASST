#!/bin/bash
#SBATCH --job-name=q3_ablA_k1024_norm
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_ablA_k1024_norm_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_ablA_k1024_norm_%x.err

# Ablation A: keep per-sample HN K=1024 ON, turn ON --term_id_normalize aggressive.
# Answers: "Given we fixed the HN near-variant false-negative bug (see
# dev_journal.md 2026-04-22), does HN at K=1024 now beat its broken self?"
# Paired with run_ablation_B_k0_normalize_aggressive.sh (HN off) on the same
# normalization setting — that pair isolates whether HN is net-positive on
# this recipe AFTER the bug is resolved.
#
# Diff vs run_hardneg_per_sample_k1024_cold_aries.sh:
#   + --term_id_normalize aggressive
#   * job-name / output tag / wandb exp name / save name
# Nothing else changes.  Keeps HARD_NEG_K_PER_SAMPLE=1024, all loss + schedule
# hyperparameters identical to the baseline.
...
# Moved aries -> taurus (user request, 2026-04-22 14:28 UTC). 6 GPUs on
# taurus; per-GPU batch unchanged so dynamics per-rank are bit-identical to
# the aries baseline.  Global batch = 6 * 1536 = 9216 (vs 12288 on aries).
# Since A and B are both run on the same 6-GPU setup, the A-vs-B comparison
# is apples-to-apples; the aries 8-GPU baseline just isn't directly
...
VERSION="3var_clean_gc_wr$((WIKI_RANK / 1000))k_m${MARGIN}_maxsim_mfa_variantE_hardneg_per_sample_k${HARD_NEG_K_PER_SAMPLE}_tcm_ep${EPOCHS}_cold_normAGGR${SMOKE_TAG}"
SAVE_NAME="q3rag_${MODE_NAME}_bs${BS_ABBR}_t=${TEMPERATURE}_${VERSION}"
SAVE_PATH="${SAVE_DIR}/${SAVE_NAME}.pt"
WANDB_EXP_NAME="ablA_ps_k${HARD_NEG_K_PER_SAMPLE}_normAGGR_${SAVE_NAME}"

echo "[TRAIN] ABLATION A: per_sample_k=${HARD_NEG_K_PER_SAMPLE} + term_id_normalize=${TERM_ID_NORMALIZE}"
echo "[TRAIN] Paired with ABLATION B (same normalize, HN off) to isolate HN contribution."
...
echo "[TRAIN] Ablation A (k${HARD_NEG_K_PER_SAMPLE} + norm=${TERM_ID_NORMALIZE}) completed at $(date)"
