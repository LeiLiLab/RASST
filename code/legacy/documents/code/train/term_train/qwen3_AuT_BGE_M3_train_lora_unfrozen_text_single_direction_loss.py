...
def train(rank, world_size, args):
    ...
    # 0. Load enriched term labels (fixes false negatives in training)
    enriched_chunk_terms = {}
    if args.enriched_terms_pkl and os.path.isfile(args.enriched_terms_pkl):
        with open(args.enriched_terms_pkl, "rb") as _f:
            enriched_chunk_terms = pickle.load(_f)
        if is_main:
            logger.info(f"[ENRICHED] Loaded {len(enriched_chunk_terms)} chunk enrichments from {args.enriched_terms_pkl}")
    elif args.enriched_terms_pkl:
        assert False, f"enriched_terms_pkl not found: {args.enriched_terms_pkl}"
        ...
        for batch in pbar:
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                ...
                if args.term_weight > 0:
                    ...
                    # Merge enriched term labels to fix false negatives
                    if enriched_chunk_terms:
                        for cid in chunk_to_terms:
                            extra = enriched_chunk_terms.get(cid)
                            if extra:
                                chunk_to_terms[cid].update(extra)
                                chunk_to_terms[cid].discard(NULL_TOKEN)
                    ...
                    # -------- Deduplicate terms locally (fixes cross-chunk false negatives) --------
                    # Each unique term string is encoded exactly once. The pos_mask is
                    # built by matching term CONTENT (not chunk ownership), so that
                    # "process" from chunk A and "process" from chunk B share one column
                    # and both chunks correctly mark it as positive.
                    local_terms_dedup = sorted({
                        t for cid in local_chunk_ids
                        for t in chunk_to_terms[cid]
                        if t != NULL_TOKEN
                    })
                    local_term_to_col = {t: j for j, t in enumerate(local_terms_dedup)}

                    # DDP: all ranks must execute collectives in the same order.
                    if global_step % 50 == 0:
                        local_max_tpc = max(
                            (len(v - {NULL_TOKEN}) for v in chunk_to_terms.values()), default=0
                        )
                        global_max_tpc = local_max_tpc
                        if world_size > 1:
                            tpc_tensor = torch.tensor(local_max_tpc, device=device, dtype=torch.int32)
                            dist.all_reduce(tpc_tensor, op=dist.ReduceOp.MAX)
                            global_max_tpc = int(tpc_tensor.item())

                        if is_main:
                            logger.info(
                                f"A_local={A_local}, T_dedup={len(local_terms_dedup)}, "
                                f"unique_chunks={len(local_chunk_ids)}, "
                                f"max_terms_per_chunk_local={local_max_tpc}, "
                                f"max_terms_per_chunk_global={global_max_tpc}"
                            )

                    if not local_terms_dedup:
                        local_term_embs = torch.zeros((0, 1024), device=device, dtype=torch.bfloat16)
                    else:
                        local_term_embs = encode_text(local_terms_dedup)

                    # -------- 2) gather global terms + build content-aware pos_mask --------
                    if world_size > 1:
                        t_local = local_term_embs.shape[0]
                        t_locals = [None] * world_size
                        dist.all_gather_object(t_locals, t_local)
                        t_max_pad = max(t_locals)

                        if t_max_pad > t_local:
                            local_term_embs_padded = F.pad(local_term_embs, (0, 0, 0, t_max_pad - t_local))
                        else:
                            local_term_embs_padded = local_term_embs

                        global_term_embs = all_gather_with_grad(local_term_embs_padded)

                        # Gather term strings so every rank can match by content
                        all_rank_terms = [None] * world_size
                        dist.all_gather_object(all_rank_terms, local_terms_dedup)

                        col_mask = torch.zeros(world_size * t_max_pad, device=device, dtype=torch.bool)
                        pos_mask = torch.zeros((A_local, world_size * t_max_pad), device=device)

                        for r in range(world_size):
                            r_start = r * t_max_pad
                            r_terms = all_rank_terms[r]
                            col_mask[r_start : r_start + len(r_terms)] = True
                            r_term_to_j = {ts: j for j, ts in enumerate(r_terms)}
                            for i, cid in enumerate(local_chunk_ids):
                                for t in chunk_to_terms[cid]:
                                    if t != NULL_TOKEN and t in r_term_to_j:
                                        pos_mask[i, r_start + r_term_to_j[t]] = 1.0
                    else:
                        global_term_embs = local_term_embs
                        pos_mask = torch.zeros((A_local, len(local_terms_dedup)), device=device)
                        for i, cid in enumerate(local_chunk_ids):
                            for t in chunk_to_terms[cid]:
                                if t != NULL_TOKEN and t in local_term_to_col:
                                    pos_mask[i, local_term_to_col[t]] = 1.0
                        col_mask = torch.ones(len(local_terms_dedup), device=device, dtype=torch.bool)
                    ...
                    # -------- 4) Single-direction multi-positive InfoNCE (A2T only) --------
...
def main():
    ...
    parser.add_argument("--enriched_terms_pkl", type=str, default="",
                        help="Path to enriched_chunk_terms.pkl (fixes false negatives by adding all vocab terms found in chunk text)")
