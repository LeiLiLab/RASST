#!/bin/bash
# Eval-only readout for the fixed 1.92s context checkpoint from r0xi5xkt.
#
# Intended direct Aries usage:
#   cd /mnt/taurus/home/jiaxuanluo/InfiniSST
#   LOG=/mnt/gemini/data1/jiaxuanluo/logs/direct_ctx192_r0xi5xkt_eval_$(date -u +%Y%m%dT%H%M%S)
#   nohup env CUDA_VISIBLE_DEVICES=7 SLURM_JOB_PARTITION=aries SLURM_JOB_ID=direct_ctx192_r0xi5xkt_eval \
#     MASTER_PORT=30492 \
#     bash documents/code/train/term_train/launchers/2026/05/20260518__ctx192_r0xi5xkt_best_dev_acl_med_aries1_eval.sh \
#     > ${LOG}.out 2> ${LOG}.err &
#   echo $!
#   echo ${LOG}.out
#   echo ${LOG}.err
...
export VARIANT_TAG="ctx192_r0xi5xkt_eval"
export VERSION="3var_clean_ctx192_r0xi5xkt_best_dev_acl_med_${COMPUTE_TAG}"
export WANDB_EXP_NAME="ctx192_r0xi5xkt_best_dev_acl_med_${COMPUTE_TAG}_20260518"
export NOTES_FILE="${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260518__ctx192_r0xi5xkt_best_dev_acl_med_eval.md"
...
export RESUME="/mnt/gemini/home/jiaxuanluo/train_outputs/q3rag_scale_lora-r128-tr128_bs12k_t=0.07_3var_clean_gc_wr1000k_m0.0_maxsim_mfa_variantE_hardneg_per_sample_k1024_tcm_ep5_cold_best.pt"

export EVAL_ONLY=true
export TASK_TAG="eval"
export DATA_TAG="3variant_clean_ctx192_r0xi5xkt_best_readout"
...
export BASELINE_RUN_IDS="r0xi5xkt lh1b88kw dxwrgbln"
export SELECT_CLEAN_GPUS=true

# Match the legacy fixed-1.92s checkpoint recipe from W&B run r0xi5xkt.
...
export MAXSIM_WINDOWS="6 10 16 24"
export MAXSIM_STRIDE=2
export MFA_WINDOW_SELECTION="hard_max"
export MFA_POSITIVE_SCOPE="auto"
export TERM_ID_NORMALIZE="aggressive"

# Eval-only: disable train-time negative-bank initialization.
export HARD_NEG_K=0
...
export TCM_NEG_THRESHOLD=0.25
export TCM_LOSS_FORM="squared_hinge"
...
export MASTER_PORT="${MASTER_PORT:-30492}"
...
export RUN_VERDICT="${RUN_VERDICT:-Eval-only fixed 1.92s context readout for r0xi5xkt best checkpoint across dev, ACL6060, and medicine base/gs1k/gs10k.}"
...
echo "[CTX192_EVAL] ckpt=${RESUME}"
echo "[CTX192_EVAL] dev=${DEV_JSONL} glossary=${EVAL_WIKI_GLOSSARY} sizes=base ${EVAL_GLOSSARY_SIZES}"
echo "[CTX192_EVAL] acl=${ACL_DEV_JSONL} glossary=${ACL_EVAL_WIKI_GLOSSARY} sizes=base ${ACL_EVAL_GLOSSARY_SIZES}"
echo "[CTX192_EVAL] medicine=${MEDICINE_DEV_JSONL} glossary=${MEDICINE_EVAL_WIKI_GLOSSARY} sizes=base ${MEDICINE_EVAL_GLOSSARY_SIZES}"
echo "[CTX192_EVAL] compute=${COMPUTE_TAG} cuda=${CUDA_VISIBLE_DEVICES:-<unset>}"
