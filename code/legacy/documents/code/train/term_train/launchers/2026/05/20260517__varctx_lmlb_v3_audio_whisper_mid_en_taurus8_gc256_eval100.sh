#!/bin/bash
#SBATCH --job-name=q3_vctx_wmid_g256
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_wmid_g256_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_wmid_g256_%x.err
...
# Speech-encoder ablation: keep the text side fixed at BGE-M3 and replace the
# default Qwen3-Omni audio encoder with Whisper medium.en.
export TEXT_ENCODER_PRESET="${TEXT_ENCODER_PRESET:-bge-m3}"
export TEXT_MODEL_ID="${TEXT_MODEL_ID:-BAAI/bge-m3}"
export TEXT_INPUT_PREFIX="${TEXT_INPUT_PREFIX-}"
export TEXT_POOLING="${TEXT_POOLING:-cls}"

export AUDIO_ENCODER_PRESET="${AUDIO_ENCODER_PRESET:-whisper-mid-en}"
export AUDIO_ENCODER_TYPE="${AUDIO_ENCODER_TYPE:-whisper}"
export AUDIO_MODEL_ID="${AUDIO_MODEL_ID:-openai/whisper-medium.en}"
export AUDIO_FEATURE_EXTRACTOR_ID="${AUDIO_FEATURE_EXTRACTOR_ID:-openai/whisper-medium.en}"
export AUDIO_INPUT_DTYPE="${AUDIO_INPUT_DTYPE:-auto}"
export AUDIO_HIDDEN_DIM="${AUDIO_HIDDEN_DIM:-0}"
export TARGET_MODULES="${TARGET_MODULES:-q_proj k_proj v_proj out_proj fc1 fc2}"
...
# Keep inline eval aligned with the hard-negative refresh cadence:
# NEG_BANK_REFRESH_STEPS=50 in the shared launcher, eval every 100 steps.
...
export VARIANT_TAG="${VARIANT_TAG:-vctx576_aud_wmid_t8_d100_tau1_g256}"
export VERSION="${VERSION:-3var_gsv2full_gsdedup_varctx576_aud_wmid_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_wr1000k_m0.0_maxsim_mfa_variantE_hn1024_tcmoff_ep${EPOCHS}_v3_dev100Tau1_eval100_taurus8}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-variantE_varctx576_aud_wmid_taurus8_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_dev100Tau1_eval100}"
export NOTES_FILE="${NOTES_FILE:-${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260517__varctx_lmlb_v3_audio_whisper_mid_en_taurus8_gc256_eval100.md}"
export DATA_TAG="${DATA_TAG:-3variant_gsv2full_gsdedup_vctx576_wmid}"
export EXPERIMENT_FAMILY="${EXPERIMENT_FAMILY:-sst_ood_hardneg}"
export EXTRA_WANDB_TAGS="${EXTRA_WANDB_TAGS:-variant:vctx576_aud_wmid_t8_d100_tau1_g256 compute:taurus-8gpu}"
...
export RUN_VERDICT="${RUN_VERDICT:-Whisper-medium.en speech-encoder ablation with BGE-M3 text encoder, varctx576 data, Taurus 8GPU, GradCache chunk 256, eval100 aligned to refresh50, dev100 plus full ACL/medicine inline readouts.}"
