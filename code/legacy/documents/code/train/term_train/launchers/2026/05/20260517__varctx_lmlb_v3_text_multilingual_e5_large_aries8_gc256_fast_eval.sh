#!/bin/bash
#SBATCH --job-name=q3_vctx_e5l_g256
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_e5l_g256_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_e5l_g256_%x.err
...
# Text-encoder ablation sibling of the BGE-large gc256 launcher.  E5 retrieval
# mode needs the text prefix; use mean pooling, which is the standard E5 pooling
# recipe and avoids relying on CLS behavior from the BGE baseline.
...
export TEXT_INPUT_PREFIX="${TEXT_INPUT_PREFIX:-query: }"
export TEXT_POOLING="${TEXT_POOLING:-mean}"
...
# Keep the same fast inline eval policy as the BGE-large gc256 run.
...
export VARIANT_TAG="${VARIANT_TAG:-vctx576_txt_e5l_a8_d100_tau1_g256}"
export VERSION="${VERSION:-3var_gsv2full_gsdedup_varctx576_txt_e5l_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_wr1000k_m0.0_maxsim_mfa_variantE_hn1024_tcmoff_ep${EPOCHS}_v3_dev100Tau1_eval100_aries8}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-variantE_varctx576_txt_e5l_aries8_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_dev100Tau1_eval100}"
export NOTES_FILE="${NOTES_FILE:-${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260517__varctx_lmlb_v3_text_multilingual_e5_large_aries8_gc256_dev100_tau1_eval.md}"
export DATA_TAG="${DATA_TAG:-3variant_gsv2full_gsdedup_vctx576_e5l}"
export EXPERIMENT_FAMILY="${EXPERIMENT_FAMILY:-sst_ood_hardneg}"
export EXTRA_WANDB_TAGS="${EXTRA_WANDB_TAGS:-variant:vctx576_txt_e5l_a8_d100_tau1_g256 compute:aries-8gpu}"
export BASELINE_RUN_IDS="${BASELINE_RUN_IDS:-lh1b88kw mhukv2bi}"
export SELECT_CLEAN_GPUS="${SELECT_CLEAN_GPUS:-true}"
export MASTER_PORT="${MASTER_PORT:-30366}"
export RUN_VERDICT="${RUN_VERDICT:-Multilingual-E5-large text-encoder ablation with query prefix on varctx576, Aries 8GPU, GradCache chunk 256, mean text pooling, GPU-chunked eval scoring, dev100 base/gs1k/gs10k inline eval plus full ACL/medicine readouts, tau sweep 0.75 only.}"
