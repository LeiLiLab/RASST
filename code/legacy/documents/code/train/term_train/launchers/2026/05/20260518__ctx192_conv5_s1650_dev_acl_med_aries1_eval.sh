#!/bin/bash
# Eval-only readout for the fixed 1.92s smallest-dense MFA conv5 checkpoint.
#
# Intended direct Aries usage:
#   cd /mnt/taurus/home/jiaxuanluo/InfiniSST
#   LOG=/mnt/gemini/data1/jiaxuanluo/logs/direct_ctx192_conv5_s1650_eval_$(date -u +%Y%m%dT%H%M%S)
#   nohup env CUDA_VISIBLE_DEVICES=7 SLURM_JOB_PARTITION=aries SLURM_JOB_ID=direct_ctx192_conv5_s1650_eval \
#     MASTER_PORT=30493 \
#     bash documents/code/train/term_train/launchers/2026/05/20260518__ctx192_conv5_s1650_dev_acl_med_aries1_eval.sh \
#     > ${LOG}.out 2> ${LOG}.err &
#   echo $!
#   echo ${LOG}.out
#   echo ${LOG}.err
...
export VARIANT_TAG="ctx192_conv5_s1650_eval"
export VERSION="3var_gsdedup_ctx192_conv5_s1650_dev_acl_med_${COMPUTE_TAG}"
export WANDB_EXP_NAME="ctx192_conv5_s1650_dev_acl_med_${COMPUTE_TAG}_20260518"
export NOTES_FILE="${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260518__ctx192_conv5_s1650_dev_acl_med_eval.md"

export TRAIN_JSONL="/mnt/gemini/home/jiaxuanluo/term_train_3variant_gsv2_full0_31_clean_mfa_gsrepaired_gsdedup.jsonl"
export DEV_JSONL="/mnt/gemini/data1/jiaxuanluo/term_dev_with_wiki_synth_normalized.jsonl"
...
export MEDICINE_DEV_JSONL="/mnt/gemini/home/jiaxuanluo/medicine_eval_ctx1p92/medicine_dev_dataset.jsonl"

export RESUME="/mnt/gemini/home/jiaxuanluo/train_outputs/q3rag_scale_lora-r128-tr128_bs12k_t=0.07_3var_gsv2full_gsfix_mfa_gsdedup_gc_wr1000k_m0.0_maxsim_mfa_variantE_hn1024_tcmoff_conv5_bs12k_smallest_dense_normAGGR_8gpu_aries_best_eval_dev_topk10_filtered_recallattau_0p80_gs10000.pt"

export EVAL_ONLY=true
export TASK_TAG="eval"
export DATA_TAG="3variant_gsdedup_ctx192_conv5_s1650_readout"
export EXPERIMENT_FAMILY="sst_ood_hardneg"
export EXTRA_WANDB_TAGS="variant:${VARIANT_TAG} compute:${COMPUTE_TAG} ablation:ctx192 readout:dev_acl_med"
export BASELINE_RUN_IDS="7xu2b4so lh1b88kw dxwrgbln"
export SELECT_CLEAN_GPUS=true

# Match the smallest-dense MFA conv5 checkpoint recipe from W&B run 7xu2b4so.
export FIXED_AUDIO_SECONDS=1.92
export EVAL_FIXED_AUDIO_SECONDS=1.92
...
# Eval-only: disable train-time negative-bank initialization and losses.
export HARD_NEG_K=0
...
export MASTER_PORT="${MASTER_PORT:-30493}"

# Recall readout shape: base / 1k / 10k for each domain.  For dev, the fixed
# 1.92s GT bank is already >1k, so the script may skip gs1000 as non-expanding.
...
export MEDICINE_EVAL_WIKI_GLOSSARY="/mnt/gemini/home/jiaxuanluo/medicine_eval_ctx1p92/medicine_glossary_gt_plus_medicine_wiki_gs10000.json"
...
export RUN_VERDICT="${RUN_VERDICT:-Eval-only fixed 1.92s smallest-dense MFA conv5 readout for 7xu2b4so secondary checkpoint across dev, ACL6060, and medicine base/gs1k/gs10k.}"
...
echo "[CTX192_CONV5_EVAL] ckpt=${RESUME}"
echo "[CTX192_CONV5_EVAL] dev=${DEV_JSONL} glossary=${EVAL_WIKI_GLOSSARY} sizes=base ${EVAL_GLOSSARY_SIZES}"
echo "[CTX192_CONV5_EVAL] acl=${ACL_DEV_JSONL} glossary=${ACL_EVAL_WIKI_GLOSSARY} sizes=base ${ACL_EVAL_GLOSSARY_SIZES}"
echo "[CTX192_CONV5_EVAL] medicine=${MEDICINE_DEV_JSONL} glossary=${MEDICINE_EVAL_WIKI_GLOSSARY} sizes=base ${MEDICINE_EVAL_GLOSSARY_SIZES}"
echo "[CTX192_CONV5_EVAL] compute=${COMPUTE_TAG} cuda=${CUDA_VISIBLE_DEVICES:-<unset>}"
