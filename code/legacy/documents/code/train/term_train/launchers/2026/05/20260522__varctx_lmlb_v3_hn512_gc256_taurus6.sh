...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_hn512_t6_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_hn512_t6_%x.err
...
export LOCAL_TMP_DIR="${LOCAL_TMP_DIR:-/dev/shm/q3_hn512_t6_${USER}_${RUN_STAMP}}"
...
export VARIANT_TAG="${VARIANT_TAG:-hn512_varctx576_v3_gc${GRAD_CACHE_CHUNK_SIZE}_gpu${CUDA_DEVICE_TAG}_tcmoff_ep6}"
export VERSION="${VERSION:-3var_gsv2full_gsdedup_varctx576_bs${BATCH_SIZE}_gc${GRAD_CACHE_CHUNK_SIZE}_wr1000k_m0.0_maxsim_mfa_variantE_hn512_tcmoff_ep${EPOCHS}_v3_smallest_dense_normAGGR_gpu${CUDA_DEVICE_TAG}_taurus}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-variantE_hn512_gsv2full_gsdedup_varctx576_v3_bs${BATCH_SIZE}_gc${GRAD_CACHE_CHUNK_SIZE}_tcmoff_ep${EPOCHS}_gpu${CUDA_DEVICE_TAG}_taurus}"
export NOTES_FILE="${NOTES_FILE:-${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260522__varctx_lmlb_v3_hn512_gc256_taurus6.md}"

export EXTRA_WANDB_TAGS="${EXTRA_WANDB_TAGS:-variant:hn512_varctx576_v3 compute:taurus-6gpu ablation:hard_neg512 source:lh1b88kw}"
export BASELINE_RUN_IDS="${BASELINE_RUN_IDS:-lh1b88kw e981df6j 40fgbr2y bgz7akb6 ah9u1bao dxwrgbln}"
export RUN_VERDICT="${RUN_VERDICT:-HN512 ablation from lh1b88kw recipe: hard_neg_k_per_sample=512, grad_cache_chunk_size=${GRAD_CACHE_CHUNK_SIZE}, target global batch 8192 with effective equal-rank batch ${BATCH_SIZE}; primary best metric is eval_dev/recall@10_gs10000 and secondary best metric is eval_acl6060/recall@10.}"
