#!/bin/bash
#SBATCH --job-name=q3_vctx_bgel_t8
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_bgel_t8_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_bgel_t8_%x.err
...
# Text-encoder ablation: keep Qwen3-Omni audio and varctx576 data fixed, swap
# BGE-M3 text encoder for BGE-large-en-v1.5.  Taurus uses all 8 GPUs so the
# global batch matches the Aries control while keeping per-GPU batch at 1024.
...
# To avoid another NCCL wait timeout, keep the expensive dev pass as a fixed
# 100-sample smoke readout and reduce threshold diagnostics to tau=0.75 only.
export EVAL_STEPS_SAMPLE="${EVAL_STEPS_SAMPLE:-240}"
...
export VARIANT_TAG="${VARIANT_TAG:-vctx576_txt_bgel_t8_d100_tau1}"
export VERSION="${VERSION:-3var_gsv2full_gsdedup_varctx576_txt_bgel_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_wr1000k_m0.0_maxsim_mfa_variantE_hn1024_tcmoff_ep${EPOCHS}_v3_dev100Tau1_eval240_taurus8}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-variantE_varctx576_txt_bgel_taurus8_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_dev100Tau1_eval240}"
export NOTES_FILE="${NOTES_FILE:-${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260517__varctx_lmlb_v3_text_bge_large_en_taurus8_dev100_tau1_eval.md}"
...
export EXTRA_WANDB_TAGS="${EXTRA_WANDB_TAGS:-variant:vctx576_txt_bgel_t8_d100_tau1 compute:taurus-8gpu}"
export BASELINE_RUN_IDS="${BASELINE_RUN_IDS:-lh1b88kw}"
export SELECT_CLEAN_GPUS="${SELECT_CLEAN_GPUS:-true}"
export MASTER_PORT="${MASTER_PORT:-30031}"
export RUN_VERDICT="${RUN_VERDICT:-BGE-large-en-v1.5 text-encoder ablation on varctx576, Taurus 8GPU, GPU-chunked eval scoring, dev100 base/gs1k/gs10k inline eval plus full ACL/medicine readouts, tau sweep 0.75 only.}"
