#!/bin/bash
#SBATCH --job-name=q3_vctx_wavl_fu_b8g128
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_wavl_fu_b8g128_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_wavl_fu_b8g128_%x.err
...
export VARIANT_TAG="${VARIANT_TAG:-vctx576_aud_wavl_fu_t8_b8k_g128}"
export VERSION="${VERSION:-3var_gsv2full_gsdedup_varctx576_aud_wavl_fu_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_wr1000k_m0.0_maxsim_mfa_variantE_hn1024_tcmoff_ep${EPOCHS}_v3_dev100Tau1_eval200_taurus8}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-variantE_varctx576_aud_wavl_fu_taurus8_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_dev100Tau1_eval200_full}"
export NOTES_FILE="${NOTES_FILE:-${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260518__varctx_lmlb_v3_audio_wavlm_large_taurus8_bs8k_gc128_eval200_layerdrop0_findunused_full.md}"
...
export EXTRA_WANDB_TAGS="${EXTRA_WANDB_TAGS:-variant:vctx576_aud_wavl_fu_t8_b8k_g128 compute:taurus-8gpu}"
export BASELINE_RUN_IDS="${BASELINE_RUN_IDS:-lh1b88kw r1pxeaxj 9034wae5 hikwfmaa 34sbpz92 r8q9uf8k}"
export MASTER_PORT="${MASTER_PORT:-30486}"
export SELECT_CLEAN_GPUS="${SELECT_CLEAN_GPUS:-true}"
export RUN_VERDICT="${RUN_VERDICT:-WavLM-large speech-encoder ablation with BGE-M3 text encoder, WavLM checkpointing shim, WavLM layerdrop disabled, WavLM retriever DDP find_unused_parameters enabled for GradCache stability, dynamic MFA frame-time mapping, global batch 8192, GradCache chunk 128, and eval200 aligned to refresh50.}"
