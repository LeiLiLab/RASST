...
export RUN_STAMP="${RUN_STAMP:-hn256_best_secondary_fixeddenom_dev100k_fastchunks_taurus_gpu45_20260523T0358Z}"
export MODEL_TAG="hn256_best_secondary_s1200_fixeddenom"
export VARIANT_TAG="hn256_best_secondary_s1200_fixeddenom_dev100k_fastchunks"
...
export EXTRA_WANDB_TAGS="variant:hn256_best_secondary_s1200_fixeddenom compute:taurus-2gpu comparison:nohn_hn256_hn1024 calibration:dev_delta readout:dev100k_acl_tagged_medstrict protocol:fixed-raw-denominator hn:256 perf:fastchunks"
export BASELINE_RUN_IDS="lrdx14pm e981df6j lh1b88kw 40fgbr2y zji769ve ry8osg4u e8t8zdtd 31xmxmdp 9esujv2w fwdr8y3c"

export CUDA_DEVICE_LIST="${CUDA_DEVICE_LIST:-4,5}"
export NUM_GPUS=2
export PER_GPU_BATCH=1
export BATCH_SIZE=2
export MASTER_PORT="${MASTER_PORT:-30458}"
export SELECT_CLEAN_GPUS=false
export WAIT_FOR_CLEAN_GPUS=false
export LOCAL_TMP_DIR="${LOCAL_TMP_DIR:-/tmp/jiaxuanluo_q3_hn256_eval_dev100k_fastchunks_${RUN_STAMP}}"
...
export NOTES_FILE="${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260523__hn256_best_secondary_fixeddenom_dev100k_heldout_taurus2_fastchunks_eval.md"
...
export RUN_VERDICT="Eval-only HN256 fixed-denominator comparison. Dev tau selection uses raw/base plus gs10k and gs100k only; ACL6060, tagged ACL6060, and strict medicine are readout-only. This run uses larger GPU score chunks."
