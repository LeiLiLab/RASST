#!/bin/bash
#SBATCH --job-name=q3_vctx_wavlm_b8g256
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_wavlm_b8g256_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_vctx_wavlm_b8g256_%x.err
...
# Full WavLM-large-plus audio-encoder ablation.  WavLM emits roughly 50 fps
# features, so these frame windows/stride are 4x the Qwen/Whisper-12.5fps frame
# counts while preserving the same time spans and reducing MaxSim window count.
...
# Speed-focused inline eval: still aligned with refresh50, but less frequent
# than the Whisper run.  Dev stays sampled; ACL/medicine remain full readouts.
...
export VARIANT_TAG="${VARIANT_TAG:-vctx576_aud_wavlmlp_t8_bs8k_g256}"
export VERSION="${VERSION:-3var_gsv2full_gsdedup_varctx576_aud_wavlmlp_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_wr1000k_m0.0_maxsim_mfa_variantE_hn1024_tcmoff_ep${EPOCHS}_v3_dev100Tau1_eval200_taurus8}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-variantE_varctx576_aud_wavlmlp_taurus8_bs8k_gc${GRAD_CACHE_CHUNK_SIZE}_dev100Tau1_eval200_full}"
export NOTES_FILE="${NOTES_FILE:-${REPO_ROOT}/documents/code/train/term_train/notes/2026/05/20260518__varctx_lmlb_v3_audio_wavlm_large_plus_taurus8_bs8k_gc256_eval200_full.md}"
export DATA_TAG="${DATA_TAG:-3variant_gsv2full_gsdedup_vctx576_wavlmlp}"
export EXPERIMENT_FAMILY="${EXPERIMENT_FAMILY:-sst_ood_hardneg}"
export EXTRA_WANDB_TAGS="${EXTRA_WANDB_TAGS:-variant:vctx576_aud_wavlmlp_t8_bs8k_g256 compute:taurus-8gpu}"
export BASELINE_RUN_IDS="${BASELINE_RUN_IDS:-lh1b88kw r1pxeaxj}"
export MASTER_PORT="${MASTER_PORT:-30468}"
export SELECT_CLEAN_GPUS="${SELECT_CLEAN_GPUS:-true}"
export RUN_VERDICT="${RUN_VERDICT:-WavLM-large-plus speech-encoder ablation with BGE-M3 text encoder, dynamic MFA frame-time mapping, WavLM 50fps-equivalent MaxSim windows, global batch 8192, GradCache chunk 256, and eval200 aligned to refresh50.}"
