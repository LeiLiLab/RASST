...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tfpool_ph.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tfpool_ph.err

# Transformer Pooling + Phoneme Append: train from scratch.
# Audio: 1-layer cross-attention pooling with learnable query.
# Text:  term [SEP] PHONEMES: ARPAbet sequence (pre-computed in JSONL).
# Aries 8 GPUs, GradCache 12K, margin=0.1, no O-HNM.
...
export NLTK_DATA="/mnt/taurus/home/jiaxuanluo/nltk_data"
...
# Paths
TRAIN_JSONL="/mnt/gemini/data1/jiaxuanluo/term_train_3variant_1m_phoneme.jsonl"
...
# Text encoder: LoRA r128 + Phoneme Append
TEXT_FULL_FINETUNE="false"
TEXT_LR="0"
TEXT_LORA_RANK=128
TEXT_LORA_ALPHA=256
TEXT_TARGET_MODULES="query key value dense"
USE_PHONEME_APPEND="true"

# Training — GradCache: 8 GPUs × 1536 = 12288 effective batch
PER_GPU_BATCH=1536
...
# Eval & checkpointing (scaled for 12K batch)
SAVE_STEPS=100
...
VERSION="${VERSION}_${POOLING_TYPE}pool"
if [ "${USE_PHONEME_APPEND}" = "true" ]; then
    VERSION="${VERSION}_phoneme"
fi
...
echo "[TRAIN] Transformer Pooling + Phoneme Append (from scratch)"
...
echo "[TRAIN] Phoneme append: ${USE_PHONEME_APPEND}"
echo "[TRAIN] LR: ${LR}"
echo "[TRAIN] Best metric: ${BEST_METRIC}"
echo "[TRAIN] Best metric (secondary): ${BEST_METRIC_SECONDARY}"
