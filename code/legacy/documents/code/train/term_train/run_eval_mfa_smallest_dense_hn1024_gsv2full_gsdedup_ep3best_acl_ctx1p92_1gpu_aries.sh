#!/bin/bash
#SBATCH --job-name=q3_ep3_acl_ctx192
...
#SBATCH --gres=gpu:1
#SBATCH --time=4:00:00
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_ep3_acl_ctx192_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_ep3_acl_ctx192_%x.err
...
export VARIANT_TAG="acl_ctx192_gsdedup_ep3best"
export VERSION="3var_gsv2full_gsfix_mfa_gsdedup_ep3best_acl_ctx192_${COMPUTE_TAG}_smallest_dense"
export WANDB_EXP_NAME="gsdedup_ep3best_acl_ctx192_${COMPUTE_TAG}"
export NOTES_FILE="/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/notes_hn1024_gsv2full_gsdedup_ep3best_acl_ctx1p92_eval_aries.md"
...
export DATA_TAG="acl6060_ctx192_ep3best_eval"
...
export RUN_VERDICT="One-shot ACL6060 1.92s readout for the ah9u1bao ep3 best checkpoint; control for the 3.84s ACL readout."
...
echo "[EP3BEST_ACL_CTX192] ckpt=${RESUME}"
echo "[EP3BEST_ACL_CTX192] dev=<disabled>"
echo "[EP3BEST_ACL_CTX192] acl=${ACL_DEV_JSONL}"
echo "[EP3BEST_ACL_CTX192] glossary=${EVAL_WIKI_GLOSSARY} sizes=${EVAL_GLOSSARY_SIZES}"
echo "[EP3BEST_ACL_CTX192] fixed_audio_seconds=${FIXED_AUDIO_SECONDS} eval_fixed_audio_seconds=${EVAL_FIXED_AUDIO_SECONDS}"
echo "[EP3BEST_ACL_CTX192] beta=${TCM_SWEEP_FBETA} taus=${TCM_SWEEP_THRESHOLDS}"
echo "[EP3BEST_ACL_CTX192] compute=${COMPUTE_TAG} batch=$((NUM_GPUS * PER_GPU_BATCH))"
