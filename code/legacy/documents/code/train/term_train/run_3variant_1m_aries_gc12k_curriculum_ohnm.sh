#!/bin/bash
#SBATCH --job-name=q3_tfpool_ohnm
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tfpool_ohnm.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tfpool_ohnm.err

# Transformer Pooling + Curriculum soft O-HNM, train from scratch.
# easy_neg_weight decays from 1.0 → 0.05 over 500 steps (~1 epoch).
...
# Training — GradCache 12K, from scratch
PER_GPU_BATCH=1536
...
# Curriculum soft O-HNM: easy_neg decays 1.0 → 0.05 over 500 steps
ONLINE_HARD_NEG_K=256
EASY_NEG_WEIGHT=1.0
EASY_NEG_WEIGHT_FINAL=0.05
HN_DECAY_STEPS=500
...
if [ "${ONLINE_HARD_NEG_K}" -gt 0 ]; then
    VERSION="${VERSION}_softohnm${ONLINE_HARD_NEG_K}"
fi
...
echo "[TRAIN] Transformer Pooling + Curriculum soft O-HNM (from scratch)"
echo "[TRAIN] Pooling: ${POOLING_TYPE}"
echo "[TRAIN] easy_neg_weight: ${EASY_NEG_WEIGHT} -> ${EASY_NEG_WEIGHT_FINAL} over ${HN_DECAY_STEPS} steps"
...
torchrun \
    ...
    --easy_neg_weight_final "${EASY_NEG_WEIGHT_FINAL}" \
    --hn_decay_steps "${HN_DECAY_STEPS}" \
