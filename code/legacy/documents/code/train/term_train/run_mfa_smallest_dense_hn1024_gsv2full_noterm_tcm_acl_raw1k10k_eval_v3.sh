#!/bin/bash
#SBATCH --job-name=q3_ntcm_acl_eval
...
#SBATCH --gres=gpu:1
#SBATCH --time=4:00:00
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_ntcm_acl_eval_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_ntcm_acl_eval_%x.err
...
export VARIANT_TAG="acl_raw1k10k_final_n64_p1n4"
export VERSION="3var_gsv2full_gsfix_mfa_ntcm_acl_raw1k10k_v3_final_n64_p1n4_${COMPUTE_TAG}_smallest_dense"
export WANDB_EXP_NAME="ntcm_acl_raw1k10k_v3_final_n64_p1n4_${COMPUTE_TAG}"
export NOTES_FILE="/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/notes_hn1024_gsv2full_noterm_tcm_acl_raw1k10k_eval_v3.md"
...
export RESUME="/mnt/gemini/home/jiaxuanluo/train_outputs/q3rag_scale_lora-r128-tr128_bs12k_t=0.07_3var_gsv2full_gsfix_mfa_ntcm_final_v3_n64_p1n4_s2000_aries-8gpu_smallest_dense_smoke4650_best.pt"
...
export MASTER_PORT="${MASTER_PORT:-30340}"

export DATA_TAG="3variant_gsv2full_gsfix_mfa_acl_raw1k10k_eval"
export EXPERIMENT_FAMILY="sst_tcm_noterm"
export EXTRA_WANDB_TAGS="variant:${VARIANT_TAG} compute:${COMPUTE_TAG}"
export BASELINE_RUN_IDS="us4obwe3 tau6iuo3"
...
export RUN_VERDICT="Eval-only ACL raw/gs1k/gs10k tau-filtered recall and dev-v3 no-term noise for final_n64_p1n4 at tau=0.75."

echo "[NTCM_ACL_EVAL] ckpt=${RESUME}"
echo "[NTCM_ACL_EVAL] acl=${ACL_DEV_JSONL}"
echo "[NTCM_ACL_EVAL] glossary=${EVAL_WIKI_GLOSSARY} sizes=${EVAL_GLOSSARY_SIZES} tau=${TCM_SWEEP_THRESHOLDS}"
echo "[NTCM_ACL_EVAL] compute=${COMPUTE_TAG} batch=$((NUM_GPUS * PER_GPU_BATCH))"
