...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tf_sp.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tf_sp.err

# Transformer Pooling + Hybrid Dense-Sparse text embedding (full training).
# Text emb = (1-α)*CLS_dense + α*sparse_weighted_pool, with BGE-M3 pretrained sparse_linear.
# Reads best sparse_weight from sweep result file if SPARSE_WEIGHT is "auto".
# Taurus 6 GPUs, GradCache, margin=0.1, no phoneme, no O-HNM.
...
# Audio model (LoRA) + Transformer Pooling
USE_LORA="true"
LORA_RANK=128
LORA_ALPHA=256
TARGET_DIM=1024
...
# Text encoder: LoRA r128 + Hybrid sparse
...
SPARSE_WEIGHT="auto"
SWEEP_RESULT_FILE="/mnt/gemini/data1/jiaxuanluo/sweep_sparse_result.txt"

# Training — GradCache: 6 GPUs × 1536 = 9216 effective batch
PER_GPU_BATCH=1536
...
# ---- Resolve auto sparse_weight from sweep results ----
if [ "${SPARSE_WEIGHT}" = "auto" ]; then
    if [ ! -f "${SWEEP_RESULT_FILE}" ]; then
        echo "[ERROR] SPARSE_WEIGHT=auto but sweep result file not found: ${SWEEP_RESULT_FILE}"
        exit 1
    fi
    BEST_LINE=$(grep "^BEST_SPARSE_WEIGHT=" "${SWEEP_RESULT_FILE}")
    if [ -z "${BEST_LINE}" ]; then
        echo "[ERROR] Could not find BEST_SPARSE_WEIGHT in ${SWEEP_RESULT_FILE}"
        cat "${SWEEP_RESULT_FILE}"
        exit 1
    fi
    SPARSE_WEIGHT="${BEST_LINE#BEST_SPARSE_WEIGHT=}"
    echo "[AUTO] Resolved sparse_weight=${SPARSE_WEIGHT} from ${SWEEP_RESULT_FILE}"
    echo "[AUTO] Full sweep results:"
    cat "${SWEEP_RESULT_FILE}"
fi
...
VERSION="3var_clean_gc_wr$((WIKI_RANK / 1000))k"
if [ "$(echo "${MARGIN} > 0" | bc -l)" -eq 1 ]; then
    VERSION="${VERSION}_m${MARGIN}"
fi
VERSION="${VERSION}_${POOLING_TYPE}pool_sp${SPARSE_WEIGHT}"
...
echo "[TRAIN] Transformer Pooling + Hybrid Dense-Sparse (from scratch)"
echo "[TRAIN] Data: ${TRAIN_JSONL}"
echo "[TRAIN] Save: ${SAVE_PATH}"
echo "[TRAIN] Batch: ${BATCH_SIZE} (${NUM_GPUS} GPUs * ${PER_GPU_BATCH})"
echo "[TRAIN] Pooling: ${POOLING_TYPE}"
echo "[TRAIN] Sparse weight: ${SPARSE_WEIGHT}"
echo "[TRAIN] LR: ${LR}"
