#!/usr/bin/env python3
"""
XEUS + BGE-M3 TTS Term Training Script

Replaces Qwen3-Omni AuT with XEUS (E-Branchformer, 577M) as the audio encoder.
Both speech chunks and TTS term bank audio are encoded by the same XEUS model.
XEUS takes raw waveforms directly — no WhisperFeatureExtractor needed.

Triplet:
- speech_utt: source chunk audio (raw waveform -> XEUS)
- term_text: term_key text (-> BGE-M3)
- term_tts: TTS audio of the same term (raw waveform -> XEUS)
...
DEFAULT_XEUS_HIDDEN_DIM = 1024

DEFAULT_XEUS_LORA_TARGET_MODULES = [
...
class XeusRetriever(nn.Module):
    """
    Audio retriever using XEUS (E-Branchformer, 577M).
    Accepts raw 16 kHz waveforms, encodes with XEUS, then attentive-pools
    and projects to *target_dim*.
    """
    ...
    def __init__(
        ...
        super().__init__()
        assert os.path.isfile(checkpoint_path), f"XEUS checkpoint not found: {checkpoint_path}"
        try:
        ...
        xeus_model, _ = SSLTask.build_model_from_file(None, checkpoint_path, "cpu")
        ...
        if use_flash_attn:
            try:
                ...
                logger.info("[XEUS] Flash Attention enabled")
            except AttributeError:
                logger.warning("[XEUS] Could not enable Flash Attention — continuing without it")

        if use_lora:
            if lora_target_modules is None:
                lora_target_modules = list(DEFAULT_XEUS_LORA_TARGET_MODULES)
            lora_config = LoraConfig(
                r=lora_rank,
                lora_alpha=lora_alpha,
            ...
            xeus_model.print_trainable_parameters()
        else:
            for p in xeus_model.parameters():
                p.requires_grad = False

        self.xeus = xeus_model
    ...
    def _xeus_encode_last_layer(self, wavs: torch.Tensor, wav_lengths: torch.Tensor) -> torch.Tensor:
        """Return last-layer hidden states from XEUS: [B, T_out, D]."""
        return self.xeus.encode(wavs, wav_lengths, use_mask=False, use_final_output=False)[0][-1]

    # ------ forward ------

    def forward(self, wavs: torch.Tensor, wav_lengths: torch.Tensor) -> torch.Tensor:
        if self.gradient_checkpointing and self.training:
            feats = grad_checkpoint(
                self._xeus_encode_last_layer, wavs, wav_lengths, use_reentrant=False,
            )
        else:
            feats = self._xeus_encode_last_layer(wavs, wav_lengths)

        batch_size, T_out, _ = feats.shape
        T_in = wavs.shape[1]
        assert T_in > 0, "Input waveform length must be > 0"
        ratio = T_out / T_in
        output_lens = (wav_lengths.float() * ratio).long().clamp(min=1, max=T_out)
        mask = torch.arange(T_out, device=feats.device).unsqueeze(0).expand(batch_size, -1) < output_lens.unsqueeze(1)
...
# ---------------------------------------------------------------------------
# Dataset & collation (raw waveforms — no WhisperFeatureExtractor)
# ---------------------------------------------------------------------------
...
def collate_fn(batch: List[Dict]) -> Dict:
    """Collate batch into raw-waveform tensors for XEUS."""
    ...
    return {
        "speech_wavs": torch.from_numpy(np.stack(speech_list)).float(),
        "speech_wav_lens": torch.tensor(speech_lens, dtype=torch.long),
        "tts_wavs": torch.from_numpy(np.stack(tts_list)).float(),
        "tts_wav_lens": torch.tensor(tts_lens, dtype=torch.long),
...
def train(rank: int, world_size: int, args: argparse.Namespace) -> None:
    ...
    # --- Audio retriever (XEUS + LoRA) ---
    retriever = XeusRetriever(
        checkpoint_path=args.xeus_checkpoint_path,
        target_dim=args.target_dim,
        hidden_dim=args.xeus_hidden_dim,
        ...
        use_flash_attn=args.xeus_use_flash_attn,
        gradient_checkpointing=args.gradient_checkpointing,
    ).to(device)

    # --- Text encoder (BGE-M3 + LoRA) ---
    ...
    audio_lora_params = [p for p in raw_retriever.xeus.parameters() if p.requires_grad]
    ...
    def run_sample_eval(current_step: int, current_epoch: int) -> None:
        ...
        with torch.no_grad():
            for eval_batch in eval_loader:
                speech_wavs = eval_batch["speech_wavs"].to(device)
                speech_wav_lens = eval_batch["speech_wav_lens"].to(device)
                tts_wavs = eval_batch["tts_wavs"].to(device)
                tts_wav_lens = eval_batch["tts_wav_lens"].to(device)
                ...
                with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                    speech_embs = raw_retriever(speech_wavs, speech_wav_lens)
                    term_tts_embs = raw_retriever(tts_wavs, tts_wav_lens)
        ...
        if valid_tts_count > 0:
            ...
            if tts_term_bank_count > 0:
                ...
                proto_scores = speech_valid @ proto_bank.t()
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for batch in iterator:
            ...
            speech_wavs = batch["speech_wavs"].to(device)
            speech_wav_lens = batch["speech_wav_lens"].to(device)
            tts_wavs = batch["tts_wavs"].to(device)
            tts_wav_lens = batch["tts_wav_lens"].to(device)
            ...
            with torch.amp.autocast("cuda", dtype=torch.bfloat16):
                lambda_weight = args.tts_loss_weight
                speech_embs = retriever(speech_wavs, speech_wav_lens)
                if lambda_weight > 0.0:
                    term_tts_embs = retriever(tts_wavs, tts_wav_lens)
                else:
                    term_tts_embs = None
                text_inputs = text_tokenizer(
                    term_texts,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--save_path", type=str, default="xeus_retriever_tts.pt")
    parser.add_argument("--resume", type=str, default="")
    parser.add_argument("--text_model_id", type=str, default=DEFAULT_TEXT_MODEL_ID)

    parser.add_argument("--xeus_checkpoint_path", type=str, required=True,
                        help="Path to XEUS checkpoint.pth")
    parser.add_argument("--xeus_hidden_dim", type=int, default=DEFAULT_XEUS_HIDDEN_DIM,
                        help="Hidden dimension of XEUS E-Branchformer output (default 1024)")
    parser.add_argument("--xeus_use_flash_attn", action="store_true", default=False,
                        help="Enable Flash Attention in XEUS (requires flash-attn package)")
    parser.add_argument("--gradient_checkpointing", action="store_true", default=False,
                        help="Enable gradient checkpointing for XEUS to save GPU memory")
    ...
    parser.add_argument("--lora_target_modules", type=str, nargs="+", default=None,
                        help="XEUS LoRA target modules (default: linear_q linear_k linear_v linear_out w_1 w_2 merge_proj)")
    ...
    parser.add_argument("--wandb_exp_name", type=str, default="xeus_tts_term_train")
