...
#SBATCH --time=8:00:00
#SBATCH --array=0-2%1
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%A_%a_q3_ntcm_wt_v3_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%A_%a_q3_ntcm_wt_v3_%x.err
...
CONFIG_PATH="${CONFIG_PATH:-/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/noterm_tcm_weight_scout_grid_v3.tsv}"
...
RUN_LABEL="${RUN_LABEL:-ntcm_wt_v3}"
VARIANT_PREFIX="${VARIANT_PREFIX:-wt_}"

export VARIANT_TAG="${VARIANT_PREFIX}${GRID_VARIANT}"
export VERSION="3var_gsv2full_gsfix_mfa_${RUN_LABEL}_${GRID_VARIANT}_s${SCOUT_STEPS}_${COMPUTE_TAG}_smallest_dense"
export WANDB_EXP_NAME="${RUN_LABEL}_${GRID_VARIANT}_s${SCOUT_STEPS}_${COMPUTE_TAG}"
export NOTES_FILE="${NOTES_FILE:-/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/notes_hn1024_gsv2full_noterm_tcm_weight_scouts_v3.md}"
...
export MASTER_PORT=$((30120 + TASK_ID))

export DATA_TAG="${DATA_TAG:-3variant_gsv2full_gsfix_mfa_devv3}"
export EXPERIMENT_FAMILY="sst_tcm_noterm"
export EXTRA_WANDB_TAGS="variant:${VARIANT_TAG} compute:${COMPUTE_TAG}"
export BASELINE_RUN_IDS="${BASELINE_RUN_IDS:-us4obwe3 iie3967j 2p0n1i0j}"
...
export FULL_EVAL_EVERY_N_EVALS="${FULL_EVAL_EVERY_N_EVALS:-0}"
export FULL_EVAL_NAME="${FULL_EVAL_NAME:-dev_full}"
...
export RUN_VERDICT="${RUN_VERDICT:-Weight scout v3: T_beta=${TCM_POS}, T_alpha=${TCM_NEG}, inference_tau=${INFER_TAU}, pos_w=${TCM_POS_W}, neg_w=${TCM_NEG_W}.}"

echo "[NTCM_WT_V3] config=${CONFIG_PATH} raw_task=${TASK_ID_RAW} offset=${TASK_OFFSET} row=${TASK_ID} variant=${VARIANT_TAG}"
echo "[NTCM_WT_V3] threshold_role=${THRESHOLD_ROLE} pos=${TCM_POS} neg=${TCM_NEG} weights pos=${TCM_POS_W} neg=${TCM_NEG_W} infer_tau=${INFER_TAU}"
echo "[NTCM_WT_V3] target_step=${TARGET_MAX_STEPS} compute=${COMPUTE_TAG} batch=$((NUM_GPUS * PER_GPU_BATCH))"
