#!/bin/bash
#SBATCH --job-name=q3_gsdedup_tau_gs100k
...
#SBATCH --gres=gpu:1
#SBATCH --time=8:00:00
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_gsdedup_tau_gs100k_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_gsdedup_tau_gs100k_%x.err
...
export VARIANT_TAG="tau_sweep_gsdedup_conv5_dev_gs100k"
export VERSION="3var_gsv2full_gsfix_mfa_gsdedup_tcmoff_conv5_tau_sweep_dev_gs100k_v3_${COMPUTE_TAG}_smallest_dense"
export WANDB_EXP_NAME="gsdedup_tcmoff_conv5_tau_sweep_dev_gs100k_v3_${COMPUTE_TAG}"
export NOTES_FILE="/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/notes_hn1024_gsv2full_gsdedup_tcmoff_conv5_tau_sweep_dev_gs100k_v3.md"
...
export MASTER_PORT="${MASTER_PORT:-30344}"

export DATA_TAG="3variant_gsv2full_gsfix_mfa_gsdedup_tau_sweep_dev_gs100k_eval"
...
export BASELINE_RUN_IDS="7xu2b4so bdmc71za e9f4xnd1"
...
export EVAL_GLOSSARY_SIZES="100000"
...
export BEST_METRIC="eval_dev/topk10_chunk_any_positive_filtered_recall@tau_0p75_gs100000"
...
export RUN_VERDICT="Dense inference tau sweep for the gsdedup TCM-off conv5 best checkpoint. Metrics denominator is fixed to the strict raw/base glossary; dev/ACL retriever banks change with glossary size."

echo "[GSDEDUP_TAU_SWEEP_GS100K] ckpt=${RESUME}"
echo "[GSDEDUP_TAU_SWEEP_GS100K] dev=${DEV_JSONL}"
echo "[GSDEDUP_TAU_SWEEP_GS100K] acl=${ACL_DEV_JSONL}"
echo "[GSDEDUP_TAU_SWEEP_GS100K] eval_metric_denominator=${EVAL_METRIC_DENOMINATOR}"
echo "[GSDEDUP_TAU_SWEEP_GS100K] dev_glossary=${EVAL_WIKI_GLOSSARY} sizes=${EVAL_GLOSSARY_SIZES}"
echo "[GSDEDUP_TAU_SWEEP_GS100K] acl_glossary=${ACL_EVAL_WIKI_GLOSSARY} sizes=${ACL_EVAL_GLOSSARY_SIZES}"
echo "[GSDEDUP_TAU_SWEEP_GS100K] beta=${TCM_SWEEP_FBETA} taus=${TCM_SWEEP_THRESHOLDS}"
echo "[GSDEDUP_TAU_SWEEP_GS100K] compute=${COMPUTE_TAG} batch=$((NUM_GPUS * PER_GPU_BATCH))"
