#!/bin/bash
#SBATCH --job-name=q3_tcmoff_acl_eval
...
#SBATCH --gres=gpu:1
#SBATCH --time=4:00:00
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tcmoff_acl_eval_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_tcmoff_acl_eval_%x.err
...
export VARIANT_TAG="acl_raw1k10k_tcmoff2650"
export VERSION="3var_gsv2full_gsfix_mfa_tcmoff2650_acl_raw1k10k_v3_${COMPUTE_TAG}_smallest_dense"
export WANDB_EXP_NAME="tcmoff2650_acl_raw1k10k_v3_${COMPUTE_TAG}"
export NOTES_FILE="/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/notes_hn1024_gsv2full_tcmoff_step2650_acl_raw1k10k_eval_v3.md"
...
export TCM_POS_LOSS_WEIGHT=0
export TCM_NEG_LOSS_WEIGHT=0
...
export MASTER_PORT="${MASTER_PORT:-30341}"

export DATA_TAG="3variant_gsv2full_gsfix_mfa_acl_raw1k10k_eval_tcmoff2650"
export EXPERIMENT_FAMILY="sst_tcm_noterm"
export EXTRA_WANDB_TAGS="variant:${VARIANT_TAG} compute:${COMPUTE_TAG}"
export BASELINE_RUN_IDS="us4obwe3 j0e4un3k"
...
export RUN_VERDICT="Eval-only ACL raw/gs1k/gs10k tau-filtered recall and dev-v3 no-term noise for the TCM-off step-2650 baseline at tau=0.75."

echo "[TCMOFF_ACL_EVAL] ckpt=${RESUME}"
echo "[TCMOFF_ACL_EVAL] acl=${ACL_DEV_JSONL}"
echo "[TCMOFF_ACL_EVAL] glossary=${EVAL_WIKI_GLOSSARY} sizes=${EVAL_GLOSSARY_SIZES} tau=${TCM_SWEEP_THRESHOLDS}"
echo "[TCMOFF_ACL_EVAL] compute=${COMPUTE_TAG} batch=$((NUM_GPUS * PER_GPU_BATCH))"
