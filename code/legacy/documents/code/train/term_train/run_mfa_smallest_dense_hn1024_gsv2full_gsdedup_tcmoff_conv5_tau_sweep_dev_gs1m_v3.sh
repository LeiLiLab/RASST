#!/bin/bash
#SBATCH --job-name=q3_gsdedup_tau_gs1m
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_gsdedup_tau_gs1m_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_gsdedup_tau_gs1m_%x.err
...
export VARIANT_TAG="tau_sweep_gsdedup_conv5_dev_gs1m"
export VERSION="3var_gsv2full_gsfix_mfa_gsdedup_tcmoff_conv5_tau_sweep_dev_gs1m_v3_${COMPUTE_TAG}_smallest_dense"
export WANDB_EXP_NAME="gsdedup_tcmoff_conv5_tau_sweep_dev_gs1m_v3_${COMPUTE_TAG}"
export NOTES_FILE="/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/notes_hn1024_gsv2full_gsdedup_tcmoff_conv5_tau_sweep_dev_gs1m_v3.md"
...
export MASTER_PORT="${MASTER_PORT:-30345}"

export DATA_TAG="3variant_gsv2full_gsfix_mfa_gsdedup_tau_sweep_dev_gs1m_eval"
...
export BEST_METRIC="eval_dev/topk10_filtered_recall@tau_0p75_gs1000000"
...
export RUN_VERDICT="Dev-only dense inference tau sweep for the gsdedup TCM-off conv5 best checkpoint using the P31 1M glossary. ACL is intentionally disabled to avoid using test/OOD data for calibration."

echo "[GSDEDUP_TAU_SWEEP_GS1M] ckpt=${RESUME}"
echo "[GSDEDUP_TAU_SWEEP_GS1M] dev=${DEV_JSONL}"
echo "[GSDEDUP_TAU_SWEEP_GS1M] acl=<disabled for dev-only calibration>"
echo "[GSDEDUP_TAU_SWEEP_GS1M] dev_glossary=${EVAL_WIKI_GLOSSARY} sizes=${EVAL_GLOSSARY_SIZES}"
echo "[GSDEDUP_TAU_SWEEP_GS1M] beta=${TCM_SWEEP_FBETA} taus=${TCM_SWEEP_THRESHOLDS}"
echo "[GSDEDUP_TAU_SWEEP_GS1M] compute=${COMPUTE_TAG} batch=$((NUM_GPUS * PER_GPU_BATCH))"
