#!/bin/bash
#SBATCH --job-name=qwen3_enriched_v2
#SBATCH --partition=aries
#SBATCH --nodes=1
#SBATCH --cpus-per-task=64
#SBATCH --mem=256G
#SBATCH --gres=gpu:8
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_qwen3_enriched_v2.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_qwen3_enriched_v2.err
...
SCRIPT_PATH="/mnt/taurus/home/jiaxuanluo/InfiniSST/documents/code/train/term_train/qwen3_AuT_BGE_M3_train_lora_unfrozen_text_single_direction_loss.py"
RESUME_PATH=""
ENRICHED_TERMS_PKL="/mnt/gemini/data1/jiaxuanluo/enriched_chunk_terms.pkl"
# --- LoRA 消融实验配置区 ---
USE_LORA="true"
LORA_RANK=32
LORA_ALPHA=64
...
SAVE_NAME="q3rag_${MODE_NAME}_bs${BS_ABBR}_w${TERM_WEIGHT}-${TRANS_WEIGHT}-ttm=${TEXT_TARGET_MODULES}-temperature=${TEMPERATURE}_enriched_v2"
SAVE_PATH="/mnt/gemini/data/jiaxuanluo/${SAVE_NAME}.pt"

echo "[INFO] Starting Qwen3-Omni Stage 2 Training (Enriched Labels v2)..."
echo "[INFO] Save Path: ${SAVE_PATH}"
echo "[INFO] Resume From: ${RESUME_PATH}"
echo "[INFO] Enriched Terms: ${ENRICHED_TERMS_PKL}"
...
torchrun \
    ...
    --save_path "${SAVE_PATH}" \
    --enriched_terms_pkl "${ENRICHED_TERMS_PKL}" \
    --lr ${LR} \
...
echo "[INFO] Enriched v2 Training completed!"
