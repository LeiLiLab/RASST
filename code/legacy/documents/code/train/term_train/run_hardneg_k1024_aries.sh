#!/bin/bash
#SBATCH --job-name=q3_variantE_k1024
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_variantE_k1024_%x.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_q3_variantE_k1024_%x.err

# Variant E with HARD_NEG_K=1024 (up from 64).  Rationale:
#   * 43812 resume run (k=64, from ep5 best) failed to recover ACL6060 gs10000
#     back to 0.9085 after 640 extra steps, even though DEV gs10k briefly
#     peaked and receded.  TCM_pos / TCM_neg stayed saturated.  Reading:
#     the bank+InfoNCE at k=64 has exhausted its in-domain signal against
#     the legacy external hard-neg glossary path, now removed from the default bank.
#   * Hypothesis: giving each sample access to a much larger top-k slice of
#     the 1.3M-term bank will push "near-hard" negatives into the batch that
#     k=64 never picks, breaking the observed OOD plateau.
#   * Training-time cost: mine_hard_negatives() is chunked at 32k so the
#     mining wall time only grows with k via the topk merge step.  Unique
#     negatives per batch approach bank size, so the expensive term is the
#     contrastive sim matrix [global_B, W, N_total].  To keep peak memory
#     sane we halve GRAD_CACHE_CHUNK_SIZE (256 -> 128).  Global batch and
#     optimizer semantics are unchanged.
#
# Budget: 5 full epochs from scratch (matches k=64 baseline for apples-to-apples).
# ~530 steps/epoch * 5 = 2650 steps.  Expected wall time: ~15-20 s/step * 2650
# = 11-15h (mining chunk + bigger unique_count add ~3-6 s/step).  slurm
# --time=28h gives 2x safety margin.
#
# Submit: sbatch run_hardneg_k1024_aries.sh        (full 5-epoch)
#         MAX_STEPS=50 sbatch run_hardneg_k1024_aries.sh  (smoke test)

set -euo pipefail

# ======Configuration=====
# --- Loss (kept identical to variant E baseline, change ONLY hard_neg_k) ---
MARGIN="0.0"
HCL_BETA="0.0"
TCM_LOSS_WEIGHT="1.0"
...
# --- Data-side hard-neg bank (the ONLY knob moving for this run) ---
HARD_NEG_K=1024
NEG_BANK_SIZE=0
NEG_BANK_REFRESH_STEPS=50

# --- Shared recipe (mirrors run_hardneg_tcm_aries.sh baseline) ---
...
# Matches k=64 baseline (clean ablation: only hard_neg_k moves).
# Smoke test 43814 verified chunk=128 + k=1024 used only 19 GB/49 GB per card,
# so we can safely keep chunk=256 (estimated 30 GB/card, 19 GB safety margin).
GRAD_CACHE_CHUNK_SIZE=256
MAX_TRAIN_SECONDS="${MAX_TRAIN_SECONDS:-0}"
EPOCHS="${EPOCHS:-5}"
# Smoke-test knob: MAX_STEPS=50 via env will cap training at 50 steps regardless
# of EPOCHS.  Default 0 = no cap (full run).
...
VERSION="3var_clean_gc_wr$((WIKI_RANK / 1000))k_m${MARGIN}_maxsim_mfa_variantE_hardneg_k${HARD_NEG_K}_tcm_ep${EPOCHS}${SMOKE_TAG}"
SAVE_NAME="q3rag_${MODE_NAME}_bs${BS_ABBR}_t=${TEMPERATURE}_${VERSION}"
SAVE_PATH="${SAVE_DIR}/${SAVE_NAME}.pt"
WANDB_EXP_NAME="variantE_k${HARD_NEG_K}_${SAVE_NAME}"

echo "[TRAIN] VARIANT=E_k${HARD_NEG_K} (from scratch, 5 epoch)"
...
echo "[TRAIN] Variant E k${HARD_NEG_K} completed at $(date)"
