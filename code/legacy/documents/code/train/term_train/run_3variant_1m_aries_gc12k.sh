...
# GradCache: 12K effective batch (1536/GPU), chunk_size=512 for memory efficiency.
# Audio LoRA rank=128 (up from 32) for more expressive audio encoder.
...
# Training — GradCache: large effective batch, small sub-batch for memory
PER_GPU_BATCH=1536
...
# Eval & checkpointing (scaled for 12K batch: 3x larger → 3x fewer steps)
SAVE_STEPS=100
...
echo "[TRAIN] 3-variant 1M wiki terms training (GradCache ${BATCH_SIZE} batch, chunk=${GRAD_CACHE_CHUNK_SIZE})"
