#!/bin/bash
#SBATCH --job-name=qwen3_gneg
#SBATCH --partition=aries
#SBATCH --nodes=1
#SBATCH --cpus-per-task=48
#SBATCH --mem=256G
#SBATCH --gres=gpu:8
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_qwen3_gneg.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_qwen3_gneg.err
...
# Paths (gigaspeech + wiki_synth 1M, paths normalized for cross-machine access)
TRAIN_JSONL="/mnt/gemini/data1/jiaxuanluo/term_train_with_wiki_synth_normalized.jsonl"
...
#RESUME_PATH="/mnt/gemini/data/jiaxuanluo/q3rag_scale_lora-r32-tr128_bs4k_t=0.03_v2_wiki1M_lr5e5.pt"
RESUME_PATH=""
# Audio model (LoRA)
USE_LORA="true"
LORA_RANK=32
...
# Text encoder: LoRA r256 + dense, low LR to limit embedding drift
# r64 @ lr=1e-4 was stable; r256 needs ~10x lower LR to match drift rate
...
# Training — 512/GPU is the max that fits 48GB GPUs
PER_GPU_BATCH=512
...
# Glossary negatives: disabled — in-batch 4k negatives sufficient, avoids stale-embedding
# instability when text LoRA rank >= 128 causes fast embedding drift
GLOSSARY_NEG_PATH=""
...
# Eval & checkpointing — more frequent eval to catch optimal point early
SAVE_STEPS=300
EVAL_STEPS_SAMPLE=100
KEEP_CHECKPOINTS=5
# Must match recall@{EVAL_TOPK}_gs* metric keys from qwen3_glossary_neg_train.py
EVAL_TOPK=10
...
# Second best ckpt: ACL6060 @ glossary scale 10k (saved as *_best_acl6060_gs10000.pt)
...
echo "[INFO] Scale Training: LoRA r64 + Glossary Neg"
...
echo "[INFO] Glossary neg: ${GLOSSARY_NEG_PATH} refresh=${GLOSSARY_NEG_REFRESH_STEPS}"
...
echo "[INFO] Eval top-k: ${EVAL_TOPK}"
echo "[INFO] Best metric (primary): ${BEST_METRIC}"
echo "[INFO] Best metric (secondary, gs10k ACL): ${BEST_METRIC_SECONDARY}"
