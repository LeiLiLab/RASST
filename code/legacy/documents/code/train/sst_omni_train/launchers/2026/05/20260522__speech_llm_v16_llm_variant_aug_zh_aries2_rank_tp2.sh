#!/usr/bin/env bash
# 2-GPU Speech LLM SFT launcher for V16 LLM-variant rank-capacity sweeps.
set -euo pipefail
...
NOTES_FILE="${NOTES_FILE_OVERRIDE:-${ROOT_DIR}/documents/code/train/sst_omni_train/notes/2026/05/20260522__speech_llm_v16_llm_variant_aug_zh_${RANK_TAG}_tp2.md}"
SAVE_BASE="${SAVE_BASE_OVERRIDE:-/mnt/aries/data7/jiaxuanluo/slm/speech_llm_v16_llm_variant_aug_retriever_timeline_zh_lh1b88kw_tau073_${RANK_TAG}_tp2_aries2}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/speech_llm_v16_llm_variant_aug_retriever_timeline_zh_${RANK_TAG}_tp2_aries2}"
...
TAGS=(
  ...
  "variant:v16_llmvariant_${RANK_TAG}_tp2"
  "status:running"
  "compute:aries2"
)
for tag in "${TAGS[@]}"; do
...
WANDB_EXP_PREFIX="${WANDB_EXP_PREFIX_OVERRIDE:-speech-llm-v16-llmvariant-zh-${RANK_TAG}-tp2-aries2}" \
