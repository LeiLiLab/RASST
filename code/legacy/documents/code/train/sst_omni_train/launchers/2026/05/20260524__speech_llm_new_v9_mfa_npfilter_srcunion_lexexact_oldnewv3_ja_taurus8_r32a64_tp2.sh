#!/usr/bin/env bash
# 8-GPU Speech LLM SFT launcher for clean ja New V9 MFA npfilter source-union lexexact data.
set -euo pipefail
...
NOTES_FILE="${NOTES_FILE_OVERRIDE:-${ROOT_DIR}/documents/code/train/sst_omni_train/notes/2026/05/20260524__speech_llm_new_v9_mfa_npfilter_srcunion_lexexact_oldnewv3_ja_r32a64_tp2_taurus8.md}"

LORA_RANK="${LORA_RANK_OVERRIDE:-32}"
LORA_ALPHA="${LORA_ALPHA_OVERRIDE:-64}"
RANK_TAG="r${LORA_RANK}a${LORA_ALPHA}"
SAVE_BASE="${SAVE_BASE_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_new_v9_mfa_npfilter_srcunion_lexexact_oldnewv3_ja_${RANK_TAG}_tp2_taurus8}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/speech_llm_new_v9_mfa_npfilter_srcunion_lexexact_oldnewv3_ja_${RANK_TAG}_tp2_taurus8}"
...
MASTER_PORT="${MASTER_PORT_OVERRIDE:-29658}"
...
WANDB_EXP_PREFIX="${WANDB_EXP_PREFIX_OVERRIDE:-speech-llm-new_v9-mfa-npfilter-lexexact-oldnewv3-ja-${RANK_TAG}-tp2-taurus8}" \
