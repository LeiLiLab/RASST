#!/usr/bin/env bash
# 6-GPU Speech LLM SFT launcher for clean ja New V9 MFA npfilter source-union lexexact data.
set -euo pipefail
...
LANG_CODE="ja"
DATA_DIR="${DATA_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/speech_llm_new_v9_mfa_npfilter_srcunion_lexexact_oldnewv3_ja_20260524}"
DATASET_PATH="${DATASET_PATH_OVERRIDE:-${DATA_DIR}/train_s_ja_new_v9_mfa_openai_rewrite_oldnewv3.jsonl}"
VAL_DATASET="${VAL_DATASET_OVERRIDE:-${DATA_DIR}/dev_s_ja_new_v9_mfa_openai_rewrite_oldnewv3_first355.jsonl}"
NOTES_FILE="${NOTES_FILE_OVERRIDE:-${ROOT_DIR}/documents/code/train/sst_omni_train/notes/2026/05/20260524__speech_llm_new_v9_mfa_npfilter_srcunion_lexexact_oldnewv3_ja_r32a64_tp2_aries6.md}"

LORA_RANK="${LORA_RANK_OVERRIDE:-32}"
LORA_ALPHA="${LORA_ALPHA_OVERRIDE:-64}"
RANK_TAG="r${LORA_RANK}a${LORA_ALPHA}"
SAVE_BASE="${SAVE_BASE_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_new_v9_mfa_npfilter_srcunion_lexexact_oldnewv3_ja_${RANK_TAG}_tp2_aries6}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/speech_llm_new_v9_mfa_npfilter_srcunion_lexexact_oldnewv3_ja_${RANK_TAG}_tp2_aries6}"
...
ALLOCATED_GPUS="${HOST_GPU_DEVICES_OVERRIDE:-${CUDA_VISIBLE_DEVICES:-0,1,2,5,6,7}}"
...
TAGS=(
  ...
  "task:train"
  "data:new_v9_mfa_ja"
  "variant:npfilter_lexexact_${RANK_TAG}_tp2"
  "status:running"
  "compute:aries6"
)
for tag in "${TAGS[@]}"; do
...
WANDB_EXP_PREFIX="${WANDB_EXP_PREFIX_OVERRIDE:-speech-llm-new_v9-mfa-npfilter-lexexact-oldnewv3-ja-${RANK_TAG}-tp2-aries6}" \
