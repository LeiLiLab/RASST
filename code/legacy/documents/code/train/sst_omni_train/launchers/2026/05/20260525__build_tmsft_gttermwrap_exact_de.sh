#!/usr/bin/env bash
# Build German TM-SFT data with exact assistant <term> wrapping.
set -euo pipefail
...
SOURCE_JSONL="${SOURCE_JSONL_OVERRIDE:-${DATA_ROOT}/train_s_de_v4_ner_baseline_aligned_rate1.0_k20_final.jsonl}"
OUT_DIR="${OUT_DIR_OVERRIDE:-${DATA_ROOT}/speech_llm_tmsft_gttermwrap_exact_de_20260525}"
...
STAGE0_TRAIN="${OUT_DIR}/stage0_train_s_de_tmsft_exact_gt_from_termmap.jsonl"
STAGE0_DEV="${OUT_DIR}/stage0_dev_s_de_tmsft_exact_gt_from_termmap_first${DEV_ROWS}.jsonl"
FINAL_TRAIN="${OUT_DIR}/train_s_de_tmsft_gttermwrap_exact.jsonl"
FINAL_DEV="${OUT_DIR}/dev_s_de_tmsft_gttermwrap_exact_first${DEV_ROWS}.jsonl"
SUMMARY_JSON="${OUT_DIR}/tmsft_gttermwrap_exact_de_summary.json"
...
python3 - "${SOURCE_JSONL}" "${FINAL_TRAIN}" "${FINAL_DEV}" "${SUMMARY_JSON}" "${OUT_DIR}" <<'PY'
import json
import re
import sys
from pathlib import Path

source, train, dev, summary, out_dir = map(Path, sys.argv[1:])
...
def validate(path):
    rows = chunks = gt_terms = tagged_rows = tagged_assistant_messages = 0
    malformed = latin_cut = gt_chunks = termmap_chunks = 0
    sample_rows = []
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if not isinstance(messages, list) or not isinstance(audios, list) or not isinstance(gt, list):
                raise SystemExit(f"[ERROR] malformed row {rows} in {path}")
            user_indices = [
            ...
            if len(user_indices) != len(audios) or len(gt) != len(audios):
                raise SystemExit(
                    f"[ERROR] row {rows}: user/audio/gt mismatch "
            ...
            if row_has_tag and len(sample_rows) < 20:
                sample_rows.append({
                    "row": rows,
                    "gt_terms_by_chunk": gt[:3],
                    "assistant_tags": [
...
summary_obj = {
    "event": "tmsft_gttermwrap_exact_de",
    ...
    "stats": {
        ...
        "stage0_dev_exact_gt": load("stage0_dev_exact_gt_first355_stats.json"),
        "train_wrap": load("train_gttermwrap_exact_stats.json"),
        "dev_wrap": load("dev_gttermwrap_exact_first355_stats.json"),
    },
    "validation": {
...
print(json.dumps(summary_obj["validation"], ensure_ascii=False, indent=2, sort_keys=True), flush=True)
PY
