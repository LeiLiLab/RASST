#!/usr/bin/env bash
# Build ja New V9 Speech LLM SFT data with the same clean line used for de:
# old-new_v3 source candidate gate + source glossary union + exact-span GT.
set -euo pipefail
...
OUT_DIR="${OUT_ROOT}/${OUT_NAME_PREFIX}_ja_20260524"
...
SPACY_SHARDS="${SPACY_SHARDS_OVERRIDE:-4}"

INPUT_JSONL="${DATA_ROOT}/train_s_ja_v4_ner_baseline_aligned_rate1.0_k20_final.jsonl"
INPUT_TSV="${DATA_ROOT}/train_xl_case_robust_asr-filtered_ja_metricx-qe3.0_align.tsv"
CAND_PREFIX="${CAND_DIR}/source_candidates_ja_oldnewv3_spacy"
CAND_JSONL="${CAND_DIR}/source_candidates_ja_oldnewv3_spacy.jsonl"
...
MERGED_SOURCE_GLOSSARY="${OUT_DIR}/source_glossary_wiki100k_plus_ja_oldnewv3_candidates.json"
...
if [[ ! -s "${CAND_JSONL}" ]]; then
  ...
  for ((shard=0; shard<SPACY_SHARDS; shard++)); do
    ...
    log_file="${LOG_DIR}/stageP0_spacy_candidates_ja_shard${shard}.log"
    (
      env \
...
echo "[STAGE P1] Merge wiki100k with ja old-new_v3 source candidates for source exact matching"
...
echo "[STAGE A-D] Run MFA exact-span GT + old-new_v3 term_map builder with source candidate allowlist"
...
export USE_LEGACY_TERMMAP_SPAN_AS_TARGET_WITHOUT_OPENAI_OVERRIDE="${USE_LEGACY_TERMMAP_SPAN_AS_TARGET_WITHOUT_OPENAI_OVERRIDE:-1}"
...
echo "[OK] ja npfilter New V9 data: ${OUT_DIR}"
