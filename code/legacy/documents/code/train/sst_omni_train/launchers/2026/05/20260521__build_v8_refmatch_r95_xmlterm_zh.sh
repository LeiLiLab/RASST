...
OUT_DIR="${OUT_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/speech_llm_v8_refmatch_r95_xmlterm_zh_lh1b88kw_tau073_srcmatch100k_20260521}"
...
run_one() {
  local split="$1"
  local input="$2"
  local output="${OUT_DIR}/${split}_s_zh_v8_refmatch_r95_xmlterm_lh1b88kw_tau073_srcmatch100k.jsonl"
  local stats="${OUT_DIR}/${split}_s_zh_v8_refmatch_r95_xmlterm_stats.json"
  local sample="${OUT_DIR}/${split}_s_zh_v8_refmatch_r95_xmlterm_samples.json"
  echo "[INFO] Building ${split}/v8_refmatch_r95_xmlterm: ${output}"
  "${PYTHON_BIN}" "${SCRIPT}" \
    --input-jsonl "${input}" \
    --output-jsonl "${output}" \
    --stats-json "${stats}" \
    --sample-json "${sample}" \
    --variant refmatch_r95 \
    --term-map-style xml_tagged \
    --gt-target-match-policy full_ref \
    --lang-code zh \
    --seed 20260521
}
...
for p in sorted(out_dir.glob("*_stats.json")):
    ...
    if d.get("term_map_style") != "xml_tagged":
        raise SystemExit(f"[ERROR] unexpected term_map_style in {p}: {d.get('term_map_style')}")
    key = p.name.replace("_stats.json", "")
    rate = d["gt_term_in_term_map_rate"]
    summary[key] = {
        "rows_written": d["rows_written"],
        "chunks": d["chunks"],
        "mode_counts": d["mode_counts"],
        "raw_gt_terms_total": d["raw_gt_terms_total"],
        "target_match_kept_gt_terms": d["target_match_kept_gt_terms"],
        "target_match_dropped_gt_terms": d["target_match_dropped_gt_terms"],
        "target_match_kept_gt_term_rate": d["target_match_kept_gt_term_rate"],
        "target_match_kept_gt_chunk_rate": d["target_match_kept_gt_chunk_rate"],
        "gt_term_in_term_map_rate": rate,
    ...
    if not 0.94 <= rate <= 0.965:
        raise SystemExit(f"[ERROR] gt_term_in_term_map_rate outside target range for {p}: {rate}")
summary_path = out_dir / "v8_refmatch_r95_xmlterm_summary.json"
summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(json.dumps(summary, ensure_ascii=False, indent=2))
print(f"[INFO] Wrote summary: {summary_path}")
PY

echo "[INFO] V8 refmatch R95 XML-term data ready: ${OUT_DIR}"
