#!/usr/bin/env bash
# 6-GPU Speech LLM SFT launcher for German cap16 denoise-budget short-tag data.
set -euo pipefail
...
NOTES_FILE="${NOTES_FILE_OVERRIDE:-${ROOT_DIR}/documents/code/train/sst_omni_train/notes/2026/05/20260525__speech_llm_de_cap16_denoise_budget_ttag_r32a32_ep1_taurus6.md}"

LORA_RANK="${LORA_RANK_OVERRIDE:-32}"
LORA_ALPHA="${LORA_ALPHA_OVERRIDE:-32}"
RANK_TAG="r${LORA_RANK}a${LORA_ALPHA}"
SAVE_BASE="${SAVE_BASE_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_de_cap16_denoise_budget_ttag_${RANK_TAG}_ep1_taurus6}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/speech_llm_de_cap16_denoise_budget_ttag_${RANK_TAG}_ep1_taurus6}"
HF_EXPORT_STAGE_ROOT="${HF_EXPORT_STAGE_ROOT_OVERRIDE:-/mnt/data1/jiaxuanluo/hf_export_stage/speech_llm_de_cap16_denoise_budget_ttag_${RANK_TAG}_ep1_taurus6}"
HF_EXPORT_MIN_FREE_GB="${HF_EXPORT_MIN_FREE_GB_OVERRIDE:-90}"
HF_EXPORT_LOCAL_CACHE_ROOT="${HF_EXPORT_LOCAL_CACHE_ROOT_OVERRIDE:-/mnt/data1/jiaxuanluo/slm_cache/speech_llm_de_cap16_denoise_budget_ttag_${RANK_TAG}_ep1_taurus6/keep1.0_r32}"
HF_EXPORT_LOCAL_LATEST_LINK="${HF_EXPORT_LOCAL_LATEST_LINK_OVERRIDE:-/mnt/data1/jiaxuanluo/slm_cache/speech_llm_de_cap16_denoise_budget_ttag_${RANK_TAG}_ep1_taurus6/latest-hf}"
...
MASTER_PORT="${MASTER_PORT_OVERRIDE:-29716}"
...
ALLOCATED_GPUS="${HOST_GPU_DEVICES_OVERRIDE:-${CUDA_VISIBLE_DEVICES:-1,2,3,4,5,6}}"
IFS=',' read -r -a GPU_ARR <<< "${ALLOCATED_GPUS}"
if (( ${#GPU_ARR[@]} != 6 )); then
  echo "[ERROR] This launcher expects exactly 6 GPUs; got ${ALLOCATED_GPUS}" >&2
  exit 2
fi
...
TAGS=(
  ...
  "task:train"
  "data:de_c16_den_t"
  "variant:den_t_${RANK_TAG}_ep1_6g"
  "status:running"
  "compute:taurus6"
)
for tag in "${TAGS[@]}"; do
...
WANDB_EXP_PREFIX="${WANDB_EXP_PREFIX_OVERRIDE:-speech-llm-de-c16-denoise-ttag-${RANK_TAG}-ep1-taurus6}" \
