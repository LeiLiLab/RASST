#!/usr/bin/env bash
# Wait for all 8 Aries GPUs to be idle, then run TM-SFT exact GT term wrapping SFT.
set -euo pipefail

ROOT_DIR="${ROOT_DIR_OVERRIDE:-/mnt/taurus/home/jiaxuanluo/InfiniSST}"
TRAIN_LAUNCHER="${TRAIN_LAUNCHER_OVERRIDE:-${ROOT_DIR}/documents/code/train/sst_omni_train/launchers/2026/05/20260525__speech_llm_tmsft_gttermwrap_exact_de_aries8_r32a32_ep4.sh}"
...
MASTER_PORT_OVERRIDE="${MASTER_PORT_OVERRIDE:-29658}" \
bash "${TRAIN_LAUNCHER}"
