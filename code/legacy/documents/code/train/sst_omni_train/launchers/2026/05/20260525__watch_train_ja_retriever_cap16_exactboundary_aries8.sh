#!/usr/bin/env bash
# Wait for all 8 Aries GPUs to be idle, then run Japanese retriever cap16 exact-boundary SFT.
set -euo pipefail
...
TRAIN_LAUNCHER="${TRAIN_LAUNCHER_OVERRIDE:-${ROOT_DIR}/documents/code/train/sst_omni_train/launchers/2026/05/20260525__speech_llm_ja_retriever_cap16_exactboundary_aries8_r32a32_ep4.sh}"
...
MASTER_PORT_OVERRIDE="${MASTER_PORT_OVERRIDE:-29718}" \
bash "${TRAIN_LAUNCHER}"
