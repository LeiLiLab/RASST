...
OUT_DIR="${OUT_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/speech_llm_v5_refmatch_precision_termmap_zh_lh1b88kw_tau073_srcmatch100k_20260521}"
...
run_one() {
  local split="$1"
  local input="$2"
  local output="${OUT_DIR}/${split}_s_zh_v5_refmatch_precision_termmap_lh1b88kw_tau073_srcmatch100k.jsonl"
  local stats="${OUT_DIR}/${split}_s_zh_v5_refmatch_precision_termmap_stats.json"
  local sample="${OUT_DIR}/${split}_s_zh_v5_refmatch_precision_termmap_samples.json"
  echo "[INFO] Building ${split}/v5_refmatch_precision: ${output}"
  "${PYTHON_BIN}" "${SCRIPT}" \
    --input-jsonl "${input}" \
    --output-jsonl "${output}" \
    --stats-json "${stats}" \
...
summary_path = out_dir / "v5_refmatch_precision_termmap_summary.json"
summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(json.dumps(summary, ensure_ascii=False, indent=2))
print(f"[INFO] Wrote summary: {summary_path}")
PY

echo "[INFO] V5 refmatch precision retriever term_map data ready: ${OUT_DIR}"
