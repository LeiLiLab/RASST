#!/usr/bin/env bash
# 8-GPU Speech LLM SFT launcher for German HN1024 retriever cap16 exact-boundary data.
set -euo pipefail
...
LANG_CODE="de"
DATA_DIR="${DATA_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/speech_llm_deja_termmap_ablation_cap16_exactboundary_20260525/de/retriever_hn1024_tau078_cap16_exactboundary}"
DATASET_PATH="${DATASET_PATH_OVERRIDE:-${DATA_DIR}/train_s_de_retriever_hn1024_tau078_cap16_gttermwrap_exactboundary.jsonl}"
VAL_DATASET="${VAL_DATASET_OVERRIDE:-${DATA_DIR}/dev_s_de_retriever_hn1024_tau078_cap16_gttermwrap_exactboundary_first355.jsonl}"
NOTES_FILE="${NOTES_FILE_OVERRIDE:-${ROOT_DIR}/documents/code/train/sst_omni_train/notes/2026/05/20260525__speech_llm_de_retriever_cap16_exactboundary_r32a32_ep4_taurus8.md}"

LORA_RANK="${LORA_RANK_OVERRIDE:-32}"
LORA_ALPHA="${LORA_ALPHA_OVERRIDE:-32}"
RANK_TAG="r${LORA_RANK}a${LORA_ALPHA}"
SAVE_BASE="${SAVE_BASE_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_de_retriever_cap16_exactboundary_${RANK_TAG}_ep4_taurus8}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/speech_llm_de_retriever_cap16_exactboundary_${RANK_TAG}_ep4_taurus8}"
...
MASTER_PORT="${MASTER_PORT_OVERRIDE:-29704}"
...
WANDB_EXP_PREFIX="${WANDB_EXP_PREFIX_OVERRIDE:-speech-llm-de-retr-cap16-exact-${RANK_TAG}-ep4-taurus8}" \
