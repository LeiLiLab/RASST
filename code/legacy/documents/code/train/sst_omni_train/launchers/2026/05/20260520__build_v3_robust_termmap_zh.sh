...
OUT_DIR="${OUT_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/speech_llm_v3_robust_termmap_zh_lh1b88kw_tau073_srcmatch100k_20260520}"
...
run_one() {
  ...
  local variant="$3"
  local style="$4"
  local output="${OUT_DIR}/${split}_s_zh_v3_${variant}_termmap_lh1b88kw_tau073_srcmatch100k.jsonl"
  local stats="${OUT_DIR}/${split}_s_zh_v3_${variant}_termmap_stats.json"
  local sample="${OUT_DIR}/${split}_s_zh_v3_${variant}_termmap_samples.json"
  ...
  "${PYTHON_BIN}" "${SCRIPT}" \
    ...
    --sample-json "${sample}" \
    --variant "${variant}" \
    --term-map-style "${style}" \
    --lang-code zh \
    --seed 20260520
}

run_one train "${TRAIN_INPUT}" real plain
run_one dev "${DEV_INPUT}" real plain
run_one train "${TRAIN_INPUT}" tagged tagged
run_one dev "${DEV_INPUT}" tagged tagged
run_one train "${TRAIN_INPUT}" adv plain
run_one dev "${DEV_INPUT}" adv plain
...
for p in sorted(out_dir.glob("*_stats.json")):
    ...
    summary[key] = {
        ...
        "translation_swap_entries": d["translation_swap_entries"],
        "term_map_size_hist": d["term_map_size_hist"],
    }
summary_path = out_dir / "v3_robust_termmap_summary.json"
summary_path.write_text(json.dumps(summary, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
print(json.dumps(summary, ensure_ascii=False, indent=2))
print(f"[INFO] Wrote summary: {summary_path}")
PY

echo "[INFO] V3 robust term_map data ready: ${OUT_DIR}"
