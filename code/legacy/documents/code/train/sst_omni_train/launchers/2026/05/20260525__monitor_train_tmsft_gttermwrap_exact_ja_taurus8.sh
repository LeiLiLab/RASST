#!/usr/bin/env bash
# Monitor Taurus TM-SFT exact GT term-wrap training and register completion.
set -euo pipefail

ROOT_DIR="${ROOT_DIR_OVERRIDE:-/home/jiaxuanluo/InfiniSST}"
EVENT_ID="20260525T0250__speech_llm_train__tmsft_gttermwrap_exact_ja_r32a32_ep4_taurus8"
MANIFEST="${ROOT_DIR}/documents/code/train/sst_omni_train/manifests/2026/05/${EVENT_ID}.json"
TRAIN_PID="${TRAIN_PID_OVERRIDE:?TRAIN_PID_OVERRIDE is required}"
WANDB_RUN_ID="${WANDB_RUN_ID_OVERRIDE:-}"
RUN_ROOT="${RUN_ROOT_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_tmsft_gttermwrap_exact_ja_r32a32_ep4_taurus8}"
TRAIN_LOG_DIR="${TRAIN_LOG_DIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/speech_llm_tmsft_gttermwrap_exact_ja_r32a32_ep4_taurus8}"
DETACHED_OUT="${DETACHED_OUT_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/train_tmsft_gttermwrap_exact_ja_taurus8_20260525T0250.out}"
DETACHED_ERR="${DETACHED_ERR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/logs/train_tmsft_gttermwrap_exact_ja_taurus8_20260525T0250.err}"
...
echo "[MONITOR] pid exited at $(date -u --iso-8601=seconds)"

mapfile -t HF_DIRS < <(find "${RUN_ROOT}" -maxdepth 5 -type d -name '*-hf' -print 2>/dev/null | sort)
FAIL_HITS=""
if [[ -d "${TRAIN_LOG_DIR}" ]]; then
  FAIL_HITS="$(grep -RHE 'Traceback|ERROR|RuntimeError|CUDA out of memory|Training failed' "${TRAIN_LOG_DIR}" || true)"
fi
...
python3 - "${MANIFEST}" "${FINAL_STATUS}" "${HF_JSON}" "${TRAIN_LOG_DIR}" "${DETACHED_OUT}" "${DETACHED_ERR}" <<'PY'
import json
import subprocess
import sys
...
metadata["completion_monitor"] = {
    "script": "documents/code/train/sst_omni_train/launchers/2026/05/20260525__monitor_train_tmsft_gttermwrap_exact_ja_taurus8.sh",
...
if command -v "${HOME}/bin/codex-notify" >/dev/null 2>&1; then
  "${HOME}/bin/codex-notify" --delay 8 --detach --workspace "${ROOT_DIR}" \
    "JA TM-SFT exact term-wrap training finished: ${FINAL_STATUS}${WANDB_RUN_ID:+, run ${WANDB_RUN_ID}}"
fi
