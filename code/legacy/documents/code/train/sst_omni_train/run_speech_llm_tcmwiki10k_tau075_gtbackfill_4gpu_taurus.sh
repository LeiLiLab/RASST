#!/usr/bin/env bash
#SBATCH --job-name=slm_tcmw100kgt
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_slm_tcmw100kgt_taurus.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_slm_tcmw100kgt_taurus.err
...
NOTES_FILE="${ROOT_DIR}/documents/code/train/sst_omni_train/notes_speech_llm_tcmwiki10k_tau075_gtbackfill.md"

DATASET_PATH="/mnt/gemini/data1/jiaxuanluo/tcm_wiki100k_gt_zh_tau075_termmap/train_s_zh_v4_ner_baseline_aligned_rate1.0_k20_tcmwiki100kgt_tau075_gtbackfill.jsonl"
VAL_DATASET="/mnt/gemini/data1/jiaxuanluo/dev_s_zh_v4_ner_baseline_aligned_freq_k20_final.jsonl"
SAVE_BASE="/mnt/gemini/data/jiaxuanluo/speech_llm_tcm_filtered_wiki100kgt_tau075"
TRAIN_LOG_DIR="${ROOT_DIR}/documents/logs/speech_llm_tcmwiki100kgt_tau075_taurus"
...
# Taurus currently has long-running external jobs on GPU1/GPU4. Slurm has
# handed out busy device ids before, so allow an explicit clean-GPU override.
HOST_GPU_DEVICES_OVERRIDE="${HOST_GPU_DEVICES_OVERRIDE:-0,2,3,5}"
ALLOCATED_GPUS="${HOST_GPU_DEVICES_OVERRIDE:-${CUDA_VISIBLE_DEVICES:-0,1,2,3}}"
IFS=',' read -r -a GPU_ARR <<< "${ALLOCATED_GPUS}"
if (( ${#GPU_ARR[@]} != 4 )); then
  echo "[ERROR] This launcher expects exactly 4 allocated GPUs; got CUDA_VISIBLE_DEVICES=${ALLOCATED_GPUS}" >&2
  exit 2
fi
...
WANDB_EXP_PREFIX="speech-llm-tcmwiki100kgt-tau075-taurus" \
