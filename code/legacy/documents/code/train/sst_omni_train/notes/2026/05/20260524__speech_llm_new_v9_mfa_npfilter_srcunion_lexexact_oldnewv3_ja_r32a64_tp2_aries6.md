...
The previous de/ja New V9 line was rejected because GT was derived from term-map/fuzzy matches. The rebuilt ja dataset uses MFA source matching plus old-new_v3 noun/entity candidate gating, exact target-span support, old-new_v3 TCM filler maps, no-GT-zero, and assistant-side `<term>` tags.
...
- Runtime: aries 6 GPUs, TP=2, EP=2, sequence parallel, one epoch.
- GPU set: physical devices `0,1,2,5,6,7`.
