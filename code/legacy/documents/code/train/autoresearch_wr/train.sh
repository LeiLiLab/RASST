...
#SBATCH --gres=gpu:2
#SBATCH --time=4:00:00
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/autoresearch/%j_ar_wr.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/autoresearch/%j_ar_wr.err
...
# Let Slurm assign free GPUs via --gres=gpu:2; do NOT hardcode CUDA_VISIBLE_DEVICES
NUM_GPUS=2
...
SAVE_DIR="/mnt/data/jiaxuanluo/autoresearch_wr"
RESUME_PATH=""
...
# Training — greedy sweep: 1 epoch with time_budget as safety limit
PER_GPU_BATCH=512
...
# Eval & checkpointing — more frequent eval for short autoresearch runs
SAVE_STEPS=9999
...
CLEAN_RATIO_TAG=$(echo "${CLEAN_RATIO}" | sed 's/\.//g; s/-/n/')
VERSION="ar_wr${WIKI_RANK_K}k_cr${CLEAN_RATIO_TAG}"
SAVE_NAME="q3rag_${MODE_NAME}_bs${BS_ABBR}_t=${TEMPERATURE}_${VERSION}"
SAVE_PATH="${SAVE_DIR}/${SAVE_NAME}.pt"
WANDB_EXP_NAME="${SAVE_NAME}"

echo "[AUTORESEARCH] WIKI_RANK=${WIKI_RANK} CLEAN_RATIO=${CLEAN_RATIO} TIME_BUDGET=${TIME_BUDGET}s"
echo "[AUTORESEARCH] Save: ${SAVE_PATH}"
echo "[AUTORESEARCH] Batch: ${BATCH_SIZE} (${NUM_GPUS} GPUs * ${PER_GPU_BATCH})"
echo "[AUTORESEARCH] LR=${LR} TEMPERATURE=${TEMPERATURE}"
...
if [ "${TIME_BUDGET}" -gt 0 ]; then
    OPTS="${OPTS} --time_budget ${TIME_BUDGET}"
fi
if [ "${WARMUP_STEPS}" -ge 0 ]; then
    OPTS="${OPTS} --warmup_steps ${WARMUP_STEPS}"
fi
CLEAN_RATIO_IS_SET=$(echo "${CLEAN_RATIO} >= 0" | bc -l)
if [ "${CLEAN_RATIO_IS_SET}" -eq 1 ]; then
    OPTS="${OPTS} --clean_ratio ${CLEAN_RATIO}"
fi
...
echo "[AUTORESEARCH] Training completed at $(date)"
