...
The previous serial main-result `de/lm=3` readout regressed because the serial
agent duplicated the system prompt while trimming vLLM prompt history. After the
agent fix, `de/lm=3` should run with exactly one system prompt per vLLM request.
...
- Rerun only tagged ACL raw `de/lm=3`.
