#!/usr/bin/env bash
#SBATCH --job-name=acl_v3rand_tau
...
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%j_acl_v3rand_tau.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%j_acl_v3rand_tau.err
...
case "${TAU}" in
  0|0.0) TAU_TAG="tau0"; TAU_VALUE="0.0"; NOTES_FILE="${ROOT_DIR}/documents/code/simuleval/notes_new_v3_random_r32a64_onepaper_tau0.md" ;;
  0.75) TAU_TAG="tau075"; TAU_VALUE="0.75"; NOTES_FILE="${ROOT_DIR}/documents/code/simuleval/notes_new_v3_random_r32a64_onepaper_tau075.md" ;;
  *) echo "[ERROR] Unsupported tau: ${TAU}" >&2; exit 2 ;;
esac

HF_MODEL="/mnt/gemini/data2/jiaxuanluo/speech_llm_tcmw100kgt_tau075_new_v3_random_r32a64_aries8/keep1.0_r32/v1-20260508-123645-hf"
...
OUTPUT_BASE="/mnt/gemini/data2/jiaxuanluo/acl_onepaper_lm1_raw1k10k_timeline_fullwin_${TAU_TAG}_new_v3_random_r32a64_slm"
DENSITY_TAG="aclone_v3rand_r32a64_timeline_fullwin_${TAU_TAG}"
TRAINED_FROM_RUN="y59wfnmp"
...
log_one_glossary() {
  ...
  python3 "${WANDB_LOGGER}" \
    ...
    --run-name "v3rand_r32a64__${TARGET_PAPER}__lm${TARGET_LM}__${TAU_TAG}__${run_suffix}" \
    ...
    --extra-tags "variant:v3rand_r32a64" "compute:taurus3" "tau:${TAU_TAG}" "paper:${TARGET_PAPER}" "glossary:${run_suffix}" \
    --density "${DENSITY_TAG}" \
    --rag-top-k "${RAG_TOP_K}" \
    ...
    --model-name "${HF_MODEL}" \
    --verdict "Logged one-paper v3_random rank32/alpha64 SimulEval for ${TARGET_PAPER}, lm=${TARGET_LM}, tau=${TAU_VALUE}, glossary=${run_suffix}."
}
...
echo "[ALL DONE] v3_random one-paper eval tau=${TAU_VALUE}"
