...
# Tagged ACL de raw readout for New V10 sample50 no-GT-retrieved term-map SLM.
# Same-lm batch: each latency multiplier runs the five ACL talks in one vLLM
# process and occupies one 2-GPU tensor-parallel pair.
...
RUN_STAMP="${RUN_STAMP:-$(date -u +%Y%m%dT%H%M%S)_tagacl_newv10_s50_de_batch}"

MODEL_LABEL="${MODEL_LABEL:-new_v10_s50_no_gt_retrieved_oldnewv3_de_r32a64_hn1024_tau079_same_lm_batch_v1}"
MODEL_NAME="${MODEL_NAME:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_new_v10_no_gt_retrieved_termmap_sample50_de_r32a64_tp2_taurus8/keep1.0_r32/v0-20260525-083507-hf}"
...
NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260525__tagged_acl_new_v10_sample50_hn1024_tau079_raw_de_lm1to4_taurus.md}"
OUT_ROOT="${OUT_ROOT:-/mnt/gemini/data1/jiaxuanluo/tagged_acl_same_lm_batch_v1_new_v10_s50_hn1024_tau079_raw_de_lm1to4_${RUN_STAMP}}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUT_ROOT}/${MODEL_LABEL}}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_same_lm_batch_v1_new_v10_s50_hn1024_tau079_raw_de_lm1to4_${RUN_STAMP}}"
INDEX_CACHE_DIR="${INDEX_CACHE_DIR:-/mnt/gemini/data1/jiaxuanluo/maxsim_index_cache/batched_vllm}"
EVAL_TMPDIR_ROOT="${EVAL_TMPDIR_ROOT:-/tmp/jx_bvde_v10s50}"
...
for idx in "${!lm_values[@]}"; do
  ...
  (
    ...
    DENSITY_TAG_OVERRIDE="tagacl_bv1_newv10_s50_hn1024_tau079" \
    GLOSSARY_TAG_OVERRIDE="acl6060_tagged_gt_raw_min_norm2" \
    RAG_MODEL_PATH_OVERRIDE="${HN1024_CKPT}" \
    RAG_SCORE_THRESHOLD_OVERRIDE=0.79 \
    ...
    WANDB_RUN_PREFIX_OVERRIDE="newv10s50_hn1024_t079" \
    WANDB_EXPERIMENT_FAMILY_OVERRIDE="tagged_acl_newv10_s50_hn1024_tau079" \
    WANDB_VARIANT_PREFIX_OVERRIDE="newv10s50_hn1024_t079" \
...
python - "${OUTPUT_BASE}" "${OUT_ROOT}" ${LMS} <<'PY'
import csv
import sys
...
fields = [
    "lang", "lm", "glossary", "BLEU", "TERM_ACC", "TERM_CORRECT",
    "TERM_TOTAL", "REAL_TERM_ADOPT", "TERM_FCR", "StreamLAAL",
    "StreamLAAL_CA", "eval_results",
]
...
lines = [
    "# Tagged ACL de raw: New V10 sample50 + HN1024 tau=0.79, same-lm batch v1",
    "",
...
lines += ["", f"Summary TSV: `{tsv}`", f"Output root: `{out_root}`"]
