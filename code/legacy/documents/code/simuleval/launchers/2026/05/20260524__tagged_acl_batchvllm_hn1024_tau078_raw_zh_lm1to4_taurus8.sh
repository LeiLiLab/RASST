...
RUN_TAG="20260524T1109_tagacl_batchvllm_hn1024_tau078_raw_zh_taurus8"
MODEL_LABEL="new_v9_termtag_delay_oldnewv3_r32a64_hn1024_tau078_batchvllm"
...
OUT_ROOT="/mnt/gemini/data1/jiaxuanluo/tagged_acl_batchvllm_hn1024_tau078_raw_zh_lm1to4_${RUN_TAG}"
OUTPUT_BASE="${OUT_ROOT}/${MODEL_LABEL}"
LOG_ROOT="/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_batchvllm_hn1024_tau078_raw_zh_lm1to4_${RUN_TAG}"
NOTES_FILE="${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260524__tagged_acl_batchvllm_hn1024_tau078_raw_zh_lm1to4_taurus8.md"
...
for p in \
  ...
  "${NOTES_FILE}"; do
  if [[ ! -s "${p}" ]]; then
    echo "[ERROR] missing/empty required path: ${p}" >&2
    exit 3
  fi
done
...
[[ "${shards}" == "15" ]] || {
  echo "[ERROR] expected 15 model shards, found ${shards}: ${MODEL_NAME}" >&2
  exit 3
}

{
  ...
  echo "tau=0.78"
  echo "vllm_tp_size=8"
  echo "max_num_seqs=20"
} | tee "${OUT_ROOT}/run_meta.txt"

RUN_TAG_OVERRIDE="${RUN_TAG}" \
LANG_CODE_OVERRIDE="zh" \
LMS_OVERRIDE="1 2 3 4" \
CUDA_VISIBLE_DEVICES_PHYSICAL_OVERRIDE="${CUDA_VISIBLE_DEVICES_PHYSICAL_OVERRIDE:-0,1,2,3,4,5,6,7}" \
VLLM_TP_SIZE_OVERRIDE=8 \
MAX_NUM_SEQS_OVERRIDE=20 \
SCHEDULER_BATCH_SIZE_OVERRIDE=20 \
GPU_MEMORY_UTILIZATION_OVERRIDE="${GPU_MEMORY_UTILIZATION_OVERRIDE:-0.86}" \
...
DENSITY_TAG_OVERRIDE="tagacl_batchvllm_hn1024_tau078" \
...
RAG_DEVICE_OVERRIDE="${RAG_DEVICE_OVERRIDE:-cuda:7}" \
INDEX_BUILD_DEVICE_OVERRIDE="${INDEX_BUILD_DEVICE_OVERRIDE:-cuda:7}" \
...
WANDB_RUN_PREFIX_OVERRIDE="batchvllm_hn1024_tau078" \
WANDB_EXPERIMENT_FAMILY_OVERRIDE="tagged_acl_batchvllm_hn1024_tau078" \
WANDB_VARIANT_PREFIX_OVERRIDE="batchvllm_hn1024_tau078" \
WANDB_COMPUTE_TAG_OVERRIDE="compute:taurus8_batchvllm" \
NOTES_FILE_OVERRIDE="${NOTES_FILE}" \
LOG_ROOT_OVERRIDE="${LOG_ROOT}" \
EVAL_TMPDIR_OVERRIDE="${EVAL_TMPDIR_OVERRIDE:-/tmp/jx_bvzh8}" \
