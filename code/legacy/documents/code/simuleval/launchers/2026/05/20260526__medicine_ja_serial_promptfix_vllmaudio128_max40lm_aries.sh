...
RUN_STAMP="${RUN_STAMP:-20260526T000000_medicine_ja_serial_promptfix_vllmaudio128_max40lm_aries}"
OUT_ROOT="${OUT_ROOT:-/mnt/gemini/data1/jiaxuanluo/medicine_ja_serial_promptfix_vllmaudio128_max40lm_${RUN_STAMP}}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/medicine_ja_serial_promptfix_vllmaudio128_max40lm_${RUN_STAMP}}"
PID_FILE="${PID_FILE:-${LOG_ROOT}/launcher.pid}"

NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260526__medicine_ja_lm12_serial_promptfix_vllmaudio128_aries.md}"
...
MODEL_NAME="${MODEL_NAME:-/mnt/taurus/data1/jiaxuanluo/slm_local_cache/ja_tagged_acl_20260525/cap16_denoise_ttag/v2-20260525-235251-hf}"
...
MED_INPUTS="${MED_INPUTS:-/mnt/gemini/data1/jiaxuanluo/medicine_hardraw_ja_new_promptfix_vllmaudio128_lm12_20260526T043330_aries/ja/__medicine_inputs__/lists}"
HARD_RAW_GLOSSARY="${HARD_RAW_GLOSSARY:-${MED_INPUTS}/hard_medicine_raw__medicine5.json}"
INDEX_CACHE_DIR="${INDEX_CACHE_DIR:-/mnt/gemini/data1/jiaxuanluo/maxsim_index_cache/medicine_ja_serial_promptfix_vllmaudio128_max40lm}"

SRC_LIST="${SRC_LIST:-${MED_INPUTS}/medicine.source__medicine5_hardraw.txt}"
TGT_LIST="${TGT_LIST:-${MED_INPUTS}/medicine.target.ja__medicine5_hardraw.txt}"
SOURCE_TEXT="${SOURCE_TEXT:-${MED_INPUTS}/medicine.source_text.en__medicine5_hardraw.txt}"
REF_FILE="${REF_FILE:-${MED_INPUTS}/medicine.ref.ja__medicine5_hardraw.txt}"
...
EVAL_TMPDIR="${EVAL_TMPDIR:-/tmp/jxmjs${LM}p128}"
DENSITY_TAG="${DENSITY_TAG:-medhard_ja_serial_promptfix_vllmaudio128_max40lm}"
...
validate_runtime_prompts() {
  python3 - "${OUT_ROOT}" "${VLLM_LIMIT_AUDIO}" <<'PY'
import json
import sys
...
limit = sys.argv[2]
max_audio = None if limit == "auto" else int(limit)
...
for line in runtime_logs[0].open("r", encoding="utf-8"):
    ...
    if max_audio is not None and audio_count > max_audio:
        raise SystemExit(
...
record = {
    ...
    "note": "ja cap16-denoise tagged-term SLM; medicine hardraw runtime/eval glossary; HN1024 tau=0.78; VLLM_LIMIT_AUDIO=128; empty maps omitted; system prompt duplication fixed",
}
out = summary_dir / f"summary_medicine_hardraw_ja_lm{lm}_serial_promptfix_vllmaudio128.tsv"
with out.open("w", encoding="utf-8", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=fields, delimiter="\t")
    writer.writeheader()
    writer.writerow(record)
print(
    f"RESULT medicine ja lm{lm} BLEU={float(record['BLEU']):.4f} "
...
main() {
  ...
  df -h /mnt/gemini/data1 /mnt/taurus/data1 /dev/shm | tee "${LOG_ROOT}/df_prelaunch.txt" || true
  ...
  export WANDB_DIR="/mnt/gemini/data1/jiaxuanluo/wandb_runs"
  export WANDB_CACHE_DIR="/mnt/gemini/data1/jiaxuanluo/wandb_cache"
  export WANDB_DATA_DIR="/mnt/gemini/data1/jiaxuanluo/wandb_data"
  export WANDB_ARTIFACT_DIR="/mnt/gemini/data1/jiaxuanluo/wandb_artifacts"
