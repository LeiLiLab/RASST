...
# Aries all-lm same-lm batch readout for German tagged ACL raw RASST.
...
RUN_STAMP="${RUN_STAMP:-$(date -u +%Y%m%dT%H%M%S)_tagacl_tmsftgtwrap_de_hn1024_tau078_lm1to4_aries}"
LMS="${LMS:-1 2 3 4}"
GPU_PAIRS_CSV="${GPU_PAIRS_CSV:-0,1;2,3;4,5;6,7}"

MODEL_LABEL="${MODEL_LABEL:-tmsft_gttermwrap_exact_de_r32a32_ep4_hn1024_tau078_batch_max80}"
DENSITY_TAG="${DENSITY_TAG:-tagacl_bv1_tmsft_gtwrap_hn1024_tau078}"
EMPTY_TERM_MAP_POLICY="${EMPTY_TERM_MAP_POLICY:-none_block}"
MODEL_NAME="${MODEL_NAME:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_tmsft_gttermwrap_exact_de_r32a32_ep4_aries8/keep1.0_r32/v0-20260525-083705-hf}"
...
NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260525__tagged_acl_tmsft_gttermwrap_exact_de_hn1024_tau078_batch_aries_lm1to4.md}"
INDEX_CACHE_DIR="${INDEX_CACHE_DIR:-/mnt/gemini/data1/jiaxuanluo/maxsim_index_cache/batched_vllm}"

OUT_ROOT="${OUT_ROOT:-/mnt/gemini/data1/jiaxuanluo/tagged_acl_tmsft_gttermwrap_exact_de_hn1024_tau078_batch_${RUN_STAMP}}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUT_ROOT}/${MODEL_LABEL}}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_tmsft_gttermwrap_exact_de_hn1024_tau078_batch_${RUN_STAMP}}"
EVAL_TMPDIR_ROOT="${EVAL_TMPDIR_ROOT:-/tmp/jx_detw_a}"
...
validate_wandb_tags() {
  local tag lm pair
  local tags=(
    "family:tagged_acl_tmsft_gtwrap_de_hn1024_tau078"
    "task:eval"
    "data:tagacl_raw_de"
  )
  ...
  for lm in ${LMS}; do
    tags+=("variant:tmsftgtwrap_t078_de_raw_lm${lm}")
  done
  for tag in "${tags[@]}"; do
...
for lm in lms:
    ...
    rows.extend(data)
fields = list(rows[0])
out = summary_dir / "summary_de_lm1to4.tsv"
with out.open("w", encoding="utf-8", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=fields, delimiter="\t")
    writer.writeheader()
...
run_one_lm() {
  ...
  WANDB_EXPERIMENT_FAMILY_OVERRIDE="tagged_acl_tmsft_gtwrap_de_hn1024_tau078" \
