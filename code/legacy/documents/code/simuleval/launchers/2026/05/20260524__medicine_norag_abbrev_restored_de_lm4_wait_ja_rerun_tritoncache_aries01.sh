...
OUTPUT_BASE="${OUTPUT_BASE_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/medicine_norag_baseline_abbrev_restored_batched_20260524_de_rerun_tritoncache_lm4_aries01}"
CACHE_BASE="${CACHE_BASE_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/cache/medicine_norag_de_rerun_tritoncache_20260524T0345/lm4_aries01}"
RUN_STAMP="${RUN_STAMP_OVERRIDE:-20260524T0345}"
WAIT_SECONDS="${WAIT_SECONDS_OVERRIDE:-60}"
MAX_POLLS="${MAX_POLLS_OVERRIDE:-720}"

OUT_LOG="${LOG_ROOT}/${RUN_STAMP}_medicine_norag_abbrev_restored_de_rerun_tritoncache_lm4_aries01.out"
ERR_LOG="${LOG_ROOT}/${RUN_STAMP}_medicine_norag_abbrev_restored_de_rerun_tritoncache_lm4_aries01.err"
PID_FILE="${LOG_ROOT}/${RUN_STAMP}_medicine_norag_abbrev_restored_de_rerun_tritoncache_lm4_aries01.wrapper.pid"

mkdir -p "${LOG_ROOT}" "${CACHE_BASE}/tmp" "${CACHE_BASE}/xdg" "${CACHE_BASE}/triton" "${CACHE_BASE}/torchinductor" "${CACHE_BASE}/cuda"
...
ssh aries "setsid bash -lc 'cd ${ROOT_DIR} && export TMPDIR=${CACHE_BASE}/tmp XDG_CACHE_HOME=${CACHE_BASE}/xdg TRITON_CACHE_DIR=${CACHE_BASE}/triton TORCHINDUCTOR_CACHE_DIR=${CACHE_BASE}/torchinductor CUDA_CACHE_PATH=${CACHE_BASE}/cuda && MASTER_PORT=20344 LANGS_OVERRIDE=de TARGET_LMS_OVERRIDE=4 CUDA_VISIBLE_DEVICES_PHYSICAL_OVERRIDE_CSV=0:1 OUTPUT_BASE_OVERRIDE=${OUTPUT_BASE} FORCE_RERUN_OVERRIDE=1 bash ${MEDICINE_LAUNCHER}' > ${OUT_LOG} 2> ${ERR_LOG} < /dev/null & echo \$! > ${PID_FILE}; cat ${PID_FILE}"
