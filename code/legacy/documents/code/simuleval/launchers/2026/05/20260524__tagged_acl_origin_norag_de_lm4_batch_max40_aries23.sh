...
# En-De tagged ACL raw lm=4 InfiniSST/no-RAG batch verification.

ROOT_DIR="${ROOT_DIR_OVERRIDE:-/mnt/taurus/home/jiaxuanluo/InfiniSST}"
cd "${ROOT_DIR}"

RUN_STAMP="${RUN_STAMP:-20260524T2338_tagacl_origin_norag_de_lm4_batch_max40_aries23}"
GPU_PAIR="${GPU_PAIR:-2,3}"
MAX_NEW_TOKENS_VALUE=40
...
NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260524__tagged_acl_origin_norag_de_lm4_batch_max40_aries23.md}"

OUT_ROOT="${OUT_ROOT:-/mnt/gemini/data1/jiaxuanluo/tagged_acl_origin_norag_de_lm4_batch_max40_aries23_${RUN_STAMP}}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUT_ROOT}/origin_norag_de_lm4_batch_max40}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_origin_norag_de_lm4_batch_max40_aries23_${RUN_STAMP}}"
CACHE_BASE="${CACHE_BASE:-/mnt/gemini/data1/jiaxuanluo/cache/tagged_acl_origin_norag_de_lm4_batch_max40_aries23_${RUN_STAMP}}"
EVAL_TMPDIR="${EVAL_TMPDIR:-/tmp/jx_tacl_d4n40}"
INPUT_WORK_DIR="${OUT_ROOT}/__inputs__"
SRC_LIST_PORTABLE="${INPUT_WORK_DIR}/source.portable.list"
DENSITY_TAG="tagacl_origin_norag_batch_max40"
...
mkdir -p "${XDG_CACHE_HOME}" "${TRITON_CACHE_DIR}" "${TORCHINDUCTOR_CACHE_DIR}" \
  ...
  "${NUMBA_CACHE_DIR}" "${WANDB_CACHE_DIR}" "${WANDB_DATA_DIR}" "${WANDB_CONFIG_DIR}"
...
RUN_TAG_OVERRIDE="${RUN_STAMP}_de_lm4" \
...
WANDB_VARIANT_PREFIX_OVERRIDE="origin_norag_de_lm4_batch_max40" \
WANDB_COMPUTE_TAG_OVERRIDE="compute:aries23" \
...
row = rows[0]
summary = output_base / "__summary__" / "summary_de_lm4.tsv"
summary.parent.mkdir(parents=True, exist_ok=True)
fields = [
    "method_key", "lang", "lm", "max_new_tokens", "BLEU", "StreamLAAL",
    "StreamLAAL_CA", "TERM_ACC", "TERM_CORRECT", "TERM_TOTAL",
    "REAL_TERM_ADOPT", "TERM_FCR", "eval_results", "instances_log",
]
...
echo "[ALL DONE] ${OUTPUT_BASE}/__summary__/summary_de_lm4.tsv"
