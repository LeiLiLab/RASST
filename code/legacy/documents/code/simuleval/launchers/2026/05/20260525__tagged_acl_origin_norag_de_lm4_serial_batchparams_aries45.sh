...
RUN_STAMP="${RUN_STAMP:-20260525T132816_tagacl_origin_norag_de_lm4_serial_batchparams_aries45}"
...
NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260525__tagged_acl_origin_norag_de_lm4_serial_batchparams_aries45.md}"
...
OUT_ROOT="${OUT_ROOT:-/mnt/gemini/data1/jiaxuanluo/tagged_acl_origin_norag_de_lm4_serial_batchparams_aries45_${RUN_STAMP}}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_origin_norag_de_lm4_serial_batchparams_aries45_${RUN_STAMP}}"
...
EVAL_TMPDIR="${EVAL_TMPDIR:-/tmp/jx_tacl_de4s80}"
...
run_generation() {
  echo "[RUN] serial no-RAG baseline generation: de lm4 max80 -> ${SETTING_DIR}"
...
write_summary() {
  "${PYTHON_BIN}" - "${SETTING_DIR}/eval_results.tsv" "${SUMMARY_DIR}/tagged_acl_origin_norag_de_lm4_serial_batchparams_aries45.tsv" "${SETTING_DIR}" "${OUT_ROOT}/runtime_seconds.txt" <<'PY'
import csv
import sys
...
strip_inst = setting_dir / "instances.strip_term.log"
inst_rows = sum(1 for line in inst.open("r", encoding="utf-8", errors="replace") if line.strip())
strip_rows = sum(1 for line in strip_inst.open("r", encoding="utf-8", errors="replace") if line.strip()) if strip_inst.exists() else ""
row = rows[0]
fields = [
    "method_key", "lang", "lm", "max_new_tokens", "protocol", "BLEU",
    "StreamLAAL", "StreamLAAL_CA", "TERM_ACC", "TERM_CORRECT",
    "TERM_TOTAL", "instances_log_rows", "instances_strip_term_log_rows",
    "runtime_seconds", "eval_results", "instances_log", "instances_strip_term_log",
]
record = {
    "method_key": "origin_norag_serial_infinisst_batchparams",
    "lang": row.get("lang_code", "de"),
    "lm": "4",
    "max_new_tokens": "80",
    "protocol": "serial_simuleval_agent_v4_batch_lm4_params",
    "BLEU": row.get("BLEU", ""),
    ...
    "instances_strip_term_log_rows": str(strip_rows),
    ...
    "instances_strip_term_log": str(strip_inst) if strip_inst.exists() else "",
}
...
main() {
  ...
  echo "[ALL DONE] ${SUMMARY_DIR}/tagged_acl_origin_norag_de_lm4_serial_batchparams_aries45.tsv"
}
