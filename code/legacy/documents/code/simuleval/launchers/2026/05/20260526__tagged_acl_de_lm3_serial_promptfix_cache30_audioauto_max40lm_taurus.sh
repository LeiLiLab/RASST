...
RUN_STAMP="${RUN_STAMP:-20260526T000000_tagged_acl_de_serial_promptfix_cache80s60s_audioauto_max40lm_taurus}"
OUT_ROOT="${OUT_ROOT:-/mnt/data1/jiaxuanluo/tagged_acl_de_serial_promptfix_cache80s60s_audioauto_max40lm_${RUN_STAMP}}"
LOG_ROOT="${LOG_ROOT:-/mnt/data1/jiaxuanluo/logs/tagged_acl_de_serial_promptfix_cache80s60s_audioauto_max40lm_${RUN_STAMP}}"
PID_FILE="${PID_FILE:-${LOG_ROOT}/launcher.pid}"

NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260526__tagged_acl_de_serial_promptfix_cache80s60s_audioauto_max40lm_taurus.md}"
...
INPUT_ROOT="${INPUT_ROOT:-/mnt/data1/jiaxuanluo/main_result_rasst_serial_de_ja_cache30_max40lm_20260525T215831_main_result_rasst_serial_de_ja_cache30_max40lm_taurus_retry1/__inputs__/acl_de}"
INDEX_CACHE_DIR="${INDEX_CACHE_DIR:-/mnt/data1/jiaxuanluo/maxsim_index_cache/tagged_acl_de_serial_promptfix_cache80s60s_audioauto_max40lm}"

GPU_PAIR="${GPU_PAIR:-0,1}"
LM="${LM:-3}"
default_cache_chunks_for_lm() {
  python3 - "$1" <<'PY'
import sys
lm = float(sys.argv[1])
segment_sec = 0.96 * lm
print(max(1, int(80.0 / segment_sec)), max(1, int(60.0 / segment_sec)))
PY
}
read -r DEFAULT_MAX_CACHE_CHUNKS DEFAULT_KEEP_CACHE_CHUNKS < <(default_cache_chunks_for_lm "${LM}")
MAX_NEW_TOKENS="${MAX_NEW_TOKENS:-$((LM * 40))}"
MAX_CACHE_CHUNKS="${MAX_CACHE_CHUNKS:-${DEFAULT_MAX_CACHE_CHUNKS}}"
KEEP_CACHE_CHUNKS="${KEEP_CACHE_CHUNKS:-${DEFAULT_KEEP_CACHE_CHUNKS}}"
...
EVAL_TMPDIR="${EVAL_TMPDIR:-/tmp/jxde3pf}"
...
record = {
    ...
    "note": "de cap16-denoise tagged-term SLM; HN1024 tau=0.78; empty maps omitted; system prompt duplication fixed; cache chunks default to floor(80s/segment_sec)/floor(60s/segment_sec)",
}
out = summary_dir / f"summary_acl_tagged_raw_de_lm{lm}_promptfix.tsv"
with out.open("w", encoding="utf-8", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=fields, delimiter="\t")
    writer.writeheader()
    writer.writerow(record)
print(
    f"RESULT de lm{lm} BLEU={float(record['BLEU']):.4f} "
...
main() {
...

  {
    ...
    echo "cache_policy=max80s_keep60s_over_segment"
    echo "default_cache_chunks=${DEFAULT_MAX_CACHE_CHUNKS}/${DEFAULT_KEEP_CACHE_CHUNKS}"
  ...
  DENSITY_TAG="tagacl_de_lm${LM}_serial_promptfix_cache80s60s_audioauto_max40lm" \
