...
OUT_PREFIX="${OUT_PREFIX_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/medicine_norag_baseline_abbrev_restored_batched_20260524_de_rerun_tritoncache}"
...
export TMPDIR="${TMPDIR_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo/cache/medicine_de_rerun_tritoncache_post_eval_tmp}"
mkdir -p "${TMPDIR}"
...
run_post_one() {
  ...
  local tag="$2"
  local cs="$3"
  local out="${OUT_PREFIX}_lm${lm}_${tag}"
  ...
  for _ in $(seq 1 "${MAX_POLLS}"); do
    ...
    echo "[${ts}] wait de lm=${lm} tag=${tag} timing instances=${inst}"
    ...
    if [[ -s "${out}/timing.tsv" ]] && awk -F '\t' 'NR>1 && $5 == "failed" {found=1} END {exit found?0:1}' "${out}/timing.tsv"; then
      echo "[ERROR] de lm=${lm} failed; not running post-eval" >&2
      return 2
    fi
    sleep "${WAIT_SECONDS}"
  done
...
run_post_one 1 aries23 0.96 &
run_post_one 2 aries45 1.92 &
run_post_one 3 aries67 2.88 &
run_post_one 4 aries01 3.84 &
wait
