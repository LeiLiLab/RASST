...
# Corrected old new_v3 r64/a128 quick eval.
# The previous paper110_extracted run used extracted glossary for retrieval but
# raw tagged glossary for metrics.  This runner uses follows-kind metric
# denominator so extracted uses extracted for both retrieval and metric scoring.
...
OUT_ROOT="${OUT_ROOT:-/mnt/aries/data7/jiaxuanluo/slm/tagged_acl_old_newv3_r64_rank_metricfix_zh_lm2_${RUN_STAMP}}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_old_newv3_r64_rank_metricfix_${RUN_STAMP}}"
INDEX_CACHE_DIR="${INDEX_CACHE_DIR:-/mnt/aries/data7/jiaxuanluo/slm/maxsim_index_cache}"
NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260522__tagged_acl_old_newv3_r64_rank_metricfix_zh_lm2.md}"

check_hf() {
  local model_dir="$1"
  if [[ ! -f "${model_dir}/config.json" || ! -f "${model_dir}/generation_config.json" ]] \
     || ! compgen -G "${model_dir}/*.safetensors" >/dev/null; then
    echo "[ERROR] HF checkpoint incomplete: ${model_dir}" >&2
    exit 3
  fi
}
...
run_one() {
  local variant="$1"
  local short_variant="$2"
  local model_dir="$3"
  local run_kind="$4"
  local run_granularity="$5"
  local glossary_kinds="$6"
  local papers="$7"
  ...
  echo "[RUN] variant=${variant} short=${short_variant} kind=${run_kind}"
  env \
    ROOT_DIR="${ROOT_DIR}" \
    RUN_STAMP="${RUN_STAMP}_${short_variant}_${run_kind}" \
    MODE=full \
    ...
    EVAL_GLOSSARY_FOLLOWS_KIND_OVERRIDE=1 \
    ...
    DENSITY_TAG_OVERRIDE="tagacl_${short_variant}_${run_kind}_tau073_metricfix" \
    WANDB_RUN_PREFIX_OVERRIDE="${short_variant}_${run_kind}_metricfix" \
    WANDB_EXPERIMENT_FAMILY_OVERRIDE="tagged_acl_old_newv3_r64_rank_metricfix" \
    WANDB_VARIANT_PREFIX_OVERRIDE="${short_variant}_${run_kind}_metricfix" \
...
mkdir -p "${OUT_ROOT}" "${LOG_ROOT}"
{
  ...
  echo "metric_policy=follows_kind"
  echo "out_root=${OUT_ROOT}"
  echo "log_root=${LOG_ROOT}"
} | tee "${OUT_ROOT}/run_meta.txt"

run_one "q159wce4_newv3_r64a128_full" "q159" "${Q159_MODEL}" "p110x" "per_paper" "extracted" "2022.acl-long.110"
run_one "rj1v1p7r_newv3_random_r64a128" "rj1" "${RJ1_MODEL}" "raw" "full_corpus" "raw" "all"
run_one "rj1v1p7r_newv3_random_r64a128" "rj1" "${RJ1_MODEL}" "p110x" "per_paper" "extracted" "2022.acl-long.110"

echo "[DONE] Corrected metricfix rank evals under ${OUT_ROOT}"
