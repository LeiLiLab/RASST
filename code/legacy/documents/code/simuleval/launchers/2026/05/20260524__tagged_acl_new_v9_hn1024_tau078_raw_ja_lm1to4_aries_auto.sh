...
# Tagged ACL main-result evaluation for Japanese.
...
# waiting for an idle 2-GPU pair on aries before each setting.
#
# Guardrail: this waits until the existing medicine ja lm4 waiter has
# submitted lm4, so tagged ACL does not race it for the first freed pair.
...
RUN_STAMP="${RUN_STAMP:-$(date -u +%Y%m%dT%H%M%S)_tagacl_newv9_hn1024_tau078_raw_ja}"

MODEL_LABEL="${MODEL_LABEL:-new_v9_termtag_delay_oldnewv3_r32a64_hn1024_tau078}"
MODEL_ROOT="${MODEL_ROOT:-/mnt/gemini/data1/jiaxuanluo/slm/speech_llm_new_v9_assistant_termtag_delay_clean_no_gt_zero_oldnewv3_ja_r32a64_tp2_taurus4/keep1.0_r32}"
EXPECTED_MODEL_DIR="${EXPECTED_MODEL_DIR:-${MODEL_ROOT}/v0-20260524-123624-hf}"
...
NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260524__tagged_acl_new_v9_hn1024_tau078_raw_ja_lm1to4_aries_auto.md}"
...
EVAL_TMPDIR_ROOT="${EVAL_TMPDIR_ROOT:-/tmp/jxl_tacl_ja}"
...
MEDICINE_LM4_LOG_ROOT="${MEDICINE_LM4_LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/medicine_hardraw_ja_hn1024_tau078_new_v9_lm12_5samples_20260524T0710_ja_hardraw_newv9_hn1024_tau078}"
MEDICINE_LM4_PID_FILE="${MEDICINE_LM4_PID_FILE:-${MEDICINE_LM4_LOG_ROOT}/submit_lm4.pid}"
MEDICINE_LM4_WAIT_POLL_SECS="${MEDICINE_LM4_WAIT_POLL_SECS:-120}"
...
check_host() {
  local host
  host="$(hostname -s)"
  [[ "${host}" == aries* ]] || fail "This launcher should run on aries; current host=${host}"
}
...
wait_medicine_lm4_submitted() {
  echo "[WAIT] waiting for medicine ja lm4 submission marker: ${MEDICINE_LM4_PID_FILE}"
  while true; do
    if [[ -s "${MEDICINE_LM4_PID_FILE}" ]]; then
      local pid
      pid="$(cat "${MEDICINE_LM4_PID_FILE}")"
      if [[ -n "${pid}" ]]; then
        echo "[READY] medicine ja lm4 has been submitted; pid=${pid}"
        return 0
      fi
    fi
    tail -5 "${MEDICINE_LM4_LOG_ROOT}/lm4_waiter.out" 2>/dev/null || true
    sleep "${MEDICINE_LM4_WAIT_POLL_SECS}"
  done
}
...
validate_tags_for_lm() {
  local lm="$1" pair="$2" compact_pair tag
  compact_pair="${pair//,/}"
  local tags=(
    "family:tagged_acl_new_v9_hn1024_tau078"
    "task:eval"
    "data:tagged_acl_strict_raw_ja"
    "variant:new_v9_hn1024_tau078_ja_raw_lm${lm}"
    "compute:aries_auto_gpu${compact_pair}"
  )
  for tag in "${tags[@]}"; do
...
setting_dir_for_lm() {
  local lm="$1"
  printf '%s/%s/ja/dtagacl_new_v9_hn1024_tau078_lm%s_k10_th0.78_gacl6060_tagged_gt_raw_min_norm2\n' \
...
run_one_lm() {
  ...
  RUN_STAMP="${RUN_STAMP}_${MODEL_LABEL}_ja_lm${lm}" \
  ROOT_DIR="${ROOT_DIR}" \
  MODE="full" \
  ...
  WANDB_COMPUTE_TAG_OVERRIDE="compute:aries_auto_gpu${compact_pair}" \
...
summarize_results() {
  local summary_tsv="${OUT_ROOT}/${MODEL_LABEL}/__summary__/summary_ja_raw_lm1to4.tsv"
  local summary_md="${OUT_ROOT}/${MODEL_LABEL}/__summary__/summary_ja_raw_lm1to4.md"
...
rows = []
for lm in lms:
    path = out_root / model_label / "ja" / f"dtagacl_new_v9_hn1024_tau078_lm{lm}_k10_th0.78_gacl6060_tagged_gt_raw_min_norm2" / "eval_results.tsv"
...
lines = [
    "# Tagged ACL ja raw: new_v9 + HN1024 tau=0.78",
    "",
...
main() {
  preflight
  wait_medicine_lm4_submitted
  mkdir -p "${OUT_ROOT}/${MODEL_LABEL}/__summary__" "${LOG_ROOT}" "$(dirname "${NOTES_FILE}")"
  touch "${NOTES_FILE}"
  {
    ...
    echo "medicine_lm4_pid_file=${MEDICINE_LM4_PID_FILE}"
  ...
  summarize_results
  echo "[ALL DONE] Tagged ACL New V9 HN1024 tau0.78 raw ja lm=${LMS}"
  echo "[SUMMARY] ${OUT_ROOT}/${MODEL_LABEL}/__summary__/summary_ja_raw_lm1to4.md"
}
