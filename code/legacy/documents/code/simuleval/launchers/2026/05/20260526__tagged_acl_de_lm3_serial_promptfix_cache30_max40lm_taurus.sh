...
RUN_STAMP="${RUN_STAMP:-20260526T001737_tagged_acl_de_lm3_serial_promptfix_cache30_max40lm_taurus}"
OUT_ROOT="${OUT_ROOT:-/mnt/data1/jiaxuanluo/tagged_acl_de_lm3_serial_promptfix_cache30_max40lm_${RUN_STAMP}}"
LOG_ROOT="${LOG_ROOT:-/mnt/data1/jiaxuanluo/logs/tagged_acl_de_lm3_serial_promptfix_cache30_max40lm_${RUN_STAMP}}"
PID_FILE="${PID_FILE:-${LOG_ROOT}/launcher.pid}"

NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260526__tagged_acl_de_lm3_serial_promptfix_cache30_max40lm_taurus.md}"
...
INDEX_CACHE_DIR="${INDEX_CACHE_DIR:-/mnt/data1/jiaxuanluo/maxsim_index_cache/tagged_acl_de_lm3_serial_promptfix_cache30_max40lm}"
...
for line in runtime_logs[0].open("r", encoding="utf-8"):
    ...
    if row.get("type") != "llm_input":
        continue
    total += 1
    counts[row.get("prompt", "").count("<|im_start|>system")] += 1
if total == 0:
...
print(f"[CHECK] runtime prompt system_count ok: {total} llm_input rows in {runtime_logs[0]}")
PY
}

write_summary() {
  python3 - "${OUT_ROOT}" "${MAX_NEW_TOKENS}" "${MAX_CACHE_CHUNKS}" "${KEEP_CACHE_CHUNKS}" <<'PY'
import csv
import json
import sys
from pathlib import Path
...
out_root = Path(sys.argv[1])
max_new, max_cache, keep_cache = sys.argv[2:5]
paths = sorted(out_root.glob("de/*_lm3_*/eval_results.tsv"))
if len(paths) != 1:
...
record = {
    ...
    "note": "de cap16-denoise tagged-term SLM; HN1024 tau=0.78; empty maps omitted; system prompt duplication fixed",
}
out = summary_dir / "summary_acl_tagged_raw_de_lm3_promptfix.tsv"
with out.open("w", encoding="utf-8", newline="") as f:
    writer = csv.DictWriter(f, fieldnames=fields, delimiter="\t")
    writer.writeheader()
    writer.writerow(record)
print(
    f"RESULT de lm3 BLEU={float(record['BLEU']):.4f} "
...
main() {
  ...
  DENSITY_TAG="tagacl_de_lm3_serial_promptfix_cache30_max40lm" \
