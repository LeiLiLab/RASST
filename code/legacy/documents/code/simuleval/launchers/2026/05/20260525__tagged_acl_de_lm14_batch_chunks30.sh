...
# Generic En-De tagged ACL raw same-LM batch readout for lm=1,4.
# Intended for cap16 / cap16_denoise comparisons with fixed cache chunks.
...
RUN_STAMP="${RUN_STAMP:-$(date -u +%Y%m%dT%H%M%S)_tagacl_de_lm14_batch_chunks30}"
LMS="${LMS:-1 4}"
...
OUT_ROOT="${OUT_ROOT:-/mnt/gemini/data1/jiaxuanluo/tagged_acl_de_lm14_batch_chunks30_${RUN_STAMP}}"
OUTPUT_BASE="${OUTPUT_BASE:-${OUT_ROOT}/${MODEL_LABEL}}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_de_lm14_batch_chunks30_${RUN_STAMP}}"
EVAL_TMPDIR_ROOT="${EVAL_TMPDIR_ROOT:-/tmp/jx_dlm14b}"
...
out = summary_dir / f"summary_de_{suffix}.tsv"
with out.open("w", encoding="utf-8", newline="") as f:
...
if [[ "${status}" != "0" ]]; then
  fail "At least one lm batch failed; see ${LOG_ROOT}"
fi
merge_summaries
date -u +%Y-%m-%dT%H:%M:%SZ > "${OUT_ROOT}/.success"
echo "[ALL DONE] ${SUMMARY_DIR}/summary_de_$(printf '%s\n' ${LMS} | awk '{printf "%slm%s", (NR==1?"":"_"), $1}').tsv"
