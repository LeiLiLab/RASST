...
# Tagged ACL zh tau=0.0 ablation:
# Speech LLM new_v9, HN1024 lh1b88kw retriever, raw glossary, lm=1..4.
# Runs sequentially on the only idle Aries GPU pair observed at planning time.
...
RUN_STAMP="${RUN_STAMP:-20260526T0210_tagacl_newv9_hn1024_tau000_raw_zh_lm1to4_aries01_seq}"

MODEL_LABEL="${MODEL_LABEL:-new_v9_termtag_delay_oldnewv3_r32a64_hn1024_tau000_readout}"
...
NOTES_FILE="${NOTES_FILE:-${ROOT_DIR}/documents/code/simuleval/notes/2026/05/20260526T0210__tagged_acl_new_v9_hn1024_tau000_raw_zh_lm1to4_aries01_seq.md}"

OUTPUT_STORAGE_ROOT="${OUTPUT_STORAGE_ROOT:-/mnt/taurus/data2/jiaxuanluo}"
OUT_ROOT="${OUT_ROOT:-${OUTPUT_STORAGE_ROOT}/tagged_acl_new_v9_hn1024_tau000_raw_zh_lm1to4_${RUN_STAMP}}"
LOG_ROOT="${LOG_ROOT:-${OUTPUT_STORAGE_ROOT}/logs/tagged_acl_new_v9_hn1024_tau000_raw_zh_lm1to4_${RUN_STAMP}}"
INDEX_CACHE_DIR="${INDEX_CACHE_DIR:-/mnt/gemini/data1/jiaxuanluo/maxsim_index_cache}"
EVAL_TMPDIR_OVERRIDE="${EVAL_TMPDIR_OVERRIDE:-/mnt/taurus/data2/jiaxuanluo/tmp/jx_tacl0}"

GPU_PAIRS_CSV="${GPU_PAIRS_CSV:-0,1}"
LMS="${LMS:-1 2 3 4}"
MIN_STORAGE_FREE_MB="${MIN_STORAGE_FREE_MB:-100000}"
MIN_TMP_FREE_MB="${MIN_TMP_FREE_MB:-5000}"
...
index_path_for_raw() {
  ...
  model_hash="$(printf '%s' "${HN1024_CKPT}" | sha1sum | awk '{print substr($1,1,10)}')"
  glossary_hash="$(printf '%s' "${RAW_GLOSSARY}" | sha1sum | awk '{print substr($1,1,10)}')"
  glossary_tag="$(basename "${RAW_GLOSSARY}" .json)"
...
validate_model_dir() {
  ...
  require_path "${EXPECTED_MODEL_DIR}/generation_config.json"
  require_path "${EXPECTED_MODEL_DIR}/model.safetensors.index.json"
  local shard_count
...
check_space() {
  local free_mb
  free_mb="$(df -Pm "${OUTPUT_STORAGE_ROOT}" | awk 'NR == 2 {print $4}')"
  [[ -n "${free_mb}" ]] || fail "Could not read free space for ${OUTPUT_STORAGE_ROOT}"
  (( free_mb >= MIN_STORAGE_FREE_MB )) || fail "${OUTPUT_STORAGE_ROOT} free space ${free_mb} MB < ${MIN_STORAGE_FREE_MB} MB"
  (( ${#EVAL_TMPDIR_OVERRIDE} <= 80 )) || fail "EVAL_TMPDIR_OVERRIDE is too long for vLLM IPC safety: ${EVAL_TMPDIR_OVERRIDE}"
  free_mb="$(df -Pm "$(dirname "${EVAL_TMPDIR_OVERRIDE}")" | awk 'NR == 2 {print $4}')"
  [[ -n "${free_mb}" ]] || fail "Could not read free space for $(dirname "${EVAL_TMPDIR_OVERRIDE}")"
  (( free_mb >= MIN_TMP_FREE_MB )) || fail "$(dirname "${EVAL_TMPDIR_OVERRIDE}") free space ${free_mb} MB < ${MIN_TMP_FREE_MB} MB"
}

check_gpu_idle() {
  command -v nvidia-smi >/dev/null || fail "nvidia-smi not found"
  local gpu_csv gpu line mem util
  gpu_csv="$(nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv,noheader,nounits)"
  IFS=',' read -r -a gpus <<< "${GPU_PAIRS_CSV}"
  for gpu in "${gpus[@]}"; do
    gpu="${gpu//[[:space:]]/}"
...
validate_tags() {
  local tags=(
    "family:tagged_acl_new_v9_hn1024_tau000"
    "task:eval"
    "data:tagged_acl_strict_raw_zh"
    "variant:new_v9_hn1024_tau000_zh_raw_lm1to4"
    "compute:aries_direct_gpu01_seq"
  )
  local tag
  for tag in "${tags[@]}"; do
...
mkdir -p \
  ...
  "${OUTPUT_STORAGE_ROOT}/wandb_runs" \
  "${OUTPUT_STORAGE_ROOT}/wandb_cache" \
  "${OUTPUT_STORAGE_ROOT}/wandb_data" \
  "${OUTPUT_STORAGE_ROOT}/wandb_artifacts"
...

{
  ...
  echo "output_storage_root=${OUTPUT_STORAGE_ROOT}"
  ...
  echo "rag_score_threshold=0.0"
  echo "rag_top_k=10"
  echo "rag_timeline_lookback_sec=1.92"
  echo "eval_tmpdir=${EVAL_TMPDIR_OVERRIDE}"
...
export WANDB_DIR="${WANDB_DIR:-${OUTPUT_STORAGE_ROOT}/wandb_runs}"
export WANDB_CACHE_DIR="${WANDB_CACHE_DIR:-${OUTPUT_STORAGE_ROOT}/wandb_cache}"
export WANDB_DATA_DIR="${WANDB_DATA_DIR:-${OUTPUT_STORAGE_ROOT}/wandb_data}"
export WANDB_ARTIFACT_DIR="${WANDB_ARTIFACT_DIR:-${OUTPUT_STORAGE_ROOT}/wandb_artifacts}"
...
DENSITY_TAG_OVERRIDE="tagacl_new_v9_hn1024_tau000" \
WANDB_RUN_PREFIX_OVERRIDE="new_v9_hn1024_tau000" \
WANDB_EXPERIMENT_FAMILY_OVERRIDE="tagged_acl_new_v9_hn1024_tau000" \
WANDB_VARIANT_PREFIX_OVERRIDE="new_v9_hn1024_tau000" \
WANDB_COMPUTE_TAG_OVERRIDE="compute:aries_direct_gpu01_seq" \
TERM_FCR_POLICY_OVERRIDE="term_map_source_ref_negative_sentence" \
RAG_MODEL_PATH_OVERRIDE="${HN1024_CKPT}" \
RAG_SCORE_THRESHOLD_OVERRIDE="0.0" \
...
for lm in 1 2 3 4; do
  out_dir="${OUT_ROOT}/${MODEL_LABEL}/zh/dtagacl_new_v9_hn1024_tau000_lm${lm}_k10_th0.0_gacl6060_tagged_gt_raw_min_norm2"
...
echo "[ALL DONE] Tagged ACL New V9 HN1024 tau0.0 raw zh lm1-lm4 complete"
