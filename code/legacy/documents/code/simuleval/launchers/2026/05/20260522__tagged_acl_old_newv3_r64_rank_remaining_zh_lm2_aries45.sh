...
# Continue old new_v3 r64/a128 rank quick eval after long W&B tag failure.
# Uses short W&B variant prefixes and reuses existing output roots when possible.
...
RUN_STAMP="${RUN_STAMP:-20260522T154327_resume}"
...
OUT_ROOT="${OUT_ROOT:-/mnt/aries/data7/jiaxuanluo/slm/tagged_acl_old_newv3_r64_rank_quick_zh_lm2_20260522T154327}"
LOG_ROOT="${LOG_ROOT:-/mnt/gemini/data1/jiaxuanluo/logs/tagged_acl_old_newv3_r64_rank_quick_20260522T154327_resume}"
...
run_one() {
  ...
  env \
    ...
    DENSITY_TAG_OVERRIDE="tagacl_${short_variant}_${run_kind}_tau073" \
    WANDB_RUN_PREFIX_OVERRIDE="${short_variant}_${run_kind}" \
    WANDB_EXPERIMENT_FAMILY_OVERRIDE="tagged_acl_old_newv3_r64_rank_quick" \
    WANDB_VARIANT_PREFIX_OVERRIDE="${short_variant}_${run_kind}" \
...
# Log the existing q159 paper110 TSV with a valid short W&B tag, then run the
# missing rj1v1p7r readouts.
run_one "q159wce4_newv3_r64a128_full" "q159r64" "${Q159_MODEL}" "paper110_extracted" "per_paper" "extracted" "2022.acl-long.110"
run_one "rj1v1p7r_newv3_random_r64a128" "rj1r64" "${RJ1_MODEL}" "full_raw" "full_corpus" "raw" "all"
run_one "rj1v1p7r_newv3_random_r64a128" "rj1r64" "${RJ1_MODEL}" "paper110_extracted" "per_paper" "extracted" "2022.acl-long.110"

echo "[DONE] Remaining rank quick evals under ${OUT_ROOT}"
