...
# Run SimulEval baseline (NO RAG) for RealSI en->zh:
# (1) Each talk uses its own glossary (only for output naming / bookkeeping).
...
# Base SimulEval sweep script (rank16 baseline, no RAG)
BASE_SIMULEVAL_SCRIPT="${ROOT_DIR}/documents/code/simuleval/rank16/baseline/bypass_simuleval_rank32_iter_0000452_hf_baseline_no_rag_sweep.sh"
...
OUTPUT_BASE_DEFAULT="/mnt/gemini/data2/jiaxuanluo/realsi_rank16_baseline_no_rag_per_talk"
...
RAG_K2_VALUES_OVERRIDE="${RAG_K2_VALUES_OVERRIDE:-}"  # naming only
...
# Fixed settings (must match the base baseline script naming)
RAG_HOP_SIZE="0.48"
RAG_K1_FIXED="10"
...
# 1) Run SimulEval baseline per talk
...
PY
  ...
  echo "[INFO] ============================================================"
  echo "[INFO] Running baseline talk_id=${TALK_ID}"
  ...
  (
    export GLOSSARY_PATHS_OVERRIDE="${GLOSSARY_PATH}"
    export SRC_LIST_OVERRIDE="${SRC_LIST_OVERRIDE_LOCAL}"
    export TGT_LIST_OVERRIDE="${TGT_LIST_OVERRIDE_LOCAL}"
    export OUTPUT_BASE_OVERRIDE="${OUTPUT_BASE}"
    export MODEL_NAME_OVERRIDE="${MODEL_NAME_OVERRIDE}"
    export LANG_CODE_OVERRIDE="${LANG_CODE}"
    export LATENCY_MULTIPLIERS_OVERRIDE="${LATENCY_MULTIPLIERS_OVERRIDE}"
    export RAG_K2_VALUES_OVERRIDE="${RAG_K2_VALUES_OVERRIDE}"
    export CUDA_VISIBLE_DEVICES_PHYSICAL_OVERRIDE="${CUDA_VISIBLE_DEVICES_PHYSICAL_OVERRIDE}"
    export CLEAN_OUTPUT_DIR_OVERRIDE="${CLEAN_OUTPUT_DIR_OVERRIDE}"
    export BACKUP_PARTIAL_RUNS="${BACKUP_PARTIAL_RUNS}"
    export RESUME_MODE="${RESUME_MODE}"
    bash "${BASE_SIMULEVAL_SCRIPT}"
  )
done

# 2) Post-evaluate term accuracy per talk
...
SUMMARY_TSV="${OUTPUT_BASE}/${LANG_CODE}/realsi_per_talk_baseline_streamlaal_term_summary.tsv"
...
for LATENCY_MULTIPLIER in "${LATENCY_MULTIPLIERS[@]}"; do
  for RAG_K2 in "${RAG_K2_VALUES[@]}"; do
    echo "[INFO] ============================================================"
    echo "[INFO] Evaluating baseline per-talk: lm=${LATENCY_MULTIPLIER} k2=${RAG_K2} th=${RAG_CONFIDENCE_THRESHOLD}"
...
PY
      ...
      MODEL_SHORT="$(basename "${MODEL_NAME_OVERRIDE:-baseline}")"
      OUTPUT_DIR="${OUTPUT_BASE}/${LANG_CODE}/${MODEL_SHORT}_g${GLOSSARY_TAG}_cs*_hs${RAG_HOP_SIZE}_lm${LATENCY_MULTIPLIER}_k2${RAG_K2}_k1${RAG_K1_FIXED}_th${THRESHOLD_TAG}"

      set +e
      OUTPUT_DIR_REAL="$(ls -dt ${OUTPUT_DIR} 2>/dev/null | head -n 1)"
      set -e
      if [[ -z "${OUTPUT_DIR_REAL}" ]]; then
        echo "[WARN] Output dir not found for talk_id=${TALK_ID} (lm=${LATENCY_MULTIPLIER} k2=${RAG_K2})" >&2
        continue
      fi

      INSTANCES_PATH="${OUTPUT_DIR_REAL}/instances.log"
      ...
      echo "${EVAL_OUT}" > "${OUTPUT_DIR_REAL}/post_eval_streamlaal_term.log"
      echo -e "${TALK_ID}\t${GLOSSARY_TAG}\t${LATENCY_MULTIPLIER}\t${RAG_K2}\t${BLEU_SCORE}\t${STREAM_LAAL}\t${STREAM_LAAL_CA}\t${TERM_ACC}\t${TERM_CORRECT}\t${TERM_TOTAL}\t${OUTPUT_DIR_REAL}" >> "${SUMMARY_TSV}"
    done
  done
done
