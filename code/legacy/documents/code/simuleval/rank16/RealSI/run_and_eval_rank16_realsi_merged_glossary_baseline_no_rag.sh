...
# Run SimulEval baseline (NO RAG) for RealSI en->zh using ONE merged glossary (naming/bookkeeping only),
# then post-evaluate with StreamLAAL + term accuracy.
#
...
# Merged glossary path (used for output naming + term accuracy evaluation)
...
OUTPUT_BASE_DEFAULT="/mnt/gemini/data2/jiaxuanluo/realsi_rank16_baseline_no_rag_merged_glossary"
...
echo "[INFO] Merged glossary (naming + eval): ${MERGED_GLOSSARY_PATH}"
echo "[INFO] SRC_LIST_OVERRIDE=${SRC_LIST_OVERRIDE_LOCAL}"
echo "[INFO] TGT_LIST_OVERRIDE=${TGT_LIST_OVERRIDE_LOCAL}"

# Run SimulEval baseline once with merged glossary (for naming only)
...
for LATENCY_MULTIPLIER in "${LATENCY_MULTIPLIERS[@]}"; do
  for RAG_K2 in "${RAG_K2_VALUES[@]}"; do
    set +e
    OUTPUT_DIR_REAL="$(
    ls -dt "${OUTPUT_BASE}/${LANG_CODE}/${MODEL_SHORT}_g${GLOSSARY_TAG}_cs"*_hs${RAG_HOP_SIZE}_lm${LATENCY_MULTIPLIER}_k2${RAG_K2}_k1${RAG_K1_FIXED}_th${THRESHOLD_TAG} 2>/dev/null | head -n 1
    )"
    set -e
    if [[ -z "${OUTPUT_DIR_REAL}" ]]; then
      echo "[WARN] Output dir not found for merged baseline (lm=${LATENCY_MULTIPLIER} k2=${RAG_K2})" >&2
      continue
    fi
    ...
    set -e
    if [[ "${EVAL_RC}" != "0" ]]; then
      echo "[WARN] stream_laal_term.py failed for merged baseline (rc=${EVAL_RC})" >&2
      echo "${EVAL_OUT}" >&2
      continue
    fi

    echo "${EVAL_OUT}" > "${OUTPUT_DIR_REAL}/post_eval_streamlaal_term.log"
    echo "[INFO] Evaluated merged baseline: output_dir=${OUTPUT_DIR_REAL} (lm=${LATENCY_MULTIPLIER} k2=${RAG_K2})"
  done
done
