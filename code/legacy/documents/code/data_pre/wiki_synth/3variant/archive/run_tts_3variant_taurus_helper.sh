...
#SBATCH --array=4-7
#SBATCH --output=/mnt/gemini/data1/jiaxuanluo/logs/%A-%a_tts_3v_h.out
#SBATCH --error=/mnt/gemini/data1/jiaxuanluo/logs/%A-%a_tts_3v_h.err
#SBATCH --chdir=/tmp

# Helper shards on taurus to speed up 3-variant TTS.
# Same num-shards=8 as aries job — both sides race on the same shard,
# whichever finishes a WAV first wins (the other skips via file-exists check).
...
echo "================================================================"
echo "[TTS] 3-variant HELPER on taurus (racing with aries)"
...
echo "[TTS] Helper shard ${SHARD_ID} completed at $(date)"
