...
SOURCE_DATA_ROOT="${SOURCE_DATA_ROOT_OVERRIDE:-/mnt/gemini/data1/jiaxuanluo}"
OUTPUT_ROOT="${OUTPUT_ROOT_OVERRIDE:-/mnt/data1/jiaxuanluo/speech_llm_zh_cap16_denoise_budget_20260526}"
LOG_ROOT="${LOG_ROOT_OVERRIDE:-/mnt/data1/jiaxuanluo/logs/zh_cap16_denoise_budget_ttag_20260526T0437}"
CAP16_OUT_ROOT="${CAP16_OUT_ROOT_OVERRIDE:-${OUTPUT_ROOT}/cap16_exactboundary}"
BRANCH_DIR="${OUTPUT_ROOT}/zh/hn1024_tau078_cap16_denoise_budget_ttag_v1"

CAP16_BUILDER="${ROOT_DIR}/documents/code/data_pre/hard_negative_jsonl_for_speech_llm/launchers/2026/05/20260525__build_deja_termmap_ablation_cap16_exactboundary.sh"
...
SRC_DIR="${CAP16_OUT_ROOT}/zh/retriever_hn1024_tau078_cap16_exactboundary"
TRAIN_RETRIEVED="${SRC_DIR}/train_s_zh_retriever_results_hn1024_tau078.jsonl"
DEV_RETRIEVED="${SRC_DIR}/dev_s_zh_retriever_results_hn1024_tau078_first355.jsonl"
STAGE1_TRAIN="${BRANCH_DIR}/train_s_zh_retriever_hn1024_tau078_cap16_denoise_budget_stage1.jsonl"
STAGE1_DEV="${BRANCH_DIR}/dev_s_zh_retriever_hn1024_tau078_cap16_denoise_budget_stage1_first355.jsonl"
FINAL_TRAIN="${BRANCH_DIR}/train_s_zh_retriever_hn1024_tau078_cap16_denoise_budget_ttag_exactboundary.jsonl"
FINAL_DEV="${BRANCH_DIR}/dev_s_zh_retriever_hn1024_tau078_cap16_denoise_budget_ttag_exactboundary_first355.jsonl"

ALLOCATED_GPU_CSV="${TCM_BUILD_GPU_DEVICES_OVERRIDE:-${CUDA_VISIBLE_DEVICES:-0,1,2,3}}"
WAIT_FOR_GPUS="${WAIT_FOR_GPUS:-1}"
WAIT_MAX_SECONDS="${WAIT_MAX_SECONDS:-0}"
WAIT_POLL_SECONDS="${WAIT_POLL_SECONDS:-60}"
WAIT_GPU_MAX_USED_MIB="${WAIT_GPU_MAX_USED_MIB:-1000}"
WAIT_GPU_MAX_UTIL_PCT="${WAIT_GPU_MAX_UTIL_PCT:-10}"
...
for p in \
  ...
  "${REBUILD_SCRIPT}" \
  "${WRAP_SCRIPT}" \
  "${SOURCE_DATA_ROOT}/train_s_zh_v4_ner_baseline_aligned_rate1.0_k20_final.jsonl" \
  "${SOURCE_DATA_ROOT}/glossary_for_zh_rate1.0_k20.json"; do
...
wait_for_selected_gpus() {
  ...
  IFS=',' read -r -a selected_gpus <<< "${ALLOCATED_GPU_CSV}"
  local started now elapsed query ok row mem util gpu
  started="$(date +%s)"
  while true; do
    ...
    for gpu in "${selected_gpus[@]}"; do
      gpu="${gpu// /}"
      row="$(awk -F, -v id="${gpu}" '{gsub(/ /, "", $1); if ($1 == id) print $0}' <<< "${query}" | head -n 1)"
      if [[ -z "${row}" ]]; then
        echo "[ERROR] Could not inspect GPU ${gpu}" >&2
        exit 5
      fi
      IFS=',' read -r _ mem util <<< "${row}"
      mem="${mem// /}"
      util="${util// /}"
      if (( mem > WAIT_GPU_MAX_USED_MIB || util > WAIT_GPU_MAX_UTIL_PCT )); then
        ok=0
        break
      fi
    done
    if (( ok == 1 )); then
      echo "[WAIT] GPUs ready: ${ALLOCATED_GPU_CSV}"
      return 0
    fi
    now="$(date +%s)"
    elapsed=$((now - started))
    if (( WAIT_MAX_SECONDS > 0 && elapsed >= WAIT_MAX_SECONDS )); then
      echo "[ERROR] Timed out waiting for GPUs ${ALLOCATED_GPU_CSV} after ${elapsed}s" >&2
      exit 75
    fi
    echo "[WAIT] GPUs ${ALLOCATED_GPU_CSV} busy; sleeping ${WAIT_POLL_SECONDS}s"
    echo "${query}"
...
echo "[INFO] SOURCE_DATA_ROOT=${SOURCE_DATA_ROOT}"
echo "[INFO] OUTPUT_ROOT=${OUTPUT_ROOT}"
echo "[INFO] CAP16_OUT_ROOT=${CAP16_OUT_ROOT}"
echo "[INFO] BRANCH_DIR=${BRANCH_DIR}"
echo "[INFO] LOG_ROOT=${LOG_ROOT}"
echo "[INFO] ALLOCATED_GPU_CSV=${ALLOCATED_GPU_CSV}"
df -h /mnt/gemini/data1 /mnt/data1 || true

if [[ ! -s "${TRAIN_RETRIEVED}" || ! -s "${DEV_RETRIEVED}" || "${FORCE_OVERWRITE}" == "1" ]]; then
  echo "[STAGE] Build zh HN1024 tau0.78 cap16 exact-boundary retriever data"
  wait_for_selected_gpus
  DATA_ROOT_OVERRIDE="${SOURCE_DATA_ROOT}" \
    OUT_ROOT_OVERRIDE="${CAP16_OUT_ROOT}" \
    LOG_ROOT_OVERRIDE="${LOG_ROOT}/cap16_exactboundary" \
    LANGS_OVERRIDE="zh" \
    BRANCHES_OVERRIDE="retriever" \
    FORCE_OVERWRITE="${FORCE_OVERWRITE}" \
    TCM_BUILD_GPU_DEVICES_OVERRIDE="${ALLOCATED_GPU_CSV}" \
    bash "${CAP16_BUILDER}"
else
  echo "[SKIP] Existing cap16 retrieved JSONLs found under ${SRC_DIR}"
fi

for p in "${TRAIN_RETRIEVED}" "${DEV_RETRIEVED}"; do
  [[ -s "${p}" ]] || { echo "[ERROR] Missing cap16 retrieved path after build: ${p}" >&2; exit 6; }
done

echo "[STAGE] Rebuild zh train/dev term maps with denoise budget policy"
...
schedule = {
    "version": "zh_cap16_denoise_budget_ttag_v1",
...
echo "[DONE] ZH cap16 denoise-budget short-tag data ready: ${BRANCH_DIR}"
