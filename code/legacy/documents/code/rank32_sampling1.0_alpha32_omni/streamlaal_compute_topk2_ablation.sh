...
# Compute StreamLAAL/TERM metrics from SimulEval instances.log (JSONL) and summarize into TSV.
# This script is aligned with documents/code/rank32_sampling1.0_alpha32_omni/streamlaal_compute_transcript_sampling_10.sh
...
# SimulEval outputs (topk2 ablation)
OUTPUT_ZH_BASE="/mnt/gemini/data2/jiaxuanluo/infinisst_omni_vllm_rag_rank32_iter_0000452_hf_zh_topk2_ablation/zh"
OUTPUT_DIR_GLOB="iter_0000452-hf_cs*_hs*_lm*_rk*_vk*"
...
# MWER segmenter (required by the FBK tool in some environments)
...
# Output
SUMMARY_TSV="${OUTPUT_ZH_BASE}/topk2_ablation_streamlaal_summary.tsv"
...
# sed extraction patterns for fields from dirname (basic regex with escaped capture groups)
...
for out in "${OUTPUT_ZH_BASE}"/${OUTPUT_DIR_GLOB}; do
  ...
  if [[ ! -f "${INSTANCES_PATH}" ]]; then
    echo "[WARN] Missing ${SIMULEVAL_INSTANCES_FILE} (skip): ${out}" >&2
    continue
  fi
  if [[ ! -s "${INSTANCES_PATH}" ]]; then
    echo "[WARN] Empty ${SIMULEVAL_INSTANCES_FILE} (skip): ${out}" >&2
    continue
  fi

  echo "[INFO] Post-eval: ${out}"

  # Do not let a non-zero eval exit code abort the entire TSV generation.
  set +e
  EVAL_OUT="$(
