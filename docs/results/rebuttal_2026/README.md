...
状态：**100-request pilot 与 full Flash request preparation 已完成；16/16 shards 均为
`PREPARED`、0/16 已提交，等待作者确认约 50 USD 预算并轮换 Gemini key**。
...
- API key 只允许从 Taurus mode `0600` 私有文件读取，key 值不进入 Git、命令、
  ...
  [`llm_judge_pilot_100.md`](llm_judge_pilot_100.md)。
- 正式候选 bundle 位于 Taurus `full_flash_api_default/`；run config SHA-256 为
  `2d997a3f8c151dec9e2e108da41a60a32451a8a753878d9beb9e0bc474afeb35`，manifest
  SHA-256 为
  `37053c87fb926338bce3323443b6017622f55c8a18b127c682faaffe3ee81a88`。它只冻结
  requests/sidecars/state，不是评分结果。
