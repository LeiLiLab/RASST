...
- **Legacy paper-derived glossary v1 的 `lm=2` 对比已完成。** 使用五篇论文各自的
  Gemini-extracted glossary，RASST 相对复用既有 `instances.log` 重算的 InfiniSST，
  En-Zh/Ja/De exact-form TERM_ACC 分别提高 `+14.89/+17.84/+25.00` points；BLEU
  差值为 `+0.3448/-0.0257/-2.4515`。这里的 denominator 是各篇 paper-derived
  glossary 在 reference 中覆盖的 occurrence，不是完整 raw-gold ACL glossary，
  因此这是 matched-glossary diagnostic，不能表述为 raw-gold coverage。完整协议、
  三语结果与 hashes 见
...
状态：**legacy v1 matched-glossary `lm=2` 已完成；fresh Gemini 2.5 extraction
仍 blocked，推理未启动**。

- Legacy v1 的 RASST 三语运行和 InfiniSST 三语 post-eval 已全部完成。InfiniSST
  没有重新推理；post-eval 的 BLEU/StreamLAAL 与登记 baseline 全精度一致，只重算
  glossary-sensitive metrics。统一对比见
...
- Legacy v1 保留了 glossary 文件与 SHA-256，但原始 extraction 旁没有记录 exact
  ...
  `gemini-2.5-flash` reproduction，也不能直接替换 rebuttal 草稿中要求完整 raw-gold
  denominator 和 exact extractor provenance 的保护标记。
