...
\caption{Quality--latency trade-off on ACL 60/60. X-axis is latency measured by StreamLAAL in milliseconds. Y-axis is quality measured by terminology accuracy (exact match), BLEU and XCOMET-XXL. \method improves terminology accuracy and BLEU in all three directions.}
...
\paragraph{Evaluation} We evaluate on two datasets. The ACL 60/60 dev set~\citep{salesky-etal-2023-evaluating} contains five English ACL talks translated into 10 target languages. We evaluate En--Zh/De/Ja directions and feed each unsegmented talk directly to the model. Each talk comes with a technical term list annotated by domain experts. We use these human-curated terminology lists as the retrieval glossary and after source-side deduplication it contains 238 terms.

The ESO test set~\citep{eso_dataset} comprises five long-form English oncology training videos with human translations into French, Spanish, German, and Slovene. We report En--De direction and use GPT-5.4\footnote{\url{https://developers.openai.com/api/docs/models/gpt-5.4}} to extract and filter domain-specific glossary candidates, including procedures, diagnoses, biomarkers, clinical-trial concepts, and acronyms, and manually verify the retained entries. The final glossary has 212 unique source terms. Appendix~\ref{app:eso_glossary} gives the prompts. To test larger retrieval glossary, we expand the ACL and ESO glossaries to 1K and 10K entries using unseen Wikidata terms from relevant domains.
...
We evaluate aligned sentence triplets (transcript, hypothesis, reference) using SacreBLEU~\citep{papineni-etal-2002-bleu,post-2018-call} for surface-form accuracy and XCOMET-XXL~\citep{guerreiro-etal-2024-xcomet} for semantic accuracy.


\paragraph{Terminology Accuracy}
We introduce terminology accuracy, which measures how accurately a model translates domain-specific terms. Our primary metric is exact match: for each term occurrence in the reference, we check whether its translation appears verbatim in the aligned hypothesis.

To account for valid orthographic and inflectional variation, we also report a form-aware metric. This metric accepts differences in case, spacing, and hyphenation across all languages, as well as contiguous German lemma sequences identified by a fixed morphological analyzer. It does not accept synonyms or semantic paraphrases. Appendix~\ref{app:form_aware_metric} provides the complete evaluation protocol and results.
...
For the retriever, we use a Qwen3-Omni Audio Transformer\footnote{\url{https://huggingface.co/Atotti/Qwen3-Omni-AudioTransformer}} as the speech encoder, followed by a linear projection. For text encoding, we use the dense BGE-M3\footnote{\url{https://huggingface.co/BAAI/bge-m3}} encoder and use the contextualized representation at its first \textsc{[CLS]} position as the term vector. For the Speech LLM, we use Qwen3-Omni-30B-A3B-Instruct\footnote{\url{https://huggingface.co/Qwen/Qwen3-Omni-30B-A3B-Instruct}}.

\paragraph{Training}
We train both the cross-modal retriever and the Speech LLM with LoRA~\citep{hu2022lora}. For retriever training, the number of hard negative $N$ is 1024 and the global batch size is 8192. For Speech LLM training, we enable sequence packing with a maximum packed sequence length of 3,072 tokens and use a global batch size of 4 packed sequences. Full hyperparameters and details are provided in Appendix~\ref{app:training_details}.
