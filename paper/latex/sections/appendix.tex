...
We fine-tune the Speech LLM using LoRA with rank $r{=}8$ and $\alpha{=}32$.
We enable sequence packing with a maximum sequence length of 2028 tokens.
We optimize with AdamW (learning rate $1\times10^{-4}$, weight decay 0.01) and gradient clipping (max norm 1.0), with a global batch size of 4.
Training is performed with DDP and expert parallelism (EP) of size 2 on two L40S GPUs.
...
\crchange{This section reports additional ablations that do not fit in the main text.}
...
\caption{Precision–recall curves across glossary sizes for retrievers trained with $N{=}0, 256, 1024$ hard negatives, obtained by sweeping $\tau \in [0,1]$. The dashed vertical line marks $\tau{=}0.78$, the operating point at which recall drops by 1\% from $\tau{=}0$ at the 1K glossary.}
...
\section{\texorpdfstring{\crchange{Form-Aware Terminology Accuracy}}{Form-Aware Terminology Accuracy}}
\label{app:form_aware_metric}

\begin{crblock}
The primary exact-form scorer first aligns each system output to the reference sentences with the same mWER-based segmentation used by StreamLAAL. A glossary entry is eligible when its English source form occurs in the aligned source sentence and its prescribed target form occurs in the reference. Target forms are deduplicated, and each eligible target form is counted once per sentence.

Our form-aware diagnostic preserves this denominator and exact-match logic. For misses, it applies Unicode NFKC normalization, case folding, and removal of spacing and hyphenation differences. For German only, it additionally accepts an equal contiguous lemma sequence produced by spaCy 3.8.5 with \texttt{de\_core\_news\_sm} 3.8.0. The diagnostic does not accept synonyms or semantic paraphrases. It was run on existing hypotheses, not newly decoded outputs. Across 32 system settings, it scored 27,296 eligible occurrences; every row with archived exact numerator and denominator reproduced the original exact count.
\end{crblock}
...
\bottomrule
\end{tabular}
\caption{\crchange{RASST exact-form and conservative form-aware TERM\_ACC (\%) at latency multiplier 2.}}
\label{tab:form_aware}
\end{table}

\begin{crblock}
The largest adjustment is in German, where inflection and productive compounding create valid surface variants. At latency multiplier 2, ACL En--De InfiniSST also rises from 67.59 to 74.44, while \method rises from 82.99 to 87.38; the form-aware margin therefore remains 12.94 points. On ESO En--De the corresponding margin is 23.03 points (82.23 versus 59.20). Thus normalization reduces both systems' apparent error, without changing the conclusion that retrieval improves terminology realization.
\end{crblock}

\subsection{\texorpdfstring{\crchange{XCOMET-XXL}}{XCOMET-XXL}}
...
\begin{crblock}
\method has higher XCOMET in 8 of 12 ACL settings. Its mean difference is $+0.116$ points: $+0.636$ for En--Zh, $+0.005$ for En--De, and $-0.293$ for En--Ja. The En--Ja pattern changes with latency: retrieval hurts at the two lowest-latency settings but helps at the two higher-latency settings. We hypothesize that the base model's weaker partial-context Japanese translation makes additional terminology hints harder to integrate when little speech is available. Table~\ref{tab:eso_de_main} reports the ESO En--De XCOMET results; there \method is lower in all four settings, by 1.385 points on average.
\end{crblock}

\section{\texorpdfstring{\crchange{Term-Tag Supervision}}{Term-Tag Supervision}}

\begin{crblock}
The \texttt{\textless term\textgreater} tags in Speech LLM targets explicitly identify when a retrieved translation is realized. We train an otherwise matched En--Zh model without these tags. As shown in Table~\ref{tab:term_tag_ablation}, the tagged model improves exact-form TERM\_ACC by 3.60--5.06 points across all latency settings. Its mean BLEU is also comparable (48.5536 versus 48.4728), supporting the use of tags as training-time supervision. The tags are stripped before evaluation and are not required in user-facing output.
\end{crblock}
...
\section{\texorpdfstring{\crchange{Qualitative and Failure Analysis}}{Qualitative and Failure Analysis}}
...
\bottomrule
\end{tabular}
\caption{\crchange{Manual categorization of cases where InfiniSST is exact-match correct and \method is not.}}
...
Of the 14 genuinely absent terms, seven are not retrieved because their similarity falls just below $\tau{=}0.78$. The other seven are retrieved but not used: some hints arrive near the end of a sentence after the model has committed to another rendering, while others are omitted because the concept is inferable from context. The remaining 22 cases are genuine substitutions or hallucinations, including ``BETO'' rendered as ``BERT.'' Acoustic acronym confusions also occur without retrieval, such as ``BLEU'' rendered as ``blue.'' These cases suggest that better confidence calibration and delayed commitment may complement retrieval.
\end{crblock}

\section{\texorpdfstring{\crchange{Robustness to Glossary and Retrieval Quality}}{Robustness to Glossary and Retrieval Quality}}

\subsection{\texorpdfstring{\crchange{Paper-Derived Glossaries}}{Paper-Derived Glossaries}}

\begin{crblock}
To approximate a less curated setting, we extract a separate glossary from the paper PDF corresponding to each ACL talk using Gemini, without consulting the transcripts, references, or human evaluation glossary. These glossaries overlap with only 57 of 238 human glossary entries (23.95\% source-term coverage). We use the paper-derived glossary for \method retrieval but retain the human glossary as the fixed TERM\_ACC denominator for both systems. Table~\ref{tab:paper_glossary} shows the latency-multiplier-2 results. As expected, the gains are much smaller because retrieval cannot supply entries absent from the index. Nevertheless, terminology accuracy improves for En--Zh and En--De. The extraction is a legacy run for which the exact Gemini model identifier was not archived, so we treat it as a robustness diagnostic rather than a fully reproducible headline result.
\end{crblock}
...
\bottomrule
\end{tabular}
\caption{\crchange{ACL latency-multiplier-2 evaluation with paper-derived retrieval glossaries. Cells show \method / InfiniSST; TERM uses the unchanged human glossary.}}
\label{tab:paper_glossary}
\end{table}

\subsection{\texorpdfstring{\crchange{Retrieval Degradation}}{Retrieval Degradation}}

\begin{crblock}
We replace 25\% or 50\% of sentence-relevant retrieved entries with irrelevant entries from the same ACL glossary, holding the number of hints and inference configuration fixed. Table~\ref{tab:retrieval_degradation} reports one fixed corruption seed at latency multiplier 2. Exact-form TERM\_ACC decreases by at most 7.59 points under 50\% replacement and remains above the corresponding InfiniSST accuracy in all three directions. All six corrupted settings have lower XCOMET than their language's uncorrupted setting, whereas BLEU is not monotonic, illustrating why a semantic quality metric is useful. Because this is a one-seed controlled corruption rather than naturally occurring interpreter noise, it measures sensitivity rather than full real-world robustness.
\end{crblock}
...
\bottomrule
\end{tabular}
\caption{\crchange{Sensitivity to same-domain retrieval corruption at latency multiplier 2. XCOMET denotes XCOMET-XXL multiplied by 100.}}
\label{tab:retrieval_degradation}
\end{table}

\FloatBarrier
\section{\texorpdfstring{\crchange{ESO Glossary Construction}}{ESO Glossary Construction}}
