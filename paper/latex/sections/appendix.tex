...
\begin{crblock}
The primary exact-form scorer first aligns each system output to the reference sentences with the same mWER-based segmentation used by StreamLAAL. A glossary entry is eligible when its English source form occurs in the aligned source sentence and its prescribed target form occurs in the reference. Target forms are deduplicated, and each eligible target form is counted once per sentence. Thus, the exact and form-aware metrics use an identical denominator.
