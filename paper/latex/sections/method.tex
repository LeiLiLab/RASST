...
\begin{crblock}
Let $\vs=(s_1,\ldots,s_T)\in\mathbb{R}^{T}$ denote a waveform sampled at $f_s$ Hz. A bilingual glossary is $\mathcal{G}=\{(e_j,\tau_j)\}_{j=1}^{M}$, where $e_j$ is a source-language expression and $\tau_j$ is its prescribed target-language form. For a chunk duration $\ell$ seconds, let $L=\lfloor f_s\ell\rceil$ be the corresponding number of samples. At step $i\geq 1$, the system receives
\begin{align}
    \vc_i = \bigl(s_{(i-1)L+1},\ldots,s_{\min(iL,T)}\bigr),
\end{align}
and has observed the prefix $\vs_{\leq i}=\bigl(s_1,\ldots,s_{\min(iL,T)}\bigr)$.

The retriever $R_\phi$, parameterized by $\phi$, selects a step-local set of glossary entries
\begin{align}
    G_i = R_\phi(\vs_{\leq i},\mathcal{G})\subseteq\mathcal{G}. \label{eqn:G_i}
\end{align}
Let $\hat{\vy}_i=(\hat{y}^{i}_1,\ldots,\hat{y}^{i}_{n_i})$ be the possibly empty target segment emitted at step $i$, and let $\hat{\vy}_{<i}=\hat{\vy}_1\oplus\cdots\oplus\hat{\vy}_{i-1}$ denote the previously emitted prefix. The generation policy is
\begin{align}
    \hat{\vy}_i \sim \pi_\theta\!\left(\cdot\mid \vs_{\leq i},\hat{\vy}_{<i},G_{1:i}\right).
\end{align}
If $n_i=0$, the policy waits; otherwise the segment is appended to the output. Every token emitted at step $i$ is assigned delay $\min(iL,T)/f_s$ seconds. The complete hypothesis is $\hat{\vy}=\hat{\vy}_1\oplus\cdots\oplus\hat{\vy}_I$, which is distinct from the reference translation $\vy^\star$. Quality and latency are evaluated using $\hat{\vy}$, $\vy^\star$, the source transcript $\vx^\star$, and the per-token delays.
\end{crblock}
...
\begin{align}
\crchange{\hat{\vy}_i} &\crchange{\sim \mathrm{SpeechLLM}(\vc_1, G_1, \hat{\vy}_1, \ldots, \vc_i, G_i).}
\end{align}
After each chunk \crchange{$\vc_i$} arrives, the retriever returns candidate terms $G_i$ likely to appear in it, and we append these terms along with their target-language translations to the chunk's representation. Conditioned on the speech context and the retrieved terms, the LLM then generates the next partial translation \crchange{$\hat{\vy}_i$}.

\subsection{Cross-Modal Retriever}
\label{sec:retriever}

\paragraph{Architecture}
We adopt a dual-encoder architecture. For text, we use the pre-trained BGE-M3 encoder~\citep{bge-m3} to map each source-language term $e_j$ to a feature vector $\vg_j\in\mathbb{R}^d$. For speech, we use the Audio Transformer of Qwen3-Omni~\citep{yang2025qwen3technicalreport} followed by a linear projection to the same dimension $d$. Since terms are typically short, the speech encoder reads only the current chunk \crchange{$\vc_i$} together with a look-back window of duration $t_{lb}$, rather than the full history $\vs_{\leq i}$ as in Equation \ref{eqn:G_i}. We denote this input as \crchange{$\vc_i'$} and the resulting features as $\vf_i\in\mathbb{R}^{m\times d}$, with $m$ as the number of frames in \crchange{$\vc_i'$}.
...
\end{equation}
The similarity between the speech \crchange{$\vc_i'$} and a term $e_j$ is then the maximum cosine similarity over all windows:
\begin{equation}
  \text{Sim}(\crchange{\vc_i'},e_j)
  = \max_{1\leq u<v\leq m}
  \text{CosSim}(\vf_i^{u,v},\vg_j),
\end{equation}
and the retrieved terms are
\begin{align}
    G_i = \text{Top-}K_j\left(\text{Sim}(\crchange{\vc_i'},e_j)\right)\subseteq \mathcal{G}
\end{align}
...
-\log \frac{ \exp(\text{Sim}(\crchange{\vc_i'},e_\text{pos})/\gamma) }{ \sum_{e\in E_\text{all}} \exp(\text{Sim}(\crchange{\vc_i'},e)/\gamma) },
...
$ during simultaneous translation, we synthesize a corresponding training corpus. Given the speech chunks \crchange{$\vc_{1:i}$}, we follow InfiniSST's procedure to produce the translation segments \crchange{$\hat{\vy}_{1:i}$}. The remaining piece beyond InfiniSST is the retrieved terms $G_{1:i}$. A naive approach is to reuse the noun-phrase terms from retriever training and always populate $G_i$ with the ground-truth term. This, however, would leave the model brittle to retriever mistakes at inference time. To reduce this training–inference mismatch, we build a glossary over all noun phrases in the training set and run the cross-modal retriever from Section~\ref{sec:retriever} on every speech chunk. Then we add the retrieved distractors in $G_i$ alongside the ground-truth term. The Speech LLM thus learns to translate in the presence of realistic retriever noise.

A training example is shown in Figure \ref{fig:train_prompt_visual}. We wrap the ground-truth term's translation in a \texttt{\textless term\textgreater\ldots\textless/term\textgreater} tag, encouraging the Speech LLM to copy ground-truth term translations directly from $G_i$. We fine-tune with a standard cross-entropy objective applied only to translation tokens \crchange{$\hat{\vy}$}. Because translations naturally lag behind speech in the training data, the model also receives implicit supervision for \emph{timing}: it observes when a term appears in the speech context and when its translation appears later in the target sequence.
