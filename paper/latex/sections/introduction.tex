...
However, they still struggle to accurately translate domain-specific \crchange{terminology}, including technical jargon, person and location names, and abbreviations.
\begin{CJK*}{UTF8}{gkai}
...
\begin{crblock}
In this work, \emph{terminology} means a specialized word, phrase, or expression whose target form is specified by a task- or domain-specific bilingual glossary. Examples include technical concepts, method and dataset names, proper names, and acronyms. This operational definition makes the boundary with ordinary vocabulary explicit: an expression is evaluated as terminology only when it is designated by the supplied glossary; glossary construction is external to \method.
\end{crblock}
...
To address these challenges, we propose \textbf{R}etrieval-\textbf{A}ugmented \textbf{S}imultaneous \textbf{S}peech \textbf{T}ranslation (\method). \method trains a lightweight speech--text retriever on variable-duration speech contexts with a contrastive loss, and performs multi-scale inference for accurate retrieval under partial input. We further synthesize training data that teaches the Speech LLM to select relevant retrieved terms and decide when to introduce them during generation, conditioned on the prior speech context and previously generated translation. \crchange{Across En$\rightarrow$Zh/De/Ja on ACL 60/60, \method improves exact-form terminology accuracy over InfiniSST in all 12 latency settings. BLEU improves in 10/12 settings and XCOMET in 8/12, showing that the strongest and most consistent benefit is terminology control rather than a universal gain in general translation quality. On ESO, we report only En$\rightarrow$De, for which the dataset provides a human reference.}
